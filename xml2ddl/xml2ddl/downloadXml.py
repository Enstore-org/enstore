...
#        _nodups = dict(map(lambda i: (i,1),x)).keys()
#        for mangledName in mangledNames:
# litvinse@fnal.gov
        for mangledName in dict(map(lambda i: (i,1),mangledNames)).keys():
...
# 
            rows = self.db.getFunctionDefinitions(mangledName)
            for row in rows:
                info = {
                    'name'       : row[0],
                    'definition' : row[4],
                    'arguments'  : ', '.join(row[1]),
                    'returns'    : row[2],
                    }
...
def parseCommandLine():
    ...
    parser.add_option("", "--port",
        dest="strPort", metavar="PORT", default=None,
