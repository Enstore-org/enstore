...
class FileDB(DbTable):
	def __init__(self, host='localhost', port=8889, jou='.', database=default_database, rdb=None, auto_journal=1):
		...
		self.retrieve_query = "\
        		select \
                		bfid, crc, deleted, drive, migration_state, \
				old_volume.label, new_volume.label, \
				old_location_cookie, new_location_cookie, \
				pnfs_path, old_pnfs_id, new_pnfs_id, \
                		sanity_size, sanity_crc, size \
        		from file, volume \
        		where \
                		file.old_volume = volume.id and \
				file.new_volume = volume.id and \
				file.migration_state = state.id and \
                		bfid = '%s';"
		self.insert_query = "\
	...
	def export_format(self, s):
		...
		return {
			...
			'old_volume': s['old_volume'],
			'new_volume': s['new_volume'],
			'old_location_cookie': s['old_location_cookie'],
			'new_location_cookie': s['new_location_cookie'],
			'pnfs_name0': s['pnfs_path'],
			'old_pnfsid': s['old_pnfs_id'],
			'new_pnfsid': s['new_pnfs_id'],
	...
	def import_format(self, s):
		...
		return {
			...
			'deleted': deleted,
			'migration_state':s['migration_state']
			'old_volume': ('lookup_vol', s['old_volume']),
			'new_volume': ('lookup_vol', s['new_volume']),
			'old_location_cookie': s['old_location_cookie'],
			'new_location_cookie': s['new_location_cookie'],
			'pnfs_path': s['pnfs_name0'],
			'old_pnfs_id': s['old_pnfsid'],
			'new_pnfs_id': s['new_pnfsid'],
...
class VolumeDB(DbTable):
	...
	def import_format(self, s):
		...
		return {
			...
			'files_staged': s['files_staged']
			'files_migrated': s['files_migrated']
			'storage_group': sts[0],
			'file_family': sts[1],
			'wrapper': sts[2]
			'state': s['state']
