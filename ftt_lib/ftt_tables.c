...
ftt_dev_entry devtable[] = {
    ...
    /*   string                  den mod hwd   pas fxd rewind            1st */
    /*   ======                  === === ===   === === ======            === */
        { "/dev/rmt%d.5",        0,  0, 0x15, 0,  0,                 0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        0,  0, 0x00, 0,  0,                 0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",       -1,  0, 0x00, 1,  0,                 0, 0, EXB_MAX_BLKSIZE},
    /* Variable ---------------------------------------------------------- */
        { "/dev/rmt%d.4",        0,  0, 0x15, 0,  0, FTT_RWOC|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        0,  0, 0x15, 0,  0,                 0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.6",        0,  0, 0x15, 0,  0, FTT_RWOC|FTT_RTOO, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.7",        0,  0, 0x15, 0,  0,        0|FTT_RTOO, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.4",        0,  1, 0x8c, 0,  0, FTT_RWOC|       0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        0,  1, 0x8c, 0,  0,                 0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.6",        0,  1, 0x8c, 0,  0, FTT_RWOC|FTT_RTOO, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.7",        0,  1, 0x8c, 0,  0,        0|FTT_RTOO, 0, EXB_MAX_BLKSIZE},
    ...
    /* Fixed --------------------------------------------------------------*/
        { "/dev/rmt%d.4",        0,  0, 0x15, 0,  1, FTT_RWOC|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        0,  0, 0x15, 0,  1,                 0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.6",        0,  0, 0x15, 0,  1, FTT_RWOC|FTT_RTOO, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.7",        0,  0, 0x15, 0,  1,        0|FTT_RTOO, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.4",        0,  1, 0x8c, 0,  1, FTT_RWOC|       0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        0,  1, 0x8c, 0,  1,                 0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.6",        0,  1, 0x8c, 0,  1, FTT_RWOC|FTT_RTOO, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.7",        0,  1, 0x8c, 0,  1,        0|FTT_RTOO, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.4",        1,  0, 0x14, 0,  1, FTT_RWOC|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        1,  0, 0x14, 0,  1,                 0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.6",        1,  0, 0x14, 0,  1, FTT_RWOC|FTT_RTOO, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.7",        1,  0, 0x14, 0,  1,        0|FTT_RTOO, 1, EXB_MAX_BLKSIZE},
    ...
    /* Unused so as not to mess with system usage--------------------------*/
        { "/dev/rmt%d",          1,  0, 0x00, 0,  1, FTT_RWOC|       0, 0, 512},
        { "/dev/rmt%d.1",        1,  0, 0x00, 0,  1,                 0, 0, 512},
        { "/dev/rmt%d.2",        1,  0, 0x00, 0,  1, FTT_RWOC|FTT_RTOO, 0, 512},
        { "/dev/rmt%d.3",        1,  0, 0x00, 0,  1,          FTT_RTOO, 0, 512},
        { 0, },
    }},
	...
	FTT_OP_UNLOAD|FTT_OP_STATUS|FTT_OP_GET_STATUS,ftt_trans_table, DLT_density_trans,
	"/dev/rmt%d", 1, AIXfind,  {
    /*   string                  den mod hwd   pas fxd rewind            1st */
    /*   ======                  === === ===   === === ======            === */
    ...
    /* Variable ---------------------------------------------------------- */
        ...
        { "/dev/rmt%d.4",        4,  1, 0x19, 0,  0, FTT_RWOC|       0|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        4,  1, 0x19, 0,  0,                          0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.6",        4,  1, 0x19, 0,  0, FTT_RWOC|FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.7",        4,  1, 0x19, 0,  0,          FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.4",        5,  1, 0x1A, 0,  0, FTT_RWOC|       0|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        5,  1, 0x1A, 0,  0,                          0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.6",        5,  1, 0x1A, 0,  0, FTT_RWOC|FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.7",        5,  1, 0x1A, 0,  0,          FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
    /* Fixed    ---------------------------------------------------------- */
        ...
        { "/dev/rmt%d.4",        4,  0, 0x19, 0,  1, FTT_RWOC|       0|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        4,  0, 0x19, 0,  1,                          0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.6",        4,  0, 0x19, 0,  1, FTT_RWOC|FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.7",        4,  0, 0x19, 0,  1,          FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.4",        5,  0, 0x1A, 0,  1, FTT_RWOC|       0|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        5,  0, 0x1A, 0,  1,                          0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.6",        5,  0, 0x1A, 0,  1, FTT_RWOC|FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.7",        5,  0, 0x1A, 0,  1,          FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.4",        4,  1, 0x19, 0,  1, FTT_RWOC|       0|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        4,  1, 0x19, 0,  1,                          0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.6",        4,  1, 0x19, 0,  1, FTT_RWOC|FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.7",        4,  1, 0x19, 0,  1,          FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.4",        5,  1, 0x1A, 0,  1, FTT_RWOC|       0|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.5",        5,  1, 0x1A, 0,  1,                          0, 0, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.6",        5,  1, 0x1A, 0,  1, FTT_RWOC|FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
        { "/dev/rmt%d.7",        5,  1, 0x1A, 0,  1,          FTT_RTOO|       0, 1, EXB_MAX_BLKSIZE},
    /* Unused so as not to interfere with system usage-------------------- */
        { "/dev/rmt%d",          0,  0, 0x00, 0,  1, FTT_RWOC|       0, 0, 512},
        { "/dev/rmt%d.1",        0,  0, 0x00, 0,  1,                 0, 0, 512},
        { "/dev/rmt%d.2",        0,  0, 0x00, 0,  1, FTT_RWOC|FTT_RTOO, 0, 512},
    ...
    {"IRIX+5", "DLT4", "SCSI", FTT_FLAG_CHK_BOT_AT_FMK, 
...
/* Compressed */
    ...
    {"IRIX+5", "DLT2", "SCSI", FTT_FLAG_CHK_BOT_AT_FMK, 
	FTT_OP_GET_STATUS, ftt_trans_table, DLT_density_trans,
	"/dev/rmt/tps%dd%d", 2, IRIXfind, {
    /*   string                    den mod hwd   pas fxd rewind            ,1st*/
    /*   ======                    === === ===   === === ======            === */
...
/* Compressed */
    ...
    {"IRIX+5", "DLT4", "SCSI-VME", FTT_FLAG_CHK_BOT_AT_FMK, 
	FTT_OP_GET_STATUS, ftt_trans_table, DLT_density_trans,
	"/dev/rmt/jag%dd%d", 2, IRIXfind, {
    /*   string                    den mod hwd   pas fxd rewind            ,1st*/
    /*   ======                    === === ===   === === ======            === */
...
/* Compressed */
    ...
    {"IRIX+5", "DLT2", "SCSI-VME", FTT_FLAG_CHK_BOT_AT_FMK, 
	FTT_OP_GET_STATUS, ftt_trans_table, DLT_density_trans,
	"/dev/rmt/jag%dd%d", 2, IRIXfind, {
    /*   string                    den mod hwd   pas fxd rewind            ,1st*/
    /*   ======                    === === ===   === === ======            === */
