...
class Queue:
    ...
    def put_queue(self, queue_item, tid=None):
        ...
        try:
            if self.queue.get(tid, None) is None:
                self.queue[tid] = []
...
def get_font(height_wanted, family='Helvetica', fit_string="", width_wanted=0):
    # def get_font(height_wanted, family='Courier', fit_string="",
    # width_wanted=0):
...
def hextorgb(hexcolor):
    if not isinstance(hexcolor, bytes):
        return 0, 0, 0
    ...
    for system_name, csc_addr in known_config_servers.items():
        ...
        while i < 3:
            ...
            if not csc:
                ...
                try:
                    sys.stderr.write(
                        "Unable to obtain ip information.  Aborting.\n")
                    sys.stderr.flush()
                except IOError:
                    pass
                sys.exit(1)
...
class Mover:
            ...
            # if current state is in text and the new state is in text.
                ...
                if self.display.type(self.state_display) == "text":
                    ...
                    self.state_display = self.display.create_text(x, y,
                                                                  font=self.font,
                                                                  text=fit_string(
                                                                      self.font, self.state, self.state_width),
    ...
    def draw_background_progress(self):  # , percent_done, alt_percent_done):
        if self.percent_done is None and self.alt_percent_done is None:
            return
    ...
    def draw_media_progress(self):
        if self.alt_percent_done is None:
            return
    ...
    def update_rate(self, rate):
        ...
        if rate is not None:
            if (old_rate is None or old_rate == 0.0) and rate != 0.0:
    ...
    def update_progress(self, percent_done, alt_percent_done):
        ...
        if percent_done is None and alt_percent_done is None:
            self.percent_done = None
            self.alt_percent_done = None
            self.undraw_progress()
            return
    ...
    def update_offline_reason(self, offline_reason):
        ...
        if self.state in ['OFFLINE', 'Unknown', 'ERROR']:
            if offline_reason is None:  # and not self.offline_reason:
    ...
    def position(self):
        ...
        if position is None:
            ...
            for i in range(len(self.display.client_positions.keys())):
                ...
                Trace.trace(0, pprint.pformat(
                    self.display.client_positions[i + 1].item_positions))
...
class MoverDisplay(Tkinter.Toplevel):
    ...
    def __init__(self, mover, **attributes):
        ...
        # self.format_mover_status(self.get_mover_status())
        self.status_text = ""
    ...
    def receive_mover_status_request(self, timeout):
        if self.mc:
            if self.transaction_ids:
                ...
                if status.get('state', None) is None:
...
class Column:
    ...
    def get_max_limit(self):
        if self.column_limit is None:
            return MIPC
        return self.column_limit

    def set_max_limit(self, limit):
        if isinstance(limit, int) and limit > 0 and limit <= MIPC:
            if self.column_limit is None or self.column_limit < limit:
    ...
    def del_item(self, index_or_name):
        if isinstance(index_or_name, int):  # We have an index.
...
# XXXXXXXXXXXXXXXXXX  --get rid of scrollbars--
    ...
    def is_up_to_date(self, command):
        ...
        if words and words[0] in ["state"]:
            ...
            if mover and mover.volume is None and words[2] in \
                ["HAVE_BOUND", "SEEK", "ACTIVE", "CLEANING", "DRAINING"] \
                or \
                mover and self.connections.get(mover.name, None) is None \
    ...
    def smooth_animation(self):
        ...
        try:
            ...
            raise_(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2])
        except BaseException:  # Tkinter.TclError
            exc, msg, tb = sys.exc_info()
            traceback.print_exception(exc, msg, tb)
            del tb  # Avoid resource leak.
        ...
        try:
            ...
            for mover in self.movers.values():
                ...
                if mover.state in ['ERROR']:
                    if mover.offline_reason is None:
    ...
    def toggle_library_managers_enabled(self):
        for mover_name, mover in self.movers.items():
            if isinstance(mover.library, list):
    ...
    def client_command(self, command_list):
        ...
        try:
            ...
            if client is None:  # it's a new client
                ...
                # If the number of client columns changed we need to
                # reposition.
                new_number = self.get_client_column_count()
                if old_number != new_number:
                    self.reposition_canvas(force=1)
    ...
    def disconnect_command(self, command_list):
        ...
        try:
            ...
            # Get these python objects representing what is drawn on the
            # screen.
            ...
            if self.master.show_waiting_clients.get():
                if len(client.mover_names) == 0:
                    # change fill color if needed
                    client.update_state(COUNTDOWN)
    ...
    def get_valid_command(self):
        ...
        if len(words) < self.comm_dict[words[0]]['length']:
            Trace.trace(1, "Insufficent length for %s command: %s" %
                        (words[0], string.join(words, "")))
            return ""
