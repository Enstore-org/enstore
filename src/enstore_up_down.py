...
def enstore_state(status):
    # given the status accumulated from all of the servers, determine the
    # state of enstore
...
class EnstoreServer:
    ...
    def need_to_send_mail(self):
        rc = 0
        if (self.seen_down_d.get(self.format_name, 0) %
                self.allowed_down) == 0:
            rc = 1
        return rc
    ...
    def writemail(self, message):
        ...
        # to be down in a row.
        if self.format_name in self.seen_down_d and self.need_to_send_mail():
            # see if this server is known to be down, if so, then do not send
            # mail
            if self.format_name not in self.offline_d:
    ...
    def set_status(self, status):
        # do not set the status if we have an override value which will be the
        # status
        self.real_status(status)
        if status == enstore_constants.DOWN:
            self.seen_down_d[self.format_name] = self.seen_down_d.get(
                self.format_name, 0) + 1
            ...
            if self.format_name in self.seen_down_d:
    ...
    def get_enstore_state(self, state, reason):
        if self.format_name not in self.offline_d:
            if self.status == enstore_constants.DOWN:
                # en_status records the state of enstore when the server is
                # done
    ...
    def check(self, ticket):
        if not 'status' in ticket.keys():
            ...
            if ticket['status'][0] == e_errors.TIMEDOUT:
                ...
                enprint(
                    "%s  BAD STATUS %s" %
                    (self.format_name, ticket['status']))
                self.set_status(enstore_constants.DOWN)
...
class ConfigServer(EnstoreServer):
    ...
    def __init__(self, offline_d, override_d, seen_down_d, allowed_down_d):
        ...
        self.config_port = string.atoi(
            os.environ.get('ENSTORE_CONFIG_PORT', 7500))
...
class LibraryManager(EnstoreServer):
    ...
    def __init__(self, name, config, offline_d,
                 override_d, seen_down_d, allowed_down_d):
    ...
    def mover_status(self):
        ...
        for mover in self.movers:
            ...
            if mover.format_name not in self.offline_d:
...
class MediaChanger(EnstoreServer):
    ...
    def __init__(self, name, offline_d, override_d,
...
def do_real_work():
    ...
    summary_d = {
        enstore_constants.TIME: enstore_functions2.format_time(
            time.time())}
    ...
    # warnings need to be generated if more than 50% of a library_managers
    # movers are down.
        ...
        if not server.name in total_servers_names:
            ...
            if state == LOW_CAPACITY:
                enprint("LOW CAPACITY: Found, %s of %s movers not responding or in a bad state" % (bad_movers,
                                                                                                   server.num_movers))
                server.writemail(
                    "Found LOW CAPACITY movers for %s" %
                    (server.name,))
    ...
    if cs.get_real_status() == enstore_constants.UP and \
       inquisitor.get_real_status() == enstore_constants.UP:
        ...
        # if the name is in the total_servers_names list, then no heartbeat was
        # received
        if len(total_servers_names) > 0:
            enprint(
                "Looking for heartbeat info from inq for %s" %
                (total_servers_names,))
            rtn_ticket = inq.get_last_alive(total_servers_names)
            enprint(
                "Got heartbeat info from inq for %s" %
                (rtn_ticket['servers']))
            ...
            for server in servers_l:
                if now_seconds - \
                        rtn_ticket['servers'][server] < seconds_in_5mins:
    ...
    # now check if there is an override set to make sure that enstore is
    # marked down
    ...
    if summary_d[enstore_constants.ENSTORE] == enstore_constants.DOWN:
        ...
        # generate a ticket
        if enstore_constants.ENSTORE not in offline_d:
            ...
            if summary_d.get(log.format_name, "") == enstore_constants.UP and \
                ...
                remedy_type = REMEDY_TYPE_D.get(
                    cs.config_host[0:2], cs.config_host)
                # the following line will set the alarm function
                alc = alarm_client.AlarmClient(
                    (cs.config_host, cs.config_port))
                Trace.init("Enstore_Up_Down")
                Trace.alarm(
                    e_errors.INFO, e_errors.ENSTOREBALLRED, {
                        'Reason': repr(reason)})
                ...
                anAlarm.ticket()
                enprint(
                    "%s - LOG, ALARM, or CONFIG server not running, a ticket will NOT be generated!!!" %
                    (e_errors.ENSTOREBALLRED,))
        else:
            enprint(
                "%s - Enstore is marked down, a ticket will NOT be generated!!!" %
                (e_errors.ENSTOREBALLRED,))
    else:
        stat = "UP"
        rtn = 0
    ...
    subject = "Please check Enstore System (config node - %s)" % (
        cs.config_host,)
    ...
    if (not no_mail) and need_to_send:
        if enstore_constants.ENSTORE not in offline_d:
            os.system(
                "/usr/bin/Mail -s \"%s\" $ENSTORE_MAIL < %s" %
    ...
    enprint(
        "Finished checking Enstore... system is defined to be %s" %
        (stat,))
