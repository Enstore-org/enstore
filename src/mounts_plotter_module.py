...
class MountsPlotterModule(enstore_plotter_module.EnstorePlotterModule):
    ...
    def fill(self, frame):
        ...
        # get list of library managers available in config and then select only
        # those to plot
        libraries = enstore_stop.find_servers_by_type(
            frame.get_configuration_client(),
            enstore_constants.LIBRARY_MANAGER)
