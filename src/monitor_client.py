...
class MonitorServerClient(generic_client.GenericClient):
    ...
    def _test_encp_transfer(self, data_sock, block_size,
        ...
        while bytes_transfered < bytes_to_transfer:
            ...
            if w or r or ex:
                ...
                try:
                    ...
                    return_value = transfer_function(*args)
                    ...
                    if return_value == 0:
                        ...
                        # equal to zero).
                        data_sock.close()
                        raise generic_client.ClientError(
    ...
    def _simulate_encp_transfer(self, ticket):
        ...
        except generic_client.ClientError as msg:
            raise msg
...
class Vetos:
    ...
    def is_vetoed_item(self, ip):
        ...
        return ip_as_canon in self.veto_item_dict
...
def get_all_ips(config_host, config_port, csc):
    ...
    if not e_errors.is_ok(x):
        raise generic_client.ClientError(
            "error from config server: %s" %
            (x['status'],))
    if 'server_list' not in x:
    ...
    for k in server_dict.keys():
        ...
        details = csc.get(k)
        if 'data_ip' in details:
            ip = details['data_ip']  # check first if a mover
        elif 'hostip' in details:
