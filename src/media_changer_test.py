...
class MediaLoaderMethods(dispatching_worker.DispatchingWorker,
    ...
    # wrapper method for client - server communication - replaced viewvol
    # above tgj1
    def viewvol(self, ticket):
    ...
    def insertvol(self, ticket):
        ...
        if "newlib" not in ticket:
            ticket["status"] = (
                e_errors.WRONGPARAMETER,
                37,
                "new library name not specified")
            Trace.log(
                e_errors.ERROR,
                "ERROR:insertvol new library name not specified")
            return
    ...
    def load(self,
        ...
        if isinstance(self.set_this_error, type(
                1)) and self.set_this_error > 9990:
            return("ERROR", self.set_this_error, None, None, None)
        else:
            return (self.set_this_error, 0, None)

    # unload volume from the drive;  default overridden for other media
    # changers
    def unload(self,
               external_label,  # volume external label
               drive,           # drive id
    ...
    def DoWork(self, function, ticket):
        ...
        if ticket['function'] in ("mount", "dismount"):
            ...
            if "external_label" not in ticket['vol_ticket']:
                print(
                    "MISSING EXTERNAL_LABEL",
                    ticket)  # XXX What is going on?
                e = "MISSING EXTERNAL LABEL for %s %s" % (
            ...
            for i in self.work_list:
                try:
                    if (i['function'] == "cleanCycle" and 'drive_id' in i and
        ...
        if ticket["function"] != "getVolState":
            if len(self.work_list) >= self.max_work:
                Trace.log(
                    ...
                    ticket['function'])
                return
              # elif work queue is temporarily closed, assume client will
              # resend
        ...
        # set the reply address - note this could be a general thing in
        # dispatching worker
        ticket["ra"] = (
            self.reply_address,
            self.client_number,
        ...
        for i in self.work_list:
            if i["ra"] == ticket["ra"]:
                Trace.log(
                    e_errors.INFO, "duplicate request, drop it %s %s" %
                    (repr(
                        i["ra"]), repr(
                        ticket["ra"])))
                return
        ...
        if ticket['function'] == "mount":
            ...
            # don't print a failure  (no tape mounted) message that is really a
            # success
            self.logdetail = 0
            sts = self.prepare(
                'Unknown',
            ...
            Trace.trace(
                11, 'mcDoWork> child prepare dismount for %s returned %s' %
                (msg, sts[2]))
        if ticket['function'] in (
                'insert', 'eject', 'homeAndRestart', 'cleanCycle', 'getVolState'):
            Trace.trace(11, 'mcDoWork> child doing %s' % (msg,))
            sts = function(ticket)
            Trace.trace(11, 'mcDoWork> child %s returned %s' % (msg, sts))
        else:
            Trace.trace(11, 'mcDoWork> child doing %s' % (msg,))
            # ok, this is a test only - see if we can mount readonly for 9840
            # and 9940 tapes
            media_type = ticket['vol_ticket']['media_type']
            if ticket['vol_ticket']['media_type'] in (
                    '9840', '9940', '9940B') and ticket['function'] == 'mount':
                if enstore_functions2.is_readonly_state(ticket['vol_ticket']['system_inhibit'][1]) or enstore_functions2.is_readonly_state(
                        ticket['vol_ticket']['user_inhibit'][1]):
            ...
            Trace.trace(
                11, 'mcDoWork> child doing %s. Media type %s' %
                (msg, media_type))
            sts = function(
                ticket['vol_ticket']['external_label'],
                ticket['drive_id'],
        ...
        # so dispatching_worker calls WorkDone
    ...
    def WorkDone(self, ticket):
        ...
        if ticket['function'] in ("mount", "dismount"):
            ...
            Trace.log(
                level, 'FINISHED %s returned %s' %
                (ticket['function'], fstat))
        # reply_with_address uses the "ra" entry in the ticket
        self.reply_with_address(ticket)
        self.robotNotAtHome = 1
        self.lastWorkTime = time.time()
        # check for cleaning jobs
        if len(self.work_list) < self.max_work and len(
                self.work_cleaning_list) > 0:
...
class AML2_MediaLoader(MediaLoaderMethods):
    ...
    def load(self,
        ...
        return self.retry_function(
            aml2.mount, external_label, drive, media_type)
    ...
    def unload(self,
        ...
        import aml2
        return self.retry_function(
            aml2.dismount, external_label, drive, media_type)
    ...
    def robotHomeAndRestart(self, ticket):
        ...
        return self.retry_function(
            aml2.robotHomeAndRestart, ticket, classTicket)
    ...
    def checkMyself(self):
        ...
        if self.robotNotAtHome and (
                time.time() - self.lastWorkTime) > self.idleTimeLimit:
...
class STK_MediaLoader(MediaLoaderMethods):
    ...
    def __init__(self, medch, max_work=7, csc=None):
        ...
        self.prepare = self.unload
        self.DEBUG = 0
        self.driveCleanTime = self.mc_config.get(
            'DriveCleanTime', {
                '9840': [
                    60, 1], '9940': [
                    60, 1]})
    ...
    def load(self,
             ...
             media_type):       # media type
        return self.retry_function(
            self.mount, external_label, drive, media_type)
    ...
    def unload(self,
               ...
               media_type):     # media type
        return self.retry_function(
            self.dismount, external_label, drive, media_type)
...
#		   print "nread=",nread, "line=",nl, l
        ...
        status = 0
        for look in range(
                19, size):  # 1st part of response is STK copyright information
...
class Manual_MediaLoader(MediaLoaderMethods):
    ...
    def loadvol(self, ticket):
        if ticket['vol_ticket']['external_label']:
            rc = os.system(
                "mc_popup_test 'Please load %s'" %
                (ticket['vol_ticket']['external_label'],)) >> 8
        if rc:
            self.set_this_error = rc + 9990

        else:
            self.set_this_error = e_errors.OK
        return MediaLoaderMethods.loadvol(self, ticket)

    def unloadvol(self, ticket):
        if ticket['vol_ticket']['external_label']:
            rc = os.system(
                "mc_popup_test 'Please unload %s'" %
                (ticket['vol_ticket']['external_label']),) >> 8
        if rc:
            self.set_this_error = rc + 9990
        else:
...
class Shelf_MediaLoader(MediaLoaderMethods):
    ...
    # Note: max_work may need to be changed, tgj
    def __init__(self, medch, max_work=1, csc=None):
        MediaLoaderMethods.__init__(self, medch, max_work, csc)
        ...
        Trace.trace(
            e_errors.INFO, "Shelf init localHost=%s OCSHost=%s" %
            (self.localHost, self.ocsHost))
    ...
    def checkRemoteConnection(self):
        ...
        fnstatus = 'OK'
        command = self.cmdPrefix + \
            "echo $(hostname) ; echo $?" + self.cmdSuffix
        ...
        Trace.log(
            e_errors.INFO, "Shelf cRC enrsh return strings=%s stat=%s" %
            (result, stat))
        if stat == 0:
    ...
    def checkOCSalive(self):
        ...
        result = pipeObj.fromchild.readlines()  # result has returned string
        Trace.log(
            e_errors.INFO, "Shelf cOa enrsh return strings=%s stat=%s" %
            (result, stat))
        if stat == 0:
            retval = result[len(result) - 1][0]
            if retval != '0':
    ...
    def allocateOCSdrive(self, drive):
        ...
        fnstatus = 'OK'
        command = self.cmdPrefix + "ocs_allocate -T " + \
            drive + " ; echo $?" + self.cmdSuffix
        ...
        result = pipeObj.fromchild.readlines()  # result has returned string
        Trace.log(
            e_errors.INFO, "Shelf aOd enrsh return strings=%s stat=%s" %
            (result, stat))
        if stat == 0:
            retval = result[len(result) - 1][0]
            if retval != '0':
                ...
                if pos == -1:  # different drive was allocated
                    ...
                    if pos != -1:
                        ...
                        Trace.log(
                            e_errors.ERROR, "ERROR:Shelf aOd enrsh wrongdrive=%s" %
                            (wrongdrive,))
    ...
    def mountOCSdrive(self, external_label, drive):
        ...
        result = pipeObj.fromchild.readlines()  # result has returned string
        Trace.log(
            e_errors.INFO, "Shelf mOd enrsh return strings=%s stat=%s" %
            (result, stat))
        if stat == 0:
            retval = result[len(result) - 1][0]
            if retval != '0':
    ...
    def deallocateOCSdrive(self, drive):
        ...
        result = pipeObj.fromchild.readlines()  # result has returned string
        Trace.log(
            e_errors.INFO, "Shelf dOd enrsh return strings=%s stat=%s" %
            (result, stat))
        if stat == 0:
            retval = result[len(result) - 1][0]
            # check if drive already deallocated (not an error)
            if retval != '0':
                retstring = result[0]
    ...
    def unmountOCSdrive(self, drive):
        ...
        result = pipeObj.fromchild.readlines()  # result has returned string
        Trace.log(
            e_errors.INFO, "Shelf uOd enrsh return strings=%s stat=%s" %
            (result, stat))
        if stat == 0:
            retval = result[len(result) - 1][0]
            if retval != '0':
    ...
    def load(self, external_label, drive, media_type):
        ...
        return self.status_code(
            fnstatus), status, self.status_message(fnstatus)
    ...
    def unload(self, external_label, drive, media_type):
        ...
        Trace.log(
            e_errors.INFO, "Shelf unload deallocate exit fnstatus=%s" %
            (fnstatus,))
        if fnstatusTmp != 'OK':
