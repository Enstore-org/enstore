...
def check_volatile_files(db_name):
    ...
    for pnfsid in pnfsids:
        try:
            ...
            if (check_layer_2(l2)):
                if (is_volatile(l2)):
                    sql_txt = "delete from volatile_files where pnfsid_string='%s'" % (
                        pnfsid,)
                    r = db.query(sql_txt)
                    continue
            if (check_layer_1(l1) and check_layer_4(l4)):
                ...
                if not check_layer_4(l4):
                    l4_str = "n"
                sql_txt = "update volatile_files set layer1='%s',layer2='%s',layer4='%s',pnfs_path='%s' where pnfsid_string='%s'" % (
                    l1_str, l2_str, l4_str, p.pnfsFilename, pnfsid,)
                r = db.query(sql_txt)
    ...
    for row in res.getresult():
        ...
        try:
            ...
            for line in f.readlines():
                ...
                if (is_file == 0 and data[0].strip(
                        " ") == "Type" and data[1].strip(" ") == "--I---r----"):
                    is_file = 1
            f.close()
        if (is_file == 1):
            try:
...
def prepare_html(db_name):
    ...
    #
    now_time = time.time()
    stmt = "select count(*) from volatile_files where layer2='n' and unix_date<%d" % (
        int(now_time - 3600))
    if (db_name == "netflow"):
        stmt = "select count(*) from volatile_files where layer2='n' and unix_date<%d" % (
            int(now_time - 12 * 3600))
    res = db.query(stmt)
    count = 0
    for row in res.getresult():
        if not row:
    ...
    if (count != 0):
        ...
        sql_txt = "select date, pnfsid_string, layer1, layer2, layer4, pnfs_path from volatile_files where layer2='n' and unix_date<%d order by date asc" % (
            int(now_time - 3600))
        os.system("rm -f %s" % fname)
        cmd = "psql  %s  -U enstore -o %s -c \"%s;\"" % (
            db_name, fname, sql_txt)
        os.system(cmd)
        cmd = "su  enstore -c \'/usr/local/etc/setups.sh 1>>/dev/null 2>&1; cd /tmp; $ENSTORE_DIR/sbin/enrcp %s %s/\'" % (
            fname, destination)
        os.system(cmd)
    delta_time = 25 * 3600
    if (db_name == "minos"):
    ...
    # now_time       = time.time()
    res = db.query(
        "select count(*) from volatile_files where layer2='y' and unix_date<%d" %
        (int(
            now_time -
            delta_time),
         ))
    ...
    if (count1 != 0):
        fname = "%s.txt" % (db_name,)
        sql_txt = "select date, pnfsid_string, layer1, layer2, layer4, pnfs_path from volatile_files where layer2='y' and unix_date<%d  order by date asc" % (
...
def do_mail(db_name):
    ...
    stmt = "select count(*) from volatile_files where layer2='n' and (unix_date<%d and unix_date>%d)" % (
        int(now_time - 3600), then_time)
    if (db_name == "netflow"):
        stmt = "select count(*) from volatile_files where layer2='n' and (unix_date<%d and unix_date>%d)" % (
            int(now_time - 12 * 3600), then_time)
    res = db.query(stmt)
    count = 0
    for row in res.getresult():
        if not row:
    ...
    if (count != 0):
        fname = "%s_bad.txt" % (db_name,)
        sql_txt = "select date, pnfsid_string, layer1, layer2, layer4, pnfs_path from volatile_files where layer2='n' and (unix_date<%d and unix_date>%d) order by date asc" % (
            int(now_time - 3600), then_time)
        if (db_name == "netflow"):
            sql_txt = "select date, pnfsid_string, layer1, layer2, layer4, pnfs_path from volatile_files where layer2='n' and (unix_date<%d and unix_date>%d) order by date asc" % (
                int(now_time - 12 * 3600), then_time)
    ...
    res = db.query(
        "select count(*) from volatile_files where layer2='y' and (unix_date<%d and unix_date>%d)" %
        (now_time, then_time))
    count1 = 0
    for row in res.getresult():
        if not row:
    ...
    if (count1 != 0):
        fname = "%s.txt" % (db_name,)
        sql_txt = "select date, pnfsid_string, layer1, layer2, layer4, pnfs_path from volatile_files where layer2='y' and (unix_date<%d and unix_date>%d) order by date asc" % (
            now_time, then_time)
        cmd = "psql  %s -U enstore -o %s -c \"%s;\"" % (
            db_name, fname, sql_txt)
        os.system(cmd)
...
if __name__ == '__main__':
    ...
    cmd = "su  enstore -c \'/usr/local/etc/setups.sh 1>>/dev/null 2>&1; cd /tmp; $ENSTORE_DIR/sbin/enrsh {} \"rm {}/*.txt\"\'".format(
        destination_host, destination_dir)
    os.system(cmd)
    yes_mail = False
...
#    while 0 in exitmutexes: pass
    ...
    if (yes_mail):
        ...
        os.system(
            'mail dcache-auto@fnal.gov -s "There are files with missing layers older than 24 hours" < mail.txt')
