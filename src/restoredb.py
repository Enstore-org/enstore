...
def cross_check(db, jou, deleted, fix=False):
	...
	for i in jou.keys():
		if not db.has_key(i):
			...
			if fix:
				db[i] = jou[i]
				print 'F> database['+i+'] =', `db[i]`
		elif ddiff(jou[i], db[i]):
			print "C> database and journal disagree on key('"+i+"')"
			print "C>  journal['"+i+"'] =", `jou[i]`
			print "C> database['"+i+"'] =", `db[i]`
			error = error + 1
			if fix:
				db[i] = jou[i]
				print 'F> database['+i+'] =', `db[i]` 
...
if __name__ == "__main__":   # pragma: no cover
	...
	for i in fj:
		if fj[i].has_key('pnfsvid'):
			del fj[i]['pnfsvid']
		if fj[i].has_key('update'):
			del fj[i]['update']
	...
	error = cross_check(fdb, fj, fd, fix=True) + cross_check(vdb, vj, vd, fix=True)
