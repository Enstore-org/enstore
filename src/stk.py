...
def query(volume, media_type="", seqNo=0):
    ...
    cmd_lookfor = "ACSSA> query vol %s" % (volume,)
    ...
    if size <= 19:
        ...
        Trace.log(e_errors.ERROR, "ERROR 2: %s %s" % (command+" => ",response))
        return (ERROR, 2,  response, '', command)
    if size <= 20:
        Trace.log(e_errors.ERROR, "ERROR 3: %s %s" % (command+" => ",response))
    ...
    if size <= 22:
        ...
        Trace.log(e_errors.INFO, "TAPE 4: %s %s" % (command+" => ",answer))
    ...
    if string.find(answer, answer_lookfor,0) != 0:
        Trace.log(e_errors.ERROR, "ERROR 5: %s %s" % (command+" => ",answer))
        ...
        Trace.log(e_errors.ERROR, "TAPE 6: %s %s" % (command+" => ",answer))
...
def query_drive(drive):
    ...
    cmd="%s %s %s" % (RSH,command,CMD_PROC)
    cmd_lookfor = "ACSSA> query drive %s" % (drive,)
    ...
    if size <= 22:
        answer = string.strip(response[20])
        Trace.log(e_errors.INFO, "DRIVE 4: %s %s" % (command+" => ",answer))
        return (TAPE, 4, answer, '', command)
    ...
    if string.find(answer, answer_lookfor,0) != 0:
        ...
        Trace.log(e_errors.ERROR, "DRIVE 6: %s %s" % (command+" => ",answer))
        return (TAPE, 6, answer, '', command)
...
status_table = (
    (DRIVE   "in use"),
    (DRIVE   "not in library"),
    (DRIVE   "offline"),
    (DRIVE   "reserved"),
    (DRIVE   "invalid drive"),
    (DRIVE   "invalid drive type"),
    (TAPE,   "misplaced tape"),
    (TAPE,   "unreadable label"),
    (TAPE,   "invalid volume"),
    (TAPE,   "volume in transit"),
    (TAPE,   "volume not found"),
    (TAPE,   "volume deleted"),
    (TAPE,   "volume access denied"),
    (TAPE,   "volume not in library")
    )
