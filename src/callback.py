...
def timeout_send(sock, msg, timeout=15 * 60):
    ...
    if isinstance(sock, int):  # In case sock is an fd for a pipe.
...
def _send_raw(sock, message, message_length, timeout):
    ...
    while ptr < message_length:
        nwritten, error_string = timeout_send(sock, message[ptr:],
                                              timeout)
        if not isinstance(nwritten, int) or nwritten <= 0:
            break
        ptr = ptr + nwritten
...
def write_tcp_obj(sock, obj, timeout=15 * 60):
    if not isinstance(sock, int) and not hasattr(sock, "fileno"):
...
def read_raw(fd, timeout=15 * 60):
    # Read in the length of the payload part of the message or the protocol
    # string for a new protocol
    ...
    try:
        ...
        if tmp[:8] == PROTOCOL:
            ...
            tmp, error_string = timeout_recv(
                fd, MSG_LEN_POSITIONS, timeout)  # the message length
            len_tmp = len(tmp)
