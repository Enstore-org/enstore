...
def is_ip(ip):
    if not isinstance(ip, type("")):
        raise_(TypeError, "Expected string type, not %s." % type(ip))

    if re.match(r"[0-9]{1,3}(\.[0-9]{1,3}){0,3}", ip):
        return 1
...
def __my_gethostbyaddr(addr):
    ...
    while try_count < 60:
        try:
            ...
            Trace.trace(
                19, "__my_gethostbyaddr: socket.gethostbyaddr returned %s" %
                (rc,))
            return rc
            ...
            Trace.trace(
                19, "__my_gethostbyaddr: exception %s" %
                (this_herrno,))
...
def update_domains(csc_or_dict):
    ...
    elif isinstance(csc_or_dict, dict):
        domains = csc_or_dict
...
def _allow(addr):
    ...
    try:
        ...
        Trace.log(
            e_errors.ERROR, '_allow: gethostbyaddr failed for %s: %s' %
            (addr, e,))
        return 0
    ...
    try:
        ...
        Trace.log(
            e_errors.ERROR, '_allow: gettaddrinfo failed for %s: %s' %
            (addr, e,))
        return 0
    ...
    for invalid_domains in invalid_domains_dict.values():
        for v in invalid_domains:
            ...
            if tok[:len(vtok)] == vtok:
                Trace.log(
                    e_errors.INFO, "allow: in invalid domains, not allowing %s" %
                    (addr,))
                return 0
...
def allow(addr):
    ...
    if isinstance(addr, type(())):
        if len(client_addr) == 2:
            ...
            raise_(TypeError, "Tuple addr has wrong length %s." % len(addr))
    if not isinstance(addr, type("")):
        raise_(TypeError, "Variable addr is of type %s." % type(addr))
...
def interface_name(ip):
    ...
    if not ip:
        return
    if not ifinfo or ip not in ifinfo:
