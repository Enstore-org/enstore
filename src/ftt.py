...
class FTT:
    ...
    def open_dev(self): return check(ftt_low.ftt_open_dev(self.d))
    def close(self): return check(ftt_low.ftt_close(self.d))
    def close_dev(self): return check(ftt_low.ftt_close_dev(self.d))
    def read(self, buf, length): return check(ftt_low.ftt_read(self.d, buf, length), (0,-1))
    def write(self, buf, length): return check(ftt_low.ftt_write(self.d, buf, length))
    def writefm(self): return check(ftt_low.ftt_writefm(self.d))
    def write2fm(self): return check(ftt_low.ftt_write2fm(self.d))
    def retry(self): raise NotImplementedError
    def skip_fm(self, count): return check(ftt_low.ftt_skip_fm(self.d, count))
    def skip_rec(self, count): return check(ftt_low.ftt_skip_rec(self.d, count))
    def skip_to_double_fm(self): return check(ftt_low.ftt_skip_to_double_fm(self.d))
    def rewind(self): return check(ftt_low.ftt_rewind(self.d))
    def retension(self): return check(ftt_low.ftt_retension(self.d))
    def unload(self): return check(ftt_low.ftt_unload(self.d))
    def erase(self): return check(ftt_low.ftt_erase(self.d))
    def set_mode(self, density, compression, blocksize):
        return check(ftt_low.ftt_set_mode(self.d, density, compression, blocksize))
    def get_mode(self): return ftt_low.ftt_get_mode(self.d)
    ...
    def name_to_density(self, name):
        return ftt_low.ftt_density_to_name(self.d, density)
    def get_max_blocksize(self): return ftt_low.ftt_get_max_blocksize(self.d)
    def set_data_direction(self, n): return check(ftt_low.ftt_set_data_direction(
        self.d, n))
    def list_all(self):
