...
class LibraryManagerClient(generic_client.GenericClient):
            ...
            if string.find(key, lmname) != -1:
                ...
                if pending_read_cnt:
                    ...
                    for work in pend_reads:
                        ...
                        reject_reason = ("", "")
                        vol_msg = ''
                        if 'reject_reason' in work:
                            reject_reason = work['reject_reason']
                        else:
                            vol = ''
                            if 'external_label' in work['fc']:
                ...
                if pending_write_cnt:
                    ...
                    for work in pend_writes:
                        ...
                        if 'reject_reason' in work:
                            reject_reason = work['reject_reason']
                        vol = ''
                        if 'external_label' in work['vc']:
                            vol = work['vc']['external_label']
                        vol_msg = ''
                        if vol:
                            vol_msg = 'VOL %s' % (vol,)
                        ff_msg = ''
                        if 'file_family' in work['vc']:
                            ff_msg = string.join(
                                (ff_msg, "FF", work['vc']['file_family']), ' ')
                        if 'file_family_width' in work['vc']:
                            ff_msg = string.join(
                                (ff_msg, "FF_W %s" %
                                 (work['vc']['file_family_width'],)), ' ')
                        if (host == node) or (not node):
                            print(
                                "%s %s %s %s %s P %d %s %s %s %s" %
                                (host,
                                 self.name,
                                 user,
                                 fn,
                ...
                for work in at_list:
                    ...
                    if (host == node) or (not node):
                        ...
                        print(
                            "%s %s %s %s %s M %s %s" %
                            (host, self.name, user, f1, f2, mover, vol))
    ...
    def remove_active_volume(self, volume):
        return self.send(
            {"work": "remove_active_volume", "external_label": volume})

    def priority(self, id, pri):
        return self.send({"work": "change_priority",
                          "unique_id": id, "priority": pri})
    ...
    def getlist(self, work):
        ...
        if not r:
            raise e_errors.EnstoreError(
                errno.ETIMEDOUT,
                "timeout waiting for library manager callback",
                e_errors.TIMEDOUT)
        ...
        if not hostaddr.allow(address):
            ...
            listen_socket.close()
            raise e_errors.EnstoreError(
                errno.ETIMEDOUT, "address %s not allowed" %
                (address,), e_errors.TIMEDOUT)
    ...
    def reset_pending_queue_counters(self, timeout=0, tries=0):
        return self.send(
            {"work": "reset_pending_queue_counters"}, timeout, tries)
...
def do_work(intf):
    ...
    if ticket:
        ...
        for mover in ticket['movers']:
            ...
            # print "%-10s %-17s %-08s %-14s(%-05s) (%-08s %08s) (%-08s %08s)
            # %-17s %-08s" %\
    ...
    elif intf.get_queue is not None:
        ...
        if intf.start_draining:
            if intf.start_draining == 'lock':
                lock = 'locked'
            elif not (intf.start_draining in ('ignore', 'pause', 'noread', 'nowrite', 'moverlock')):
                print(
                    "only 'lock', 'moverlock', 'ignore', 'noread', 'nowrite' and 'pause' are valid for start_draining option")
                sys.exit(0)
            else:
                lock = intf.start_draining
        else:
            lock = 'unlocked'
