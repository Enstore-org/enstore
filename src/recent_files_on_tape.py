...
def make_recent_file(storage_group,
	...
	f = open(temp_file, 'w')
	head = "Recent (packed and package ) written to tape in the last %s hours for storage group %s"%(duration,storage_group)
	...
	#
	query = "SELECT f.archive_mod_time, v.storage_group, v.file_family, \
	...
	and v.storage_group='%s' and f.archive_status = 'ARCHIVED' \
	and f.archive_mod_time > CURRENT_TIMESTAMP - INTERVAL '%s hours'  \
