...
class VolumeClerkMethods(dispatching_worker.DispatchingWorker, generic_server.GenericServer):
        ...
        if use_exact_match:
            if not vol or len(vol) == 0:
                ...
                if mover_type == 'DiskMover':
                    ...
                    Trace.alarm(e_errors.ERROR, 'NO VOLUME', msg)
                    self.reply_to_caller(ticket)
                return
    ...
    def set_system_noaccess(self, ticket):
        if ticket["external_label"] != None:  ## To avoid attempt to set None to NOACCESS. I do not know how this happens.
            Trace.alarm(e_errors.WARNING, "volume %s is set to %s"%
                        (ticket["external_label"], e_errors.NOACCESS),{"label":ticket["external_label"]})
