            ...
            if self.at_movers:
                for mover in self.at_movers.keys():
                    ...
                    if int(now) - int(self.at_movers[mover]['updated']) > 600:
                        Trace.alarm(e_errors.ALARM, "The mover %s has not updated its state for % minutes, will remove if from at_movers list"%(mover,now - self.at_movers[mover]['updated']))
