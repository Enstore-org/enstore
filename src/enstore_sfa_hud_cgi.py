...
def print_html(summary):
    ...
    if summary["stage_area"]["files"] > 0:
        print("<a href='enstore_sfa_show_cached_files_cgi.py'>",
              str(summary["stage_area"]["files"]), "</a>")
    else:
...
def main():
    ...
    if len(diff) != 0:
        ...
        for i in diff:
            for key, value in areas.iteritems():
                if value == i:
                    print(
                        "<li> %s area is not mounted to %s</li>" %
                        (key, value))
        print("</ul>")
        print("</div>")
        print_footer()
    ...
    for key, value in volume_map.iteritems():
        for k, v in areas.iteritems():
            if v == key:
                ...
                avail = long(stats[statvfs.F_BAVAIL]) * \
                    stats[statvfs.F_BSIZE] / KB
                total = long(stats[statvfs.F_BLOCKS]) * \
                    stats[statvfs.F_BSIZE] / KB
    ...
    res = select(
        dbinfo,
        "select coalesce(sum(f.size)/1024.,0) as total,count(*) from file f, files_in_transition fit where f.bfid=fit.bfid and f.cache_status='CACHED'")
    ...
    res = select(
        dbinfo,
        "select coalesce(sum(size)/1024.,0) as total,count(*) from file where cache_status='CACHED' and location_cookie!=cache_location")
