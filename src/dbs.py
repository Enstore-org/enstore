...
def restore(csc, intf, dbs, dbHome, jouHome):
    ...
    compression = os.popen("enrsh -n " + bckHost +
                           " ls " + bckHome + "/dbase.tar*").readline()[-2:-1]
    ...
    cmd = "enrsh -n " + bckHost + " dd if=" + \
        bckHome + "/dbase.tar bs=20b | tar xvBfb - 20"
    print(cmd)
    os.system(cmd)
...
def do_work(intf):
    ...
    if intf.restore_all:
        ...
        if dbs:
            ...
            Trace.log(
                e_errors.INFO,
                "Stopping db_checkpoint and db_deadlock to do db restore")
