...
class EnFile:
    ...
    def open(self, mode='w'):
        try:
            ...
            self.opened = 1
            enstore_functions.inqTrace(
                enstore_constants.INQFILEDBG, "%s open " %
                (self.file_name,))
        except IOError:
            self.openfile = 0
            self.opened = 0
    ...
    def close(self):
        enstore_functions.inqTrace(
            enstore_constants.INQFILEDBG, "enfile close %s" %
            (self.file_name,))
        if self.openfile:
            self.openfile.close()
            self.openfile = 0
    ...
    def write(self, doc):
        if doc.extra_queue_pages:
            for extra_page_key in doc.extra_queue_pages.keys():
                filename = "%s/%s" % (self.html_dir,
                                      doc.extra_queue_pages[extra_page_key][1],)
                ...
                try:
                    extra_file.write(
                        str(doc.extra_queue_pages[extra_page_key][0]))
                except IOError as detail:
                    msg = "Error writing %s (%s)" % (filename, detail)
                    Trace.log(e_errors.ERROR, msg, e_errors.IOERROR)
                extra_file.close()
...
class HTMLStatusFile(EnStatusFile, HTMLExtraPages, enstore_status.EnStatus):
    ...
    def get_file_list(self, lm, name):
        ...
        if enstore_constants.LMSTATE in lm and \
            ...
            if pend_q:
                ...
                if pwrite_q:
                    for pwrite_elem in pwrite_q:
                        ...
                        ff = pwrite_elem.get(
                            enstore_constants.FILE_FAMILY, None)
    ...
    def write(self):
        if self.openfile:
            ...
            doc = enstore_html.EnMigratorStatusPage(
                self.refresh, self.system_tag)
            doc.body(self.text)
...
class EnDataFile(EnFile):
    ...
    def timed_read(self, start_time, stop_time,
                   prefix=enstore_constants.LOG_PREFIX):
        do_all = FALSE
...
class EnMountDataFile(EnDataFile):
    ...
    def is_null_mover(self, mover_log_name):
        for server in self.servers:
            if 'logname' in self.config[server] and \
    ...
    def parse_line(self, line):
        try:
            ...
            if l == 9:
                # old format
                [etime, enode, epid, euser, estatus,
                    mover, type, request, volume] = field_l
                drive_id = ""
            elif l == 10:
                [etime, enode, epid, euser, estatus, mover,
                    type, request, volume, drive_id] = field_l
            elif l == 11:
            ...
            Trace.trace(
                enstore_constants.INQPLOTDBG,
                "Plot line has evil syntax (%s)" %
                (line,
                 ))
            return []
        ...
        if not self.is_null_mover(mover):
            ...
            return [etime, enode, euser, estatus, mover,
                    epid, volume, start, drive_id, msg_dicts]
        else:
            return []
    ...
    def parse_data(self, mcs, prefix):
        for line in self.lines:
            ...
            if minfo and (not mcs or enstore_status.mc_in_list(
                    minfo[MDICTS], mcs)):
                self.data.append([minfo[MDEV], minfo[MPID], minfo[MVOLUME],
                                  string.replace(
                    minfo[0], prefix, ""), minfo[MSTART],
                    minfo[MDRIVEID]])
