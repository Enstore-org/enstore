...
def do_work(intf):
    ...
    if 'status' in other_saag_links:
        del other_saag_links['status']
    dlinks = sorted(other_saag_links.keys())
    for dlink in dlinks:
        if dlink in override_d_keys:
            other_saag_links[dlink].append(
                enstore_functions2.override_to_status(
                    override_d[dlink]))
        else:
    ...
    log_file_dir = enstore_functions.get_from_config_file(
        "log_server", "log_file_path", ".")
    alarm_file = "%s/%s" % (log_file_dir, alarm_server.DEFAULT_FILE_NAME)
    try:
        if os.stat(alarm_file)[stat.ST_SIZE] == 0:
    ...
    # of this second dictionary will be a list of servers on the node, which
    # is the key.
    for server in enstat.keys():
        ...
        if server != enstore_constants.CONFIG_SERVER:
            if server in config.configdict:
