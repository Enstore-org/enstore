...
def print_volume_summary(ticket):
    ...
    print(
        "        Inhibits:",
        ticket['system_inhibit'][0],
        "+",
        ticket['system_inhibit'][1])
    print('</b><hr></pre>')
    print("</font>")
    print("<pre>")
...
def print_volume_content(ticket, list):
    format = "%%-%ds <a href=/cgi-bin/show_file_cgi.py?bfid=%%s>%%-19s</a> %%10s %%-22s %%-7s <a href=/cgi-bin/show_file_cgi.py?bfid=%%s>%%-19s</a> %%-20s %%-20s %%s" % (
        len(list))
