...
class BasicHistogram(Attribute):
    ...
    def plot(self, directory="./"):
        ...
        self.gnuplot_command = self.gnuplot_command.format(ps_file_name,
                                                           self.title,
                                                           time.strftime(
                                                               "%Y-%b-%d %H:%M:%S", time.localtime(time.time())),
                                                           self.ylabel,
                                                           self.xlabel,
                                                           string.join(
                                                               self.commands, "\n"),
    ...
    def plot(self, directory="./"):
        ...
        if (self.histogram_list[0].get_time_axis()):
            long_string = long_string + "set xlabel 'Date (year-month-day)'\n" + \
                ...
                "set timefmt \"%Y-%m-%d %H:%M:%S\"\n" + \
                "set format x \"" + \
                self.histogram_list[0].time_axis_format + "\"\n"
...
class Histogram1D(BasicHistogram):
    ...
    def __add__(self, other):
        if (self == other):
            ...
            if (hist.entries > 0):
                hist.mean = (float(self.entries) * self.mean +
                             float(other.entries) * other.mean) / float(hist.entries)
                hist.rms2 = (self.rms2 * float(self.entries) +
                             other.rms2 * float(other.entries)) / float(hist.entries)
                hist.variance = hist.rms2 - hist.mean * hist.mean
                hist.variance = math.sqrt(hist.variance)
                hist.mean_error = hist.variance / \
                    math.sqrt(float(hist.entries))
                hist.variance_error = hist.variance / \
                    math.sqrt(2. * float(hist.entries))
            else:
                hist.mean = 0.
                hist.rms2 = 0.
                hist.mean_error = 0.
    ...
    def integral(self, name="", title="", only_positive=True):
        ...
        for i in range(self.nbins):
            if (self.get_time_axis()):
                ...
                if (next >= self.nbins):
                    next = i
                if (0.5 * (self.get_bin_center(next) +
                           self.get_bin_center(i)) > time.time()):
                    stop = 1
    ...
    def fill(self, x, w=1.):
        ...
        if bin is not None:
            ...
            self.reduced_mean = (self.reduced_mean *
                                 float(self.entries) + x) / (self.entries + 1.)
            self.reduced_rms2 = (
                self.reduced_rms2 * float(self.entries) + x * x) / (self.entries + 1.)
            ...
            self.variance_error = self.variance / \
                math.sqrt(2. * float(self.entries))
    ...
    def get_bin_center(self, bin):
        if (bin >= self.nbins or bin < 0):
            return None
        return self.low + (bin + 0.5) * (self.high -
                                         self.low) / float(self.nbins)
    ...
    def get_bin_low_edge(self, bin):
        if (bin >= self.nbins or bin < 0):
            return None
        return self.low + float(bin) * (self.high -
                                        self.low) / float(self.nbins)

    def get_bin_high_edge(self, bin):
        if (bin >= self.nbins or bin < 0):
            return None
        return self.low + (bin + 1.) * (self.high -
                                        self.low) / float(self.nbins)
    ...
    def plot2(self, h, reflect=False, directory="./"):
        ...
        self.gnuplot_command += " t \'" + self.get_marker_text() + "' with " + self.get_marker_type() +\
            " lw " + str(self.get_line_width()) + " lt " + \
            str(self.get_line_color()) + ", '" + pts_file_name1 + "' "
...
class Histogram2D(Histogram1D):
    ...
    def __init__(self, name, title, nbinsX, xlow, xhigh, nbinsY, ylow, yhigh):
        Histogram1D.__init__(
            self, name, title, int(
                nbinsX * nbinsY), xlow, xhigh)
        self.ylow = ylow
        self.yhigh = yhigh
        self.nbins_x = nbinsX
        self.nbins_y = nbinsY
    ...
    def find_bin_x(self, x):
        if (float(x) < self.low):
            ...
            self.overflow = self.overflow + 1
            return None
        bin = int(float(self.nbins_x) * (float(x) -
    ...
    def find_bin_y(self, y):
        if (float(y) < self.ylow):
            ...
            self.overflow = self.overflow + 1
            return None
        bin = int(float(self.nbins_y) * (float(y) -
                                         self.ylow) / (self.yhigh - self.ylow))
        if (bin == self.nbins_y):
            bin = bin - 1
        return bin
    ...
    def get_bin_center(self, bin):
        ...
        nx = bin % self.nbins_x
        return self.low + (nx + 0.5) * (self.high - self.low) / float(
            self.nbins_x), self.ylow + (ny + 0.5) * (self.yhigh - self.ylow) / float(self.nbins_y),
    ...
    def plot_ascii(self, directory="./"):
        ...
        for bin in range(self.nbins):
            ...
            if count > 0:
                if self.time_axis:
                    self.add_text(
                        "set label \"%5d\" at \"%s\",%f center font \"Helvetica,10\"\n" %
                        (count, time.strftime(
                            "%Y-%m-%d %H:%M:%S", time.localtime(x)), y,))
                else:
                    self.add_text(
                        "set label \"%5d\" at %f,%f center font \"Helvetica,10\"\n" %
                        (count, x, y,))
