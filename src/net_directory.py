...
class NetDirectory:
	...
	def _build_ip_map(self):
		...
		for i in self._dir['ip']:
			...
			if n > 0:
				if not imap.has_key('*'):
					if p[0] != '*':
						if not imap.has_key(p[0]):
							imap[p[0]] = {}
					else:
						imap = {'*':{}}
						continue
			if n > 1:
				if not imap[p[0]].has_key('*'):
					if p[1] != '*':
						if not imap[p[0]].has_key(p[1]):
							imap[p[0]][p[1]] = {}
					else:
						imap[p[0]] = {'*':{}}
						continue
			if n > 2:
				if not imap[p[0]][p[1]].has_key('*'):
					if p[2] != '*':
						if not imap[p[0]][p[1]].has_key(p[2]):
							imap[p[0]][p[1]][p[2]] = {}
					else:
						imap[p[0]][p[1]] = {'*':{}}
						continue
			if n > 3:
				if not imap[p[0]][p[1]][p[2]].has_key('*'):
					if p[3] != '*':
						if not imap[p[0]][p[1]][p[2]].has_key(p[3]):
	...
	def _check_ip(self, node):
		...
		if len(ip) != 4:
			return False
		if self.ip_map.has_key('*'):
			return True
		if not self.ip_map.has_key(ip[0]):
			return False
		if self.ip_map[ip[0]].has_key('*'):
			return True
		if not self.ip_map[ip[0]].has_key(ip[1]):
			return False
		if self.ip_map[ip[0]][ip[1]].has_key('*'):
			return True
		if not self.ip_map[ip[0]][ip[1]].has_key(ip[2]):
			return False
		if self.ip_map[ip[0]][ip[1]][ip[2]].has_key('*'):
			return True
		if not self.ip_map[ip[0]][ip[1]][ip[2]].has_key(ip[3]):
			return False
		return True
