...
class BfidDb:
    ...
    def parse_summary(self, summary, vol):
        ...
        if len(w) != 4:
            raise_(
                BfidDbError, "invalid summary %s for vol %s" %
                (summary, vol))
        try:
    ...
    def get_all_bfids(self, vol):
        ...
        if n_entries != len(r):
            raise_(BfidDbError, "vol %s has %s entries, should have %s" % (
                vol, len(r), n_entries))
        if file_checksum != csum:
            raise_(BfidDbError, "vol %s: checksum error: has %s, should have %s" % (
    ...
    def add_bfid(self, vol, bfid):
        ...
        if pos <= 0:  # how can this be?
            raise_(BfidDbError, "vol %s: database file truncated" % (vol,))
