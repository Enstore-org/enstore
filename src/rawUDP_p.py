...
class RawUDP:
    ...
    def __init__(self, receive_timeout=60.):
        ...
        self.queue_size_p = self.manager.Value("l", 0)
        self.queue_size = 0
        self.print_queue = False
        self.enable_reinsert = True
        self.f = None
        self.max_queue_size = MAX_QUEUE_SIZE
        # use queue directly, not as intermediate place for incoming messages
        self.use_queue = False
        # intermediate place for incoming messages
    ...
    def get(self):
        ...
        if self.queue_size_p.value > 0:
            ...
            _print(
                self.d_o, "GET %s %s" %
                (self.queue_size_p.value, request_id,))
...
def put(RawUDP_obj, message):
    ...
    try:
        ...
        if RawUDP_obj.replace_keyword:
            keyword_to_replace = get_keyword(
                request, RawUDP_obj.replace_keyword)
            if keyword_to_replace and keyword_to_replace in RawUDP_obj.requests:
                try:
                    ...
                    index = RawUDP_obj.buffer.index(
                        RawUDP_obj.requests[keyword_to_replace])
                ...
                if request_id in RawUDP_obj.requests and RawUDP_obj.requests[request_id] != "":
                    ...
                    try:
                        ...
                        if RawUDP_obj.enable_reinsert and RawUDP_obj.queue_size_p.value > SHUFFLE_THRESHOLD:
                            ...
                            new_index = index
                        _print(
                            RawUDP_obj.f, "FOUND at %s reinserting at %s queue size %s" %
                            (index, new_index, RawUDP_obj.queue_size_p.value))
                        # if enable_reinsert and queue_size.value > SHUFFLE_THRESHOLD:
                        # buffer.remove(requests[request_id])
                        del RawUDP_obj.buffer[index]
                        RawUDP_obj.buffer.insert(
                            new_index, (request, client_addr))
                        do_put = False  # duplicate request, do not put into the queue
                    except ValueError as detail:
                        _print(
                            RawUDP_obj.f, "put: Exception:ValueError %s removing %s" %
                            (detail, request_id))
...
def _receiver(RawUDP_obj):
    ...
    print(
        "RECEIVER %s STARTS on %s" %
        (os.getpid(), RawUDP_obj.server_socket.getsockname(),))
    rcv_timeout = RawUDP_obj.rcv_timeout
    while True:
        r, w, x = select.select(
            [RawUDP_obj.server_socket], [], [], rcv_timeout)
            ...
            if message[0]:  # request is not empty
                ...
                try:
                    ...
                    if hasattr(RawUDP_obj, 'caller_name') and \
                        ...
                        if not " ENCP " in m:
                            # Send to stdout as enstore log service may not be
                            # available.
                            msg = " ".join(
