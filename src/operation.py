...
def send_mail(subject, message):
    ...
    msg = [	"From: %s" % (from_addr),
            ...
            "Subject: %s" % (subject),
            ""] + message
    return smtplib.SMTP('localhost').sendmail(
        from_addr, [to_addr], '\n'.join(msg))
...
def get_db():
    return pg.DB(dbname=DATABASENAME, port=DATABASEPORT,
                 host=DATABASEHOST, user=DATABASEUSER)


def get_edb(enstoredb):
    return pg.DB(host=enstoredb['db_host'], port=enstoredb['db_port'],
                 dbname=enstoredb['dbname'], user=enstoredb['dbuser'])
...
def get_unfinished_job(cluster=None):
    if cluster:
        q = "select name from job where name ilike '%s%%' and finish is null;" % (
            cluster)
    else:
...
def is_done(job):
    ...
    if t == 'E':  # write enable
        ...
        if debug:
            print("unknown job", job, c, t, repr(r))
        return 0
    t0 = 0

    for i in r:
        pp = os.path.join(p, repr(i))
...
def create_job(name, type, args, comment=''):
    ...
    for i in args:
        ...
        if i[-1] == ':':
            ...
            p = i.split(':')
            if len(p) > 1:  # yes
                q = "insert into object (job,object,association) values (%d, '%s', '%s');" % (
                    id, p[1], p[0])
            else:		# nope
                if association:
                    q = "insert into object (job, object, association) values (%d, '%s', '%s');" % (
                    ...
                    q = "insert into object (job, object) values (%d, '%s');" % (
                        id, i)
        if debug:
            print(q)
        db.query(q)
...
def retrieve_job(job):
    ...
    q = "select * from object where job = %d order by association, object;" % (
        job['id'])
    object = {}
    if debug:
        print(q)
...
def finish_job_task(job_name, task_id, comment=None,
...
def show_current_task(j):
    ...
    if ct['finish'] is None:
        ct['finish'] = ""
    if ct['action'] is None:
        ct['action'] = 'default'
...
def help(topic=None):
    if not topic:
        print(
            "operation.py create write_protect_on|write_protect_off <job> [[<association>:] [<associate>:]<object>]+")
        print("    -- creating a job")
        print(
            "operation.py list [all|open|finished|closed|completed|<job>+|has <object>]|recent <n>")
        print("    -- list job(s)")
        ...
        print(
            "operation.py recommend_write_protect_on [<library_list>] [limit <n>]")
        print("    -- recommend volumes for flipping write protect tab on")
        print(
            "operation.py recommend_write_protect_off [<library_list>] [limit <n>]")
        ...
        print(
            "operation.py auto_write_protect_off [<library_list>] [no_limit]")
        ...
        print(
            "<association> is a way to group objects. By default, there is no association")
        ...
        print(
            "<association>:<object> temporarily override default association for <object>")
        print()
        print("EXAMPLE:")
        ...
        print()
        print(
            "operation.py list [all|open|finished|closed|completed|<job>+|has <object>]")
        print()
...
def dump():
    for i in __builtins__.globals().keys():
        if i[:2] == '__':  # internal
            continue
        if isinstance(__builtins__.globals()[i], type(1)) or \
                isinstance(__builtins__.globals()[i], type(1.0)) or \
                isinstance(__builtins__.globals()[i], type("")) or \
                isinstance(__builtins__.globals()[i], type({})) or \
                isinstance(__builtins__.globals()[i], type([])):
            print(i, '=', end=' ')
...
def make_help_desk_ticket(n, cluster, script_host, job, library_type='9310'):
    ...
    short_message = "write %s %d tapes (flip tabs) in %s %s tape library" % (
        job, n, cluster.lower() + 'en', library_type.upper())
    long_message = 'Please run "flip_tab %s" on %s to write %s %d tapes (%d caps) in %s enstore %s tape library.' % (
        action, script_host, job, n, int((n - 1) / VOLUMES_PER_CAP) + 1, cluster, library_type.upper())
    ...
    if cmd == "dump":  # dump all global variables
        ...
        if res:
            ...
            if len(args) > 1:
                ...
                lt = library_type(
                    cluster, get_default_library(cluster).split(',')[0])
            qf = get_qualifier(lt)

            job_name = cluster + qf + 'WP' + \
                repr(min(res.keys())) + '-' + repr(max(res.keys()))
            create_write_protect_on_job(
                job_name, make_cap_args(res), 'AUTO-GENERATED')
            ...
            print(cc)
            os.system(cc)
            ticket = make_help_desk_ticket(
                total, cluster, script_host, 'protect', lt)
            print("ticket =", ticket)
        ...
        if res:
            ...
            job_name = cluster + qf + 'WE' + \
                repr(min(res.keys())) + '-' + repr(max(res.keys()))
            create_write_protect_off_job(
                job_name, make_cap_args(res), 'AUTO-GENERATED')
            # clean up temp directory
            clean_up_temp_dir()
            total = 0
            ...
            os.system(cc)
            ticket = make_help_desk_ticket(
                total, cluster, script_host, 'permit', lt)
            print("ticket =", ticket)
            res2 = start_next_task(job_name, ticket)
            res2.append(show_current_task(job_name))
            res2.append("ticket = " + ticket)
        ...
        if len(args) > 3:
            if args[2] == 'limit':
                res = recommend_write_protect_job(
                    library=args[1], limit=int(args[3]))
            else:
