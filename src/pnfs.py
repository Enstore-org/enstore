...
def is_nameof_name(filepath):
    # Determine if it is an ".(access)()" name.
    nameof_match = re.compile(r"\.\(nameof\)\([0-9A-Fa-f]+\)")
...
def is_parent_name(filepath):
    ...
    nameof_match = re.compile(r"\.\(parent\)\([0-9A-Fa-f]+\)")
    if re.search(nameof_match, os.path.basename(filepath)):
        return True
...
def is_pnfsid(pnfsid):
    ...
    if isinstance(pnfsid, bytes) and len(pnfsid) == 24:
...
def get_stat(pathname, unstable_filesystem=False):
    if isinstance(pathname, bytes) and is_access_name(pathname):
        ...
        stat_results = file_utils.get_stat(
            pathname, unstable_filesystem=unstable_filesystem)
...
def __is_showid(showid_data, match_string):
    if isinstance(showid_data, list) and \
            len(showid_data) >= 9 and \
            isinstance(showid_data[8], bytes):
...
def __get_special_path(filepath, replacement_path):
    ...
    # Make sure this is a string.
    if not isinstance(replacement_path, bytes):
    ...
    def touch(self, filename=None):
        ...
        try:
            ...
            if msg.errno == errno.ENOENT:
                f = file_utils.open(
                    use_filename, 'w', unstable_filesystem=True)
                f.close()
            else:
...
#            pnfs_id = string.replace(pnfs_id[0], '\n', '')
    ...
    def get_path(self, id=None, directory="", shortcut=None):
        ...
        for i in range(len(search_paths)):
            if isinstance(targets[i], list) and \
               len(targets[i]) >= 9 and \
               isinstance(targets[i][8], bytes) and \
               (targets[i][8].find("Tag ( Inode )") != -1
                ...
                old_base_id = base_id
            elif isinstance(targets[i], tuple) and \
                    len(targets[i]) == 3:
                #
                # This is a TAG or LAYER pnfsid.  Handle it.
                #
    ...
    def __get_path(self, pnfs_id, search_path, target, shortcut):
        ...
        if filepath:
            ...
            for i, piece in enumerate(pieces):
                if not file_utils.e_access(os.path.join(
                        found_search_path, piece), os.F_OK):
                    continue
                else:
                    ip = i
                    break
            filepath = os.path.join(
                found_search_path, string.join(pieces[ip:], "/"))
        else:
...
class Tag:
    ...
    def ptagchown(self, intf):
        ...
        # Determine if the target directory is in pnfs namespace
        if fname[:6] != "/pnfs/":
            msg_str = os.strerror(errno.EINVAL) + \
        ...
        try:
            if intf.file_family_wrapper == 1:
                ...
                if charset.is_in_charset(intf.file_family_wrapper):
                    ...
                    msg_str = self.INVALID_CHARACTERS % (
                        "file_family_wrapper",)
                    sys.stderr.write("%s\n" % (msg_str,))
                    return 1
            return 0
        ...
        try:
            ...
            if isinstance(intf.file_family_width, (int, long)):
                if intf.file_family_width > 0:
                    ...
                    msg_str = "Pnfs tag, {}, has to be positive integer greater than 0.".format(
                        "file_family_width")
                    sys.stderr.write("%s\n" % (msg_str,))
                    return 1
            else:
    ...
    def ppnfs_state(self):
        ...
        if os.access(fname, os.F_OK | os.R_OK):
            f = file_utils.open(fname, 'r', unstable_filesystem=True)
            try:
                self.pnfs_state = file_utils.readlines(
                    f, unstable_filesystem=True)
            finally:
                f.close()
    ...
    for i in mtab.keys():
        if string.find(p, i) == 0 and \
           string.split(os.uname()[1], '.')[0] == mtab[i][1]:
            p1 = os.path.join(LOCAL_PNFS_PREFIX,
                              string.replace(p, i, mtab[i][0][1:]))
...
class File:
    ...
    def __init__(self, file):
        if isinstance(file, dict):  # a dictionary
