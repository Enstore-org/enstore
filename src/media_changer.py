...
# to indicate, that ther slot is under some activity, like dismout into
# ths slot.
BUSY = 'busy'
...
class MediaLoaderMethods(dispatching_worker.DispatchingWorker,
    ...
    def check_mount_ticket(self, ticket):
        if ticket['function'] in ("mount", "dismount"):
            ...
            if "drive_id" not in ticket:
                ...
                Trace.log(e_errors.ERROR, "%s" % (ticket['status'],))
                return ticket['status']
            if "external_label" not in ticket['vol_ticket']:
    ...
    def getVolState(self, ticket):
        __pychecker__ = "no-argsused"
        # return 'O' - occupied aka unmounted
        return (e_errors.OK, 0, None, "O")
        ...
        while count > 0 and sts[0] != e_errors.OK:
            try:
                ...
                if sts[1] != 0:
                    if self.logdetail:
                        Trace.log(
                            e_errors.ERROR, 'retry_function: function %s %s error %s' %
                            (repr(function), args, sts[2]))
                if sts[1] == 1 and rpcErrors < 2:  # RPC failure
                    time.sleep(10)
                    rpcErrors = rpcErrors + 1
                elif (sts[1] == 5 or         # requested drive in use
                      # DAS was unable to communicate with AMU
                      sts[1] == 8 or
    ...
    def cleanCycle(self, inTicket):
        ...
        try:
            if self.driveCleanTime:
                ...
                cleanTime = self.driveCleanTime[driveType][0]
                # number of cleaning cycles
                driveCleanCycles = self.driveCleanTime[driveType][1]
            else:
                cleanTime = 60
                driveCleanCycles = 1
        except KeyError:
        ...
        if isinstance(libraryManagers, bytes):
            lm = libraryManagers
            library = string.split(libraryManagers, ".")[0]
        elif isinstance(libraryManagers, list):
        ...
        while count > 0 and sts[0] != e_errors.OK:
            try:
                ...
                if sts[1] != 0:
                    if self.logdetail:
                        Trace.log(
                            e_errors.ERROR, 'retry_function: function %s  %s  sts[1] %s  sts[2] %s  count %s' %
                    ...
                    if function == self.mount:
                        ...
                        Trace.log(
                            e_errors.INFO, 'Tried %s %s  status=%s %s  Desperation dismount  status %s %s' %
    ...
    def timed_command(self, cmd, min_response_length=0, timeout=60):
        ...
        if pid == 0:
            ...
            try:
                ...
                Trace.log(
                    e_errors.ERROR, "timed_command execv failed:  %s %s %s" %
        ...
        try:
            while active < timeout:
                ...
                except (select.error, OSError, IOError) as msg:
...
#		   print "nread=",nread, "line=",nl, l
    ...
    def getDriveState(self, ticket):
        ...
        drive_info['status'] = rt[2][len(
            drive) + 16:len(drive) + 16 + 11].strip()
        drive_info['volume'] = rt[2][len(
            drive) + 16 + 11:len(drive) + 16 + 11 + 11].strip()
    ...
    def mount(self, volume, drive, media_type="", view_first=1, ticket={}):
        ...
        if view_first:
            ...
            if attrib != "O":  # look for tape in tower (occupied="O")
                ...
                msg = "MOUNT %i: Tape is not in home position. %s => %s,%s" % (
                    E, command, status, response)
                Trace.log(e_errors.ERROR, msg)
                return ("ERROR", E, response, "", msg)
        ...
        # check if any tape is mounted in this drive
            ...
            if volser != "":  # look for any tape mounted in this drive
                ...
                msg = "MOUNT %i: Drive %s is not empty =>. %s => %s,%s" % (
        ...
        if string.find(answer, answer_lookfor, 0) != 0:
            ...
            try:
                Trace.log(
                    e_errors.INFO, "Ckecking ASCLS message %s %s" %
                    (response, answer_lookfor))  # remove after debugging AM
                ...
                for l in response:
                    if answer_lookfor in l:
                        ...
                        ar = l.split()
                        Trace.log(
                            e_errors.INFO, "Requested Drive %s. Comparing to %s" %
                            (requeseted_drive, ar))  # remove after debugging AM
                        same_drive = 0
                        for i in range(len(requeseted_drive)):
                            if int(
                                    requeseted_drive[-(i + 1)]) != int(ar[-(i + 1)]):
                                break
                        else:
                            same_drive = 1
                        if same_drive:
        ...
        if view_first:
            ...
            if volser == "":  # look for any tape mounted in this drive
                if volume != "Unknown":
                    ...
                    # which tape it has.
                    E = 14
                    msg = "Dismount %i ignored: Drive %s is empty. Thought %s was there =>. %s => %s,%s" % (
...
class MTX_MediaLoader(MediaLoaderMethods):
    ...
    def load(self, ticket):
        ...
        media_type = ticket['vol_ticket']['media_type']
        Trace.log(
            e_errors.INFO, 'MTX_MediaLoader: request to load %s of type %s into drive %s' %
    ...
    def unload(self, ticket):
        ...
        Trace.log(
            e_errors.INFO,
            'MTX_MediaLoader: request to unload %s of type %s from drive %s' %
            (external_label,
             media_type,
             drive))
    ...
    def robot_status(self):
        ...
        result = enstore_functions2.shell_command(
            "%s mtx -f %s status | grep 'Data Transfer Element'" %
            (self.sudo_cmd, self.device_name))
...
class MTXN_MediaLoader(MediaLoaderMethods):
    ...
    def __init__(self, medch, max_work=1, csc=None):
        ...
        if not e_errors.is_ok(rc[0]):
            Trace.alarm(
                e_errors.ERROR, 'can not get intial status, exiting with %s' %
                (rc,))
    ...
    def _mtx_server(self, read_pipe, write_pipe, err_pipe):
        ...
        while True:
            try:
                ...
                exc, msg, tb = sys.exc_info()
                Trace.log(
                    e_errors.ERROR, "MTX server failed:  %s %s %s" %
                    (exc, msg, traceback.format_tb(tb)))
                return
            ...
            Trace.trace(
                ACTION_LOG_LEVEL, "MTX server: received: %s" %
                (raw_msg,))
            ...
            Trace.log(
                ACTION_LOG_LEVEL, "MTX server: cmd: %s args %s" %
                (cmd, args))
            ...
            if cmd in ["Load", "Unload"]:
                ...
                while retry_cnt:
                    try:
                        ...
                        a, b = return_by(
                            self.load_unload_local, (int(
                                args[0]), int(
                                args[1]), cmd), self.mount_timeout)
                        Trace.log(
                            ACTION_LOG_LEVEL, "MTX server: load_unload_local returned %s %s" %
                            (a, b))
                        if -1 == a:
                            Trace.log(
                                ACTION_LOG_LEVEL, ' mtx load / unload timeout')
                            retry_cnt -= 1
                            if retry_cnt == 0:
                                Trace.log(
                                    ACTION_LOG_LEVEL,
                                ...
                                response = '%s pid_%s' % (
                                    e_errors.TIMEDOUT, pid_to_send_back,)
                        else:
                            break
                    except BaseException:
    ...
    def start_mtx_server(self):
        ...
        self.server = multiprocessing.Process(
            target=self._mtx_server, args=(
                self.p2cread, self.c2pwrite, self.c2pwrite))
        self.server.start()
        t = 0
        ...
        if t >= self.status_timeout:
            msg = 'mtx server not started %s s. Exiting' % (
                self.status_timeout,)
            ...
            while active < timeout:
                ...
                if raw_msg:
                    ...
                    if end_of_response in raw_msg:
                        # in the last line leave all, but end_of_response
                        message = message + \
                            raw_msg[:raw_msg.find(end_of_response)]
                        break
                    message = message + raw_msg
                    # Need to reset the timeout period.
                    start = time.time()
    ...
    def send_command(self, command_string, timeout):
        Trace.log(
            ACTION_LOG_LEVEL, 'send_command: command %s timeout %s' %
            (command_string, timeout))
        #  command_string must be: 'cmd,arg1,arg2,arg3'
        message = ""
        end_of_response = 'pid_%s' % command_string.split(',')[-1]
        Trace.log(
            ACTION_LOG_LEVEL, 'send_command: expected end of reply %s' %
            (end_of_response,))
        ...
        try:
            ...
            return (e_errors.ERROR, e_errors.ERROR,
                    [], "", 'receive_reply error')
        if len(
                message) < 100:  # status command returns a large array, we do not want to log it
            Trace.log(
                ACTION_LOG_LEVEL, "send_command: message %s" %
                (message,))
        response = message.split('\012')
        #response = message
        if 'status' in command_string:
            if self.debug:
                ofn = '/var/log/enstore/tmp/enstore/%s_%s.mtx_status.out' % (
                    self.name, end_of_response,)
                Trace.log(
                    e_errors.INFO, 'the output of the status command is in %s' %
                    (ofn,))
                of = open(ofn, 'w')
                for l in response:
                    of.write('%s\n' % (l,))
            ...
            if len(response) < 100:
                Trace.log(
                    ACTION_LOG_LEVEL, "send_command: response %s" %
                    (response,))
        if 'Load' in command_string or 'Unload' in command_string:
            ...
            # Storage Changer /dev/changer:56 Drives, 10295 Slots ( 255
            # Import/Export )
            if all(s in response[0]
                   for s in ['Storage Changer', 'Drives', 'Slots']):
   ...
   # query robot
            ...
            if ((isinstance(ret_val[3], list) and
                ...
                if 'mtx: Request Sense: Sense Key=Illegal Request' in ret_val[3]:
                        ...
                        if 'mtx: Request Sense: Additional Sense Code = 3B' in ret_val[3]:
                            ...
                            if 'mtx: Request Sense: Additional Sense Qualifier = 0D' in ret_val[3]:
                                if self.retry_count == 0:  # retry one more time
                                    self.retry_count = 1
                                update_db = False
                            # Medium Source Element Empty (Looks as the
                            # dismount command was successful)
                    ...
                    if 'mtx: Request Sense: Additional Sense Code = 28' in ret_val[3]:
                        # According to IBM recommendations,  wait and retry
                        # more times
    ...
    def insert(self, ticket):
        ...
        if ticket.get('external_label'):
            ...
            if s < 0:
                return (e_errors.ERROR, e_errors.MC_VOLNOTFOUND,
                        ticket['external_label'], 'Volume not found')
            if not 'IMPORT/EXPORT' in self.slots[s]['location']:
                return (e_errors.ERROR, '%s is not in  IMPORT/EXPORT slot (%s)' %
                        (ticket['external_label'], self.slots[s]['location']), None, None)
        else:
        ...
        while retry:
            ...
            if s_slot < 0:
                return (e_errors.ERROR, '%s can not be inserted, no free slots' % (
                    ticket['external_label'],), None, None)
            ...
            if rc[1] == e_errors.OK:
                ...
                self.slots[s_slot] = stor_el
                rc = list(rc)
                rc[3] = "Imported %s from %s to %s" % (
                    stor_el['volume'], imp_el['address'], stor_el['address'])
                rc = tuple(rc)
                break
            else:
                Trace.log(
                    e_errors.INFO, "Unload for insert command returned: %s" %
                    (rc,))
                ...
                if retry:
                    ...
                    # rc[0] == -1 - timeout
                    Trace.log(
                        e_errors.INFO, 'retrying insert %s %s' %
                        (imp_el, stor_el))
                    time.sleep(1)
                else:
                    break
    ...
    def eject(self, ticket):
        ...
        if d >= 0:
            return (e_errors.ERROR, '%s is in drive %s, can not be ejected' %
                    (external_label, self.drives[d]['address']), None, None)

        if s_slot < 0:
            return (e_errors.ERROR, e_errors.MC_VOLNOTFOUND,
                    external_label, 'Volume not found')
        if 'IMPORT/EXPORT' in self.slots[s_slot]['location']:
            return (e_errors.ERROR, '%s is in  IMPORT/EXPORT slot (%s)' %
                    (external_label, self.slots[s_slot]['location']), None, None)
        ...
        while retry_count > 0:
            ...
            if rc[1] == e_errors.OK:
                ...
                self.slots[s_slot] = stor_el
                rc = list(rc)
                rc[3] = "Ejected %s from %s to %s" % (
                    imp_el['volume'], stor_el['address'], imp_el['address'])
                rc = tuple(rc)
                break
            else:
                Trace.log(
                    e_errors.INFO, "Unload for eject command returned: %s" %
                    (rc,))
                if (rc[0] == -1) or ((isinstance(rc[3], list)
                                      and 'mtx: Request Sense: Sense Key=Unit Attention' in rc[3])):
                    # retry
                    # rc[0] == -1 - timeout
                    Trace.log(
                        e_errors.INFO, 'retrying eject %s %s' %
                        (imp_el, stor_el))
                    time.sleep(1)
                    retry_count -= 1
                else:
    ...
    def mtx_mount(self, volume, drive, media_type="", view_first=1):
        ...
        while retry:
            ...
            # indices in self.slots, and self.drives correspondingly
            s, d = self.locate_volume(volume)
            if s == -1:
                if d == -1:
                    Trace.log(e_errors.ERROR,
            ...
            if not self.is_empty(dr):
                Trace.log(
                    e_errors.ERROR, 'mtx cant mount tape. Drive %s is not empty: %s' %
                    (dr, self.drives[dr]))
                return(e_errors.ERROR, e_errors.MC_DRVNOTEMPTY, [], '',
                       'mtx cant mount tape. Drive %s is not empty: %s' % (drive, self.drives[dr]['volume']))
            Trace.log(
                e_errors.INFO, 'found %s in slot %s ...mounting' %
                (volume, self.slots[s]['address']))
            if not self.test_unit_ready():
                Trace.log(
                    e_errors.ERROR,
                    'mount: Unit is not ready. Will try anyway')
            rc = self.send_command(
                'Load,%s,%s,%s' %
                (self.slots[s]['address'],
                 self.drives[dr]['address'],
                 os.getpid()),
                self.mount_timeout *
                self.mount_retries +
                10)
            ...
            if rc[1] == e_errors.OK:
                Trace.trace(
                    ACTION_LOG_LEVEL, 'updating DB: slots[%s]=%s drives[%s]=%s' %
                    (s, self.slots[s], dr, self.drives[dr]))
                ...
                Trace.trace(
                    ACTION_LOG_LEVEL, 'updated DB: slots[%s]=%s drives[%s]=%s' %
                    (s, self.slots[s], dr, self.drives[dr]))
                break
            else:
                ...
                if rc[1] == e_errors.TIMEDOUT:
                    ...
                    if retry:
                        Trace.log(
                            e_errors.INFO, 'retrying mtx_mount %s %s' %
                            (volume, drive))
                        time.sleep(1)
                    else:
                        break
    ...
    def mtx_dismount(self, volume, drive, media_type="", view_first=1):
        ...
        while retry:
            ...
            if not self.test_unit_ready():
                Trace.log(
                    e_errors.ERROR,
                    'dismount: Unit is not ready. Will try anyway')
            rc = self.send_command(
                'Unload,%s,%s,%s' %
                (self.slots[s]['address'],
                 self.drives[dr]['address'],
            ...
            if rc[1] == e_errors.OK:
                ...
                if retry:
                    Trace.log(
                        e_errors.INFO, 'retrying mtx_dismount %s %s' %
                        (self.drives[dr], stor_el))
                    time.sleep(1)
                else:
                    break
    ...
    def locate_volume(self, vol):
        Trace.trace(
            ACTION_LOG_LEVEL, ' looking for volume %s status_valid %s' %
            (vol, self.status_valid))
        if self.status_valid == 0:
            ...
            if a == -1:
                ...
                Trace.log(
                    e_errors.ERROR,
                    'locate_volume: mtx status request timeout, retrying')
                a, b = return_by(self.status_local, (), self.status_timeout)
                if a == -1:
                    Trace.log(e_errors.ERROR,
                              'locate_volume: mtx status request timeout')
                    if self.cli_host:
                        # record what is seen using CLI"
                        rc = self.setup_cli_command()
                        if not e_errors.is_ok(rc[0]):
                            Trace.log(
                                e_errors.INFO, 'setup_cli_command returned %s' %
                                (rc,))
                            return -1, -1
                        ...
                        res = enstore_functions2.shell_command(
                            'enrsh -n %s %s ' % (self.cli_host, cmd,))
                        if not res:
                            Trace.log(
                                e_errors.ERROR, 'viewDataCartridges: IBM CLI returned %s' %
                                (res[1],))
                        else:
                            f = open(fn, 'w')
                            f.write(res[0])
                            f.close()
                            Trace.log(
                                e_errors.INFO, 'viewDataCartridges result is in %s' %
                                (fn,))
                    return -1, -1
            self.status_valid = 1
        found = False
        if vol == EMPTY:
            ...
            for d in self.drives:
                try:
                    ...
                    Trace.log(
                        e_errors.ERROR, 'exception in locate_volume %s %s' %
                        (e, d))
                    return -1, -1
            if found:
            ...
            if not found:
                ...
                Trace.trace(
                    ACTION_LOG_LEVEL, 'found in drive %s' %
                    (d['address'],))
    ...
    def locate_drive(self, drive_address):
        Trace.trace(
            ACTION_LOG_LEVEL, ' looking for drive %s , status_valid %s' %
            (drive_address, self.status_valid))
        if self.status_valid == 0:
            a, b = return_by(self.status_local, (), self.status_timeout)
            if a == -1:
                # retry
                Trace.log(
                    e_errors.ERROR,
                    'locate_drive: mtx status request timeout, retrying')
                a, b = return_by(self.status_local, (), self.status_timeout)
                if a == -1:
                    Trace.log(e_errors.ERROR,
                              'locate_drive:  mtx status request timeout')
                    if self.cli_host:
                        # record what is seen using CLI"
                        rc = self.setup_cli_command()
                        if not e_errors.is_ok(rc[0]):
                        ...
                        if not res:
                            Trace.log(
                                e_errors.ERROR, 'viewDriveSummary: IBM CLI returned %s' %
                                (res[1],))
                        else:
                            f = open(fn, 'w')
                            f.write(res[0])
                            f.close()
                            Trace.log(
                                e_errors.INFO, 'viewDriveSummary result is in %s' %
                                (fn,))
                    return -1
            self.status_valid = 1
        found = False
    ...
    def load_unload_local(self, slot, drive, load_command):
        Trace.log(
            e_errors.INFO, "MTX %s slot %s drive %s" %
            (load_command, slot, drive))
        if load_command not in ("Load", "Unload"):
            return ('ERROR', e_errors.ERROR, [], "%s" %
                    (load_command,), "Wrong command")
    ...
    def status_local(self):
        ...
        if hasattr(self, 'slots'):  # clear lists
            ...
            for i, slot_d in enumerate(self.slots):
                ...
                if vol and vol == BUSY:
                    Trace.log(
                        ACTION_LOG_LEVEL, 'busy slot: %s %s' %
                        (i, slot_d,))
                    busy_slots.append(i)
        ...
        if lines:
            ...
            while '' != line:
                try:
                    ...
                    if string.find(line, 'Data Transfer Element') != -1:
                        ...
                        # or :
                        # Data Transfer Element 270 Phys Loc F2,C4,R4,Z0 SN
                        # 00078D2B59 ID ULT3580-TD8 :Full (Storage Element 1038
                        # Loaded):VolumeTag = VQ0006L8
                        ...
                        # or:
                        # Storage Element 1037 Phys Loc EMPTY :Empty
                        # :VolumeTag=
                        ...
                        d['volume'] = EMPTY
                        if (len(
                                self.slots) - 1) in busy_slots:  # this slot was reserved, leave it BUSY
                            d['volume'] = BUSY
                            Trace.log(
                                ACTION_LOG_LEVEL, 'from status: %s' %
                                (line,))
                        if string.find(line, 'Empty') != -1:
                            pass  # just to leave status as empty
                        elif string.find(line, 'VolumeTag') != -1:
                        ...
                        Trace.log(
                            ACTION_LOG_LEVEL, 'status_local: can not interpret line: %s' %
                            (line,))
                    index = index + 1
                    line = lines[index]
                except BaseException:
            ...
            Trace.log(
                e_errors.ERROR, 'mtx status returned no result %s' %
                (result,))
        if errorString:
    ...
    def getVolState(self, ticket):
        ...
        while retry_count > 0:
            ...
            slot, drive = self.locate_volume(ticket['external_label'])
            Trace.log(
                ACTION_LOG_LEVEL, 'getVolState slot:%s, drive %s' %
                (slot, drive,))
            if (slot < 0 and drive < 0) or (slot > 0 and drive >= 0):
                ...
                ticket['state'] = 'M'
                ticket['phys_location'] = (
                    '%s,%s' %
                    (self.drives[drive]['location'],
                     self.drives[drive]['zone']))
        ...
        return (ticket.get('status'), ticket.get('location'), ticket.get(
            'media_type'), ticket.get('state'), ticket.get('external_label'))
    ...
    def listDrives(self, ticket):
        ...
        for d in self.drives:
            try:
                ...
                drive_info['name'] = ('%s(%s,%s)' %
                                      (d['address'], d['location'], d['zone']))
                drive_info['state'] = 'online'
                ...
                Trace.log(
                    e_errors.ERROR, 'exception in listDrives %s %s' %
                    (e, d))
    ...
    def updatedb(self, ticket):
        ...
        if found:
            ...
            return [e_errors.ERROR, 0, 'No slot with address %s' %
                    (ticket['volume']['address'],), 'No such slot']
        slot_info = self.slots[idx_slot]
        Trace.trace(
            ACTION_LOG_LEVEL, 'updatedb: slot[%s] %s' %
            (idx_slot, slot_info,))
    ...
    def setup_cli_command(self):
        ...
        if not self.cli_host:
            return (e_errors.ERROR, 0, None,
                    'No CLI host specified, check configuraion', '')
        ...
        if not hasattr(self, 'ibm_cli_host'):
            res = enstore_functions2.shell_command(
                'enrsh -n %s %s' %
                (self.cli_host, 'locate %s.IBMCLI' %
                 (self.name),))
            if res[0]:
                cli_file = res[0][:-1]
            else:
                return (e_errors.ERROR, 0, None, 'No %s.IBMCLI found' %
                        (self.name,), '')
            res = enstore_functions2.shell_command(
                'enrsh -n %s %s' %
                (self.cli_host, 'cat %s' %
            ...
            if res[0]:
                try:
                    ...
                    Trace.handle_error()
                    return (e_errors.ERROR, 0, None,
                            'Error executing %s.IBMCLI' % (self.name,), '')
            else:
                return (e_errors.ERROR, 0, None,
                        'Error reading %s.IBMCLI' % (self.name,), '')
    ...
    def listClean(self, ticket):
        ...
        if not res:
            ticket['status'] = (
                e_errors.ERROR, 0, None, 'IBM CLI returned %s' %
                (res[1],), '')
            Trace.log(
                e_errors.ERROR, 'QUERY_CLEAN: IBM CLI returned %s' %
                (res[1],))
            return (e_errors.ERROR, 0, None,
                    'IBM CLI returned %s' % (res[1],), '')
...
class MTXN_Local_MediaLoader(MTXN_MediaLoader):
    ...
    def __init__(self, argdict):
        ...
        self.sudo_cmd = ''
        Trace.log(
            e_errors.INFO, 'remote MC %s' %
            (argdict.get('remote_media_changer'),))
        self.mcc = media_changer_client.MediaChangerClient(
            csc, argdict.get('remote_media_changer'))
    ...
    def start_mtx_server(self):
        ...
        if not e_errors.is_ok(rc[0]):
            Trace.log(e_errors.ERROR, 'get_mtx_status returned: %s' % (rc[0],))
            msg = 'mtx server not responded in %s s. Exiting' % (
                self.status_timeout,)
            Trace.alarm(e_errors.ERROR, msg)
            self.quit()
            os._exit(1)
    ...
    def updatedb_on_retry(self):
        return self.update_db(None, None, None, None)

    #########################################################################
    # These functions are internal functions specific to MTX media changer.
    #########################################################################

    def update_db(self, volume_address, volume_name,
                  drive_address, volume_name_in_drive):
        Trace.trace(
            ACTION_LOG_LEVEL,
            ' update_db %s %s %s %s' %
            (volume_address,
             volume_name,
             drive_address,
             volume_name_in_drive))
        rc = self.mcc.update_db(
            volume_address,
            volume_name,
            drive_address,
            volume_name_in_drive)
        ...
        while retry:
            ...
            if drive_info['status'] == 'in use':
                if drive_info['volume'] == volume:
                    rc = (e_errors.OK, e_errors.OK, [], '',
                          'Already in drive %s' % (drive,))
                    break
            elif drive_info['status'] != 'available':
                Trace.log(
                    e_errors.ERROR, 'mtx cant mount tape. Drive %s is not empty: %s' %
                    (drive, drive_info['volume']))
            ...
            if not e_errors.is_ok(vt):
                Trace.log(
                    e_errors.ERROR, 'mtx_mount unrecognized volume: %s' %
                    (volume,))
                rc = (e_errors.ERROR, e_errors.ERROR, [], '',
            ...
            if vt['state'] == 'M':
                rc = (
                    ...
                    [],
                    '',
                    'Mounted in another device %s' %
                    (vt['location'],
                     ))
                break
            elif vt['state'] != 'O':
                rc = (
                    e_errors.ERROR, e_errors.ERROR, [], '', 'volume %s state%s' %
                    (volume, vt['state'],))
                break
            ...
            if rc[1] == e_errors.OK:
                ...
                if not e_errors.is_ok(rt):
                    rc = (e_errors.ERROR, e_errors.ERROR, [], '',
                          'update location failed with %s' % (rt,))
                    break
    ...
    def mtx_dismount(self, volume, drive, media_type="", view_first=1):
        ...
        while retry:
            ...
            if not e_errors.is_ok(dt):
                ...
                break
            Trace.trace(
                ACTION_LOG_LEVEL, "mtx_dismount: drive query %s" %
                (dt,))
            drive_info = dt['drive_info']
            if drive_info['status'] != 'in use':
            ...
            Trace.log(
                ...
                ('found ',
                 ...
                 drive,
                 '...dismounting'))
            # Set free slot busy to avoid conflicts with parallel mover
            # requests.
    ...
    def getVolState(self, ticket):
        ...
        if slot < 0 and drive < 0:
            ...
            ticket['state'] = 'O'
            ticket['location'] = slot
        return (ticket.get('status'), ticket.get('location'),
                ticket.get('media_type'), ticket.get('state'))
...
class IBM_3584_MediaLoader(MediaLoaderMethods):
    ...
    def timed_command(self, cmd, min_response_length=0, timeout=60):
        ...
        try:
            ...
            exc, msg, tb = sys.exc_info()
            Trace.log(
                e_errors.ERROR, "timed_command wait for child failed:  %s %s %s" %
                (exc, msg, traceback.format_tb(tb)))
            os.close(c2pread)
            return -1, [], self.delta_t(mark)[0]
...
#		   print "nread=",nread, "line=",nl, l
    ...
    def listDrives(self, ticket):
        # build the command, and what to look for in the response
        command = "/usr/bin/smc -h %s -l %s -q D" % (
            self.rmchost, self.device,)
        #answer_lookfor = ""
    ...
    def query(self, volume, media_type=""):
        ...
        #command = "query vol %s" % (volume,)
        #answer_lookfor = "%s " % (volume,)
        command = "/usr/bin/smc -h %s -l %s -q V -V '%s'" % (
            self.rmchost, self.device, volume,)
        answer_lookfor = "%s" % (volume,)
    ...
    def query_drive(self, drive):
        ...
        #command = "query drive %s" % (drive,)
        #answer_lookfor = "%s " % (drive,)
        command = "/usr/bin/smc -h %s -l %s -q D -D %s" % (
            self.rmchost, self.device, drive,)
        answer_lookfor = "%s" % (drive,)
...
#            return ("ERROR", E, answer, '', msg)
    ...
    def dismount(self, volume, drive, media_type="", view_first=1):
        ...
        # build the command, and what to look for in the response
        command = "/usr/bin/smc -h %s -l %s -d -D %s -V %s" % (
            self.rmchost, self.device, drive, volume)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            Trace.log(
                e_errors.INFO, "Media Changer %s (re) starting" %
                (intf.name,))
            mc.serve_forever()
