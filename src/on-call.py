...
def getNumber(developer):
    if developer not in dev_phones.phones:
        dev = string.lower(developer)
        if dev in dev_phones.alias:
            developer = dev
        else:
...
def getDeveloper(date):
    if date not in dev_shift.shifts:
...
if __name__ == "__main__":
    ...
    if choice == "schedule":
        ...
        command = "rm -f %s.ps; nice -n 19 remind -p2 %s | rem2ps -se 16 -e -c0 >%s.ps; nice -n 19 convert +append %s.ps %s.gif; cp %s.ps %s.gif %s" %\
                  (remlist, remlist, remlist, remlist,
                   remlist, remlist, remlist, webdir)
        ...
        command = '/usr/bin/Mail -s "%s Enstore Developer Weekly Schedule" %s < %s' % (
            today, users, remweek)
        os.popen(command, 'r').readlines()
        ...
        if choice == 'mail':
            ...
            command = '/usr/bin/Mail -s "%s You are on enstore developer shift" %s < %s' % (
                today, users, remmail)
            os.popen(command, 'r').readlines()
