...
class FTTDriver(generic_driver.Driver):
    ...
    def open(self, device=None, mode=None, retry_count=10):
        ...
        if mode not in (0, 1):
            Trace.log(
                e_errors.ERROR, "ftt_driver:read: illegal mode %s" %
                (mode,))
    ...
    def _open_dev(self, retry_count):
        ...
        for retry in xrange(retry_count):
            ...
            try:
                ...
                Trace.log(
                    e_errors.ERROR, "ftt open dev: %s %s" %
                    (detail, detail.value))
                if detail.errno == ftt.EBUSY:
                    ...
                    # ought to get readonly status of the tape from the volume
                    # database
                    Trace.log(
                        e_errors.INFO, "ftt open dev: %s %s: reopening read-only" %
                        (detail, detail.value))
                    #Trace.trace(42, "ftt.close()")
                    self.ftt.close()
                    #Trace.trace(42, "ftt.close() done")
                    ...
                    try:
                        ...
                        Trace.log(
                            e_errors.ERROR, "ftt close dev error: %s %s" %
                            (detail, detail.errno))
                    except BaseException:
                        pass
                    Trace.log(e_errors.ERROR, "raising %s" % (detail,))
                    raise_(ftt.FTTError, detail)
                    # time.sleep(5)
                else:
                    break
            except BaseException:
    ...
    def tell(self):
        ...
        try:
            ...
            Trace.log(
                e_errors.ERROR, "Unexpected exception in ftt.tell %s %s" %
                (exc, detail))
            try:
                ...
                exc, detail, tb = sys.exc_info()
                Trace.log(
                    e_errors.ERROR, "Exception while handling error %s %s" %
                    (exc, detail))
            raise_(ftt.FTTError, detail)
    ...
    def seek(self, target, eot_ok=0):  # XXX is eot_ok needed?
        ...
        try:
            ...
            if detail.errno == ftt.ELOST:
                ...
                Trace.log(
                    e_errors.ERROR, "ftt_driver:seek: ftt error %s %s" %
                    (detail, detail.value))
                raise_(ftt.FTTError, detail)  # some other FTT error

        #Trace.trace(42, "ftt.get_position()")
        fil, block = self.ftt.get_position()
        Trace.log(
            e_errors.INFO, "tape position: target %s current %s %s" %
            (target, fil, block,))
        #Trace.trace(42, "%s,%s=ftt.get_position() done" % (fil, block,))
        ...
        if target > current:
            try:
                ...
                if detail.errno == ftt.EBLANK and eot_ok:  # XXX is eot_ok needed?
                    # XXX Damn, this is unrecoverable (for AIT2, at least).
                    # What to do?
                    Trace.log(
                        e_errors.ERROR, "ftt errors positioning tape %s" %
                        (detail, ))
                    pass
                else:
                    Trace.log(
                        e_errors.ERROR, "ftt_driver:seek: %s %s" %
                        (detail, detail.value))
                    raise_(ftt.FTTError, detail)
        else:
            try:
                ...
                Trace.log(
                    e_errors.INFO, "skip_fm %s" %
                    (target - current - 1,))
                self.ftt.skip_fm(1)
                Trace.log(e_errors.INFO, "skip_fm %s" % (1,))
            except ftt.FTTError as detail:
                Trace.log(
                    e_errors.ERROR, "ftt_driver:skip_fm: %s %s" %
                    (detail, detail.value))
                raise_(ftt.FTTError, detail)
        ...
        if current != target:
            Trace.log(
                e_errors.ERROR, "ftt_driver:seek: Positioning error %s %s" %
                (current, target))
            #raise e_errors.POSIT_EXCEPTION
            raise ftt.FTTError(
                ("Lost position", ftt.ELOST, "%s %s" %
                 ((current, target))))
    ...
    def flush(self):
        ...
        if now > self._start_time and self._bytes_transferred:
            Trace.trace(25, "rate: %.3g MB/sec" %
                        (self._bytes_transferred / (now - self._start_time) / MB))
    ...
    def close(self):
        ...
        if self.ftt:
            try:
                ...
                Trace.log(
                    e_errors.ERROR, "ftt_driver:close: %s %s" %
                    (detail, detail.value))
                r = -1
        self.ftt = None
        self.fd = -1
    ...
    def read(self, buf, offset, nbytes):
        ...
        try:
            ...
            Trace.log(
                e_errors.ERROR, "ftt_driver:read: %s %s" %
                (detail, detail.value))
            raise_(ftt.FTTError, detail)
        if r > 0:
            now = time.time()
    ...
    def write(self, buf, offset, nbytes):
        if self.mode != 1:
            Trace.log(
                e_errors.ERROR,
                "ftt_driver:write: file not open for writing")
        ...
        try:
            ...
            Trace.log(
                e_errors.ERROR, "ftt_driver:write: %s %s" %
                (detail, detail.value))
            raise_(ftt.FTTError, detail)
    ...
    def writefm(self):
        ...
        try:
            ...
            Trace.log(
                e_errors.ERROR, "ftt_driver:write_fm %s %s" %
                (detail, detail.value))
            raise_(ftt.FTTError, detail)
            #raise e_errors.WRITE_ERROR, detail
        if r == -1:
    ...
    def writefm_buffered(self):
        ...
        try:
            ...
            Trace.log(
                e_errors.ERROR, "ftt_driver:writefm_buffered %s %s" %
                (detail, detail.value))
            raise_(ftt.FTTError, detail)
        if r == -1:
            ftt.raise_ftt()
    ...
    def flush_data(self):
        ...
        try:
            ...
            Trace.log(
                e_errors.ERROR, "ftt_driver:flush_data %s %s" %
                (detail, detail.value))
            raise_(ftt.FTTError, detail)
            #raise e_errors.WRITE_ERROR, detail
        if r == -1:
    ...
    def eject(self):
        ...
        for retry in (0, 1, 2):
            ...
            if not s:
                ...
                Trace.log(
                    e_errors.ERROR, "eject: mt offline failed: %s" %
                    (r,))
                if string.find(r, "Input/output error") >= 0:
                    ...
                    if s:
                        Trace.log(
                            e_errors.ERROR, "eject: mt rewind failed: %s" %
                            (r,))
                # there is no media no need to offline
                elif string.find(r, "No medium found") >= 0:
                    ok = 1
        if not ok:
    ...
    def set_mode(self, density=None, compression=None, blocksize=None):
        ...
        if compression is None:
            ...
            Trace.log(
                e_errors.WARNING, "set_mode: wrong compression value %s. Using default" %
                (compression,))
            compression = mode[2]
        ...
        try:
            ...
            Trace.trace(
                42, "%s=ftt.set_mode(%s,%s,%s) done" %
                (r, density, compression, blocksize,))
        except ftt.FTTError as detail:
            Trace.log(
                e_errors.ERROR, "set_mode %s %s" %
                (detail, detail.value))
            return -1
    ...
    def verify_label(self, volume_label=None, mode=0, expected_length=1440):
        ...
        try:
            ...
            if self.fd is None:
                return {0: e_errors.READ_BADSWMOUNT,
                        1: e_errors.WRITE_BADSWMOUNT}[mode], None
            try:
                nbytes = self.read(buf, 0, expected_length)
            except Exceptions as detail:
                Trace.log(
                    e_errors.ERROR, "verify_label returned: %s" %
                    (detail,))
                nbytes = 0
            # the returned data should be a modulo of 80.
            if not nbytes or nbytes % 80 != 0:
                Trace.trace(25, "read %s bytes checking label" % (nbytes,))
                return {0: e_errors.READ_VOL1_READ_ERR,
                        1: e_errors.WRITE_VOL1_READ_ERR}[mode], None
            Trace.trace(25, "verify_label: read %s" % (buf,))
            if buf[:4] != "VOL1":
                return {0: e_errors.READ_VOL1_MISSING,
                        1: e_errors.WRITE_VOL1_MISSING}[mode], None
            ...
            if s[0] != volume_label[0:6]:
                Trace.log(
                    e_errors.ERROR, "ftt_driver:verify_label read label returned %s" %
                    (s,))
                return {0: e_errors.READ_VOL1_WRONG,
                        1: e_errors.WRITE_VOL1_WRONG}[mode], s[0]

            return e_errors.OK, None
        except ftt.FTTError as detail:
            Trace.log(
                e_errors.ERROR, "ftt exception reading VOL1 label: %s" %
                (detail,))
            ftt_error = _ftt.ftt_get_error()
            Trace.log(DEBUG_LOG, 'ftt errors %s' % (ftt_error,))
            return {0: e_errors.READ_VOL1_READ_ERR, 1: e_errors.WRITE_VOL1_READ_ERR}[
                mode], str(detail), ftt_error
        except BaseException:
            exc, msg = sys.exc_info()[:2]
            Trace.log(e_errors.ERROR, "reading VOL1 label: %s %s" % (exc, msg))
            return {0: e_errors.READ_VOL1_READ_ERR,
                    1: e_errors.WRITE_VOL1_READ_ERR}[mode], str(detail)
