...
class FTTDriver(driver.Driver):
    ...
    def open(self, device=None, mode=None):
        ...
        self._rate = 0
        self._bytes_transferred = 0
        if verbose: print "ftt_open returns", self.ftt
        for retry in xrange(10):
            if retry:
                if verbose: print "retrying open"
            try:
                self.fd = self.ftt.open_dev()
                break
            except ftt.FTTError, detail:
                if verbose: print detail, detail.errno
                if detail.errno == ftt.EBUSY:
                    time.sleep(5)
                else:
                    break
        self._rate = self._last_rate = self._bytes_transferred = 0
        if verbose: print "ftt_open_dev returns", self.fd
        return self.fd

    def reopen(self, device, mode):
        if verbose: print "ftt reopen"
        if not self.ftt:
        ...
        self._rate = self._last_rate = self._bytes_transferred = 0
        if verbose: print "reopen: ftt_open_dev returns", self.fd
        return self.fd

    
    def rewind(self):
        r = self.ftt.rewind()
        if verbose: print "ftt_rewind returns", r

    def tell(self):
        if not self.ftt:
            if verbose: print "tell: no ftt descriptor"
            return None
        file, block = self.ftt.get_position()
        if verbose: print "ftt_get_position returns", file, block
        return file
    ...
    def seek(self, target):
        ...
        if current == target:
            ...
            if verbose: print "seek: current = ", current, "target=",target
        if target>current:
        ...
        current = self.tell()
        if verbose: print "seek2: current=", current, "target=", target
        if current != target:
    ...
    def flush(self):
        if verbose: print "flushing", self.ftt
        now=time.time()
        if verbose:
        ...
        if verbose: print "ftt_close_dev returns",r
        self.fd = -1
        return r

    def close(self):
        if verbose: print "closing", self.ftt
        r = self.ftt.close()
        if verbose: print "ftt_close returns",r
        self.ftt = None
        self.fd = -1
        return r
    ...
    def set_mode(self, density=None, compression=None, blocksize=None):
        ...
        if verbose: print "ftt mode is", self.ftt.get_mode()
        return r
