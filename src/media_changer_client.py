...
class MediaChangerClient(generic_client.GenericClient):
    ...
    def viewvol(self, volume, m_type, rcv_timeout=0,
                rcv_tries=0, reserve=False):
        ticket = {'work': 'viewvol',
    ...
    def __list_volumes2(self, control_socket, ticket):
        ...
        while t:
            try:
                ...
                if t and isinstance(t, type(())) and not t[0]:
    ...
    def update_db(self, volume_address, volume_name,
        ...
        if all(v is None for v in (volume_address, volume_name,
                                   drive_address, volume_name_in_drive)):
            # on the server side there will be inventory running for quite a
            # while
            to = 300
            retry = 3
        else:
