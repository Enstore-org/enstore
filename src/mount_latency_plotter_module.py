...
#SELECT_STATEMENT="select start, to_char(finish-start, 'HH24:MI:SS') as latency "+\
        ...
        for type in self.types:
            ...
            if drive_type:
                ...
                for mc in self.mc_mover_map.keys():
                    ...
                    #
                    q=SELECT_STATEMENT%("M",drive_type,string.join(movers,"','"),)
                    self.__fill_ntuple(q,drive_type,mc=mc)
                    #
                    # plot Dismount latencies  per robot library per media type
                    #
                    q=SELECT_STATEMENT%("D",drive_type,string.join(movers,"','"),)
                ...
                if name == tag :
                    ...
                    if ntuple:
                        ...
                        for datum in latencies:
                            start  = time.mktime(time.strptime(datum["start"],'%Y-%m-%d %H:%M:%S'),)
                            finish = time.mktime(time.strptime(datum["finish"],'%Y-%m-%d %H:%M:%S'),)
