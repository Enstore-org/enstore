...
class GenericClient:
    ...
    def _is_csc(self):
        ...
        # self.__dict__.get('is_config', 0):
        if self.name == enstore_constants.CONFIGURATION_CLIENT:
            return 1
        else:
            return 0
    ...
    def send(self, ticket, rcv_timeout=0, tries=0, long_reply=None):
        try:
            ...
            if hasattr(
                    msg, "errno") and msg.errno and msg.errno == errno.ETIMEDOUT:
        ...
        if e_errors.is_ok(x) and \
           ((long_reply is None and
             isinstance(x, dict) and x.get('long_reply', None))
                or (long_reply is not None and long_reply)):
            ...
            if (hasattr(self, "server_address") and
                ...
                # there is no need to check if access from this server is
                # allowed
                pass
            else:
                ...
                if not hostaddr.allow(x['callback_addr']):
                    x['status'] = "address %s not allowed" % (
                        x['callback_addr'],)
                    return x

            try:
                connect_socket = callback.connect_to_callback(
                    x['callback_addr'])
                x['status'] = (e_errors.OK, None)
            except (socket.error) as msg:
                message = "failed to establish control socket: %s" % (
                    str(msg),)
    ...
    # return the name used for this client/server #XXX what is this nonsense?
    # cgw
