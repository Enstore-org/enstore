...
def print_html(catalog, special, cluster):
    ...
    for fname in special:
        if fname == 'COMPLETE_FILE_LISTING':
            print(
                '<a href="enstore_file_listing_cgi.py">',
                string.split(
                    fname,
                    '.')[0],
                '</a>&nbsp;&nbsp;')
        elif fname.startswith(("COMPLETE_FILE_LISTING", "CHIMERA_DUMP", "RECENT_FILES_ON_TAPE")):
            # We hide these behind the cgi that COMPLETE_FILE_LISTING points
            # to.
            continue
        else:
            print(
    ...
    print('<a href="enstore_recent_files_on_tape_cgi.py">',
          "RECENT_FILES_ON_TAPE", '</a>&nbsp;&nbsp;')
    ...
    if catalog:
        for i in keys:
            ...
            for j in catalog[i]:
                print(
                    '<a href=/cgi-bin/show_volume_cgi.py?volume=' +
                    j +
                    '>',
                    j,
                    '</a>&nbsp;&nbsp;')
