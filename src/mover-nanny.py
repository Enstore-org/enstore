...
def get_status(mover):
    p = os.popen(
        "enstore mov --status --timeout=120 --retries=1 %s" %
        mover, 'r')
    r = p.read()
    s = p.close()
    l = string.split(r, '\n')
    ...
    while True:
        ...
        for mover in movers:
                ...
                if len(lines) == 1:
                    ...
                    if len(words) > 2 and words[2] == 'D':
                        ...
                        if reset_on_error:
                            ...
                            reset(
                                mover,
                                reason="Uninterruptible I/O wait.\nProcess status: %s" %
                                lines[0])
                if len(lines) == 0:
                    if reset_on_error:
                        noreset = 0
            ...
            if err and noreset:
                ...
                if n_strikes > 3:
                    strikes[mover] = 0
                    reset(mover, reason="%d sequential errors getting mover status"
                          % n_strikes)
                else:
...
if __name__ == '__main__':
    ...
    opts, args = getopt.getopt(
        sys.argv[1:], "h", ["check-null", "reset-on-error"])
    for o, a in opts:
        if o == "-h":
            usage()
        if o == "--check-null":
