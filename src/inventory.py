...
def get_vol_filenames(output_dir):
    if string.find(output_dir, "/dev/stdout") != -1:
        ...
        write_protect_alert_file = os.path.join(
            output_dir, "WRITE_PROTECTION_ALERT")
        volumes_too_many_mounts_file = os.path.join(
            output_dir, "VOLUMES_TOO_MANY_MOUNTS")
...
def get_authorized_tapes2():
    ...
    try:
        ...
        if auth_file is not None:
            ...
            return {}
    except (OSError, IOError, AttributeError) as detail:
...
def print_footer(volume, s_i, fd):
    ...
    fields_list = sorted(volume.keys())
...
def print_volume_quotas_status(
        volume_quotas, authorized_tapes, output_file, quotas):
    ...
    for keys in quotas:
        ...
        if keys in b_order:
            ...
            for t_order in order.get('top', []):
                if t_order[0] is None and t_order[1] == volume_quotas[keys][1]:
                    top.append(keys)
                elif t_order[1] is None and \
                        t_order[0] == volume_quotas[keys][0]:
                    top.append(keys)

            for b_order in order.get('bottom', []):
                if b_order[0] is None and b_order[1] == volume_quotas[keys][1]:
                    bottom.append(keys)
                elif b_order[1] is None and \
    ...
    for quotas in (top, middle, bottom):
        ...
        for keys in quotas:
            ...
            if volume_quotas[keys][4]:
                bk = '%d/%d' % (volume_quotas[keys]
                                [13], volume_quotas[keys][4])
            else:
                bk = '0'
...
def print_volume_quota_sums(volume_quotas, authorized_tapes, output_file,
    ...
    for key in keys:
        ...
        # vq_file.write("%2d %-15s %-15s %-11s %-12s %-6s %-9d %-10d %-12d
        # %-12d %9.2f%-3s %-12d %-13d %-13d %-16d %-13d\n"
...
def print_migrated_volumes_header(fp):
    ...
    fp.write(
        "These migrated or cloned volumes MAY be recycled or deleted from system:\n\n")
...
def verify_volume_sizes(this_volume_data, volume, volume_sizes):
    ...
    try:
        ...
        # This exception occurs on empty files. So, these are zero.
        sum_size = 0
        deleted_size = 0
...
def inventory(output_dir, cache_dir):
    ...
    n_vols = 0          # total number of volumes
    n_files = 0         # total number of files
    n_vols_lib = {}      # number of volumes per library
    n_rf_vols = {}       # number of vols that should be write-protected per library
    # number of volumes in n_rp_vols that are not write-protected per library
    n_not_rp_vols = {}
    ...
    for vk in vol_db.keys():
        ...
        if vk in vol_sum:
            try:
                ...
                # can be ignored
                print("(warning) cache problem ...", end=' ')
                vsum = {}
        else:
            vsum = {}
        ...
        if (library, storage_group) in volumes_allocated:
        ...
        if vv['media_type'] in mount_limit:
            if mounts > mount_limit[vv['media_type']
                                    ][0] and vv['library'].find("shelf") == -1:
    ...
    print("logging remaining_blanks to accounting db ...", end=' ')
    res = file_db.dbaccess.query_getresult("select * from remaining_blanks")
    for i in res:
        q = "insert into blanks values('%s', '%s', %d)" % (
            time2timestamp(t0), i[0], i[1])
        acs.db.query(q)
    print('done')
    ...
    if hour == 22:
        q = "insert into write_protect_summary (date, total, should, not_yet, done) values('%s', %d, %d, %d, %d);" % (
            time2timestamp(t0), n_vols, sum(n_rf_vols.values()), sum(n_not_rp_vols.values()), sum(n_rp_vols.values()))
        res = acs.db.query(q)
        # log individual numbers according to library
        for i in n_rf_vols.keys():
            q = "insert into write_protect_summary_by_library (date, library, total, should, not_yet, done) values('%s', '%s', %d, %d, %d, %d);" % (
                time2timestamp(t0), i, n_vols_lib[i], n_rf_vols[i], n_not_rp_vols[i], n_rp_vols[i])
            res = acs.db.query(q)
    ...
    os.system(
        'cp ' +
        volumes_defined_file +
        ' ' +
        volumes_defined_file +
        '.html')
    os.system(
        r'sed -e "s/<font color=#FF0000>//g; s/<\/font>//g; s/<blink>//g; s/<\/blink>//g" ' +
        volumes_defined_file +
        '.html > ' +
        volumes_defined_file)
...
def inventory_dirs():
    ...
    if not 'status' in inven.keys():
        ...
        print(tod(), 'Configuration Server BAD STATUS', repr(inven['status']))
        sys.exit(1)
    else:
...
#    print "inventory_rcp_dir", inventory_rcp_dir
    ...
    if "--stdout" in sys.argv:
        ...
        # Makes no sense to move files that don't exist.
        inventory_rcp_dir = ""
    elif "-o" in sys.argv:
