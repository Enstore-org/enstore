...
def usage(cmd):
    ...
    print(
        "\t -D [--drive-hours]     : plot drive hours per day, stacked by storage group")
    print(
        "\t -H [--drive-hours-sep] : plot drive hours per day, separately for each storage group")
    ...
    print(
        "\t -F [--files-rw]        : plot file reads and writes per mount, stacked by storage group")
...
if __name__ == "__main__":
    ...
    for o, a in opts:
        ...
        # ratekeeper plots
        if o in ("-r", "--rate"):
            aModule = ratekeeper_plotter_module.RateKeeperPlotterModule(
                "ratekeeper")
            f.add(aModule)
        # drive utilization
        if o in ("-u", "--utilization", "-d", "--drives"):
            aModule = drive_utilization_plotter_module.DriveUtilizationPlotterModule(
                "utilization")
            f.add(aModule)
        # drive hours per day, stacked by storage group
        if o in ("-D", "--drive-hours"):
            aModule = drive_hours_plotter_module.DriveHoursPlotterModule(
                "drive-hours")
            f.add(aModule)
        # drive hours per day, separately for each storage group
        if o in ("-H", "--drive-hours-sep"):
            aModule = drive_hours_sep_plotter_module.DriveHoursSepPlotterModule(
                "drive-hours-sep")
            f.add(aModule)
        # slot utilization
        if o in ("-s", "--slots"):
            aModule = slots_usage_plotter_module.SlotUsagePlotterModule(
                "slots")
            f.add(aModule)
        # pnfs backup time
        if o in ("-p", "--pnfs-backup"):
            aModule = pnfs_backup_plotter_module.PnfsBackupPlotterModule(
                "pnfs_backup")
            f.add(aModule)
        # file family analysis
        if o in ("-f", "--file-family-analysis"):
            aModule = file_family_analysis_plotter_module.FileFamilyAnalysisPlotterModule(
                "file_family_analysis")
            f.add(aModule)
        # encp rate multi
        if o in ("-e", "--encp-rate-multi"):
            aModule = encp_rate_multi_plotter_module.EncpRateMultiPlotterModule(
                "encp_rate_multi")
            f.add(aModule)
        # quotas
        if o in ("-q", "--quotas"):
        ...
        if o in ("-t", "--tapes-burn-rate"):
            aModule = tapes_burn_rate_plotter_module.TapesBurnRatePlotterModule(
                "tapes_burn_rate")
            f.add(aModule)
        # migration summary
        if o in ("-i", "--migration-summary"):
            aModule = migration_summary_plotter_module.MigrationSummaryPlotterModule(
                "migration_summary")
            f.add(aModule)
        # bytes per day
        if o in ("-b", "--bytes-per-day"):
            aModule = bytes_per_day_plotter_module.BytesPerDayPlotterModule(
                "bytes-per-day")
            f.add(aModule)
        # mover summary
        if o in ("-M", "--mover-summary"):
            aModule = mover_summary_plotter_module.MoverSummaryPlotterModule(
                "mover-summary")
            f.add(aModule)
        # latencies
        if o in ("-l", "--latencies"):
            aModule = mount_latency_plotter_module.MountLatencyPlotterModule(
                "latencies")
            f.add(aModule)
        # library mounts
        if o in ("-L", "--library-mounts"):
            aModule = mounts_per_robot_plotter_module.MountsPerRobotPlotterModule(
                "library-mounts")
            f.add(aModule)
        ...
        if o in ("-W", "--files-rw-sep"):
            aModule = files_rw_sep_plotter_module.FilesRWSepPlotterModule(
                "files-rw-sep")
            f.add(aModule)
        if o in ("-S", "--sfa-stats"):
            if f.csc.get("dispatcher"):
                import sfa_plotter_module
                SFA_Stats_Module = sfa_plotter_module.SFAStatsPlotterModule(
                    "SFA_Statistics")
                f.add(SFA_Stats_Module)
                pack_data_file = f.csc.get(
                    "SFA_Stats", {}).get("packaging_rates_data")
                unpack_data_file = f.csc.get(
                    "SFA_Stats", {}).get("unpackaging_rates_data")
                if pack_data_file:
