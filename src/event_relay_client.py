...
class EventRelayClient:
    ...
    def read(self, fd=None):
        ...
        if not self.invalid:
            ...
            # caught by the calling routine. (enstore_functions.read_erc does
            # it)
            msg = fd.recv(1024)
    ...
    def dump(self):
        ...
        self.dump_msg = event_relay_messages.EventRelayDumpMsg(
            self.host, self.port)
    ...
    def quit(self):
        ...
        self.quit_msg = event_relay_messages.EventRelayQuitMsg(
            self.host, self.port)
    ...
    def do_print(self):
        ...
        self.doprint_msg = event_relay_messages.EventRelayDoPrintMsg(
            self.host, self.port)
    ...
    def dont_print(self):
        ...
        self.dontprint_msg = event_relay_messages.EventRelayDontPrintMsg(
            self.host, self.port)
    ...
    def event_relay_heartbeat(self):
        ...
        self.heartbeat_msg = event_relay_messages.EventRelayHeartbeatMsg(
            self.host, self.port)
    ...
    def alive(self):
        ...
        if self.start([event_relay_messages.ALIVE, ]) == self.SUCCESS:
            ...
            # wait for events
            readable, junk, junk = select.select(
                [self.sock], [], [], DEFAULT_TIMEOUT)
            if readable:
                for fd in readable:
