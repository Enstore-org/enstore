...
def volume_assert_client_version():
    ...
    if encp_file:
        version_string = version_string + os.path.basename(encp_file)
...
def parse_comma_list(comma_seperated_string):
    # paranoid check
    if not isinstance(comma_seperated_string, bytes):
        return []
...
def report_assert_results(done_ticket):
    ...
    lc_keys = sorted(done_ticket.get('return_file_list', {}).keys())
    for key in lc_keys:
...
def handle_assert_requests(unique_id_list, assert_list, listen_socket, intf):
    ...
    while len(error_id_list) + len(completed_id_list) < len(assert_list):
        ...
        if done_ticket.get('unique_id', None) is not None:
...
def main(intf):
    ...
    assert_list, listen_socket = create_assert_list(
        check_requests, media_validate=validate)
