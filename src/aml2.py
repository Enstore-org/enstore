...
def view(volume, media_type):
    ...
    if media_code is None:
        Trace.log(
            e_errors.ERROR, 'Media code is None. media_type=%s' %
            (media_type,))
        return (-1, None)
    ...
    if stat != 0:
        ...
        Trace.log(
            e_errors.ERROR, 'aci_qvolsrange returned status=%d' %
            (stat,))
        return stat, None
    ...
    if volsers is None:
        stat = aci.cvar.d_errno
        Trace.log(
            e_errors.ERROR, 'volume %s %s NOT found' %
            (volume, media_type))
        return stat, None
...
def mount(volume, drive, media_type, view_first=1):
    media_code = aci.__dict__.get("ACI_" + media_type)
    if media_code is None:
        return 'BAD', e_errors.MC_NONE, 'Media code is None. media_type= %s' % (
            media_type,)
    ...
    if view_first:
        ...
        if stat != 0:
            return 'BAD', e_errors.MC_FAILCHKVOL, 'aci_view return code=%d' % (
                stat,)
        if volstate is None:
            return 'BAD', e_errors.MC_VOLNOTFOUND, 'volume %s not found = %d' % (
                volume, stat)
        if volstate.attrib != "O":  # look for tape in tower (occupied="O")
            return 'BAD', e_errors.MC_VOLNOTHOME, 'Tape %s is not in home position in tower. location=%s' % (
                volume, volstate.attrib,)
    ...
    # check if any tape is mounted in this drive
        ...
        if stat != 0:
            return 'BAD', e_errors.MC_FAILCHKDRV, 'aci_drivestatus2 return code = %d' % (
                stat,)
        if drvstate is None:
            return 'BAD', e_errors.MC_DRVNOTFOUND, 'drive %s not found = %d ' % (
                drive, stat)
        if drvstate.volser != "":  # look for any tape mounted in this drive
            return 'BAD', e_errors.MC_DRVNOTEMPTY, 'Drive %s is not empty. Found volume %s' % (
                drive, drvstate.volser)
...
def dismount(volume, drive, media_type, view_first=1):
    ...
    if view_first:
        ...
        if drvstate.volser == "":  # look for any tape mounted in this drive
            if volume != "Unknown":
                # FIXME: mover calling with tape when there is none in drive.
                # Return OK for now
                return 'ok', 0, 'Drive %s is empty. Thought volume %s was there.' % (
                    drive, volume)
    ...
    if arm == 'R1' or arm == 'Both':
        ...
        if not status:
            ...
            if status:
                Trace.trace(
                    e_errors.INFO,
                    'aml2 robotHomeAndRestart Start failed on R1')
                return status_table[status][0], status, status_table[status][1]
        else:
            Trace.trace(
                e_errors.INFO,
                'aml2 robotHomeAndRestart Home failed on R1')
            return status_table[status][0], status, status_table[status][1]
    if arm == 'R2' or arm == 'Both':
        ...
        if not status:
            ...
            if status:
                Trace.trace(
                    e_errors.INFO,
                    'aml2 robotHomeAndRestart Start failed on R2')
                return status_table[status][0], status, status_table[status][1]
        else:
            Trace.trace(
                e_errors.INFO,
                'aml2 robotHomeAndRestart Home failed on R2')
            return status_table[status][0], status, status_table[status][1]
    return status_table[status][0], status, status_table[status][1]
...
def insert(ticket, classTicket):
    ...
    # If the user/caller specified a specific IO box area, set the list to
    # that.
    status = 0
    if "IOarea_name" in ticket and len(ticket["IOarea_name"]) > 0:
        ...
        for IOarea_name in IOarea_input:
            if IOarea_name not in Iareas:
                ...
                mcSelf.workQueueClosed = 0
                Trace.trace(e_errors.ERROR,
                            'aml2 bad IOarea parameter specified.')
                return status_table[status][0], status, status_table[status][1]
...
def eject(ticket, classTicket):
    ...
    for volser in volumeList:
        ...
        if volsers[0].attrib == "E":
            return e_errors.NO_VOLUME, derrno.ENOVOLUME, "volser %s already ejected" % (
                volser,)
        # Make sure the tape is not being used.
        elif volsers[0].attrib != "O":
            return e_errors.NO_VOLUME, derrno.ENOVOLUME, "%s: %s" % (
                status_table[derrno.ENOVOLUME][1], volser)
        ...
        if "IOarea_name" in ticket:
            ...
            try:
                box = random.randint(
                    0, len(mediaAssgn["ACI_" + media_type]) - 1)
