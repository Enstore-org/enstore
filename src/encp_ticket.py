...
def write_request_ok(ticket):
    ...
    if isinstance(ticket['file_size'], int):
        fsize_type = 0
    elif isinstance(ticket['file_size'], type(None)):
        fsize_type = None

    inode_type = long(0)
    if isinstance(ticket['wrapper']['inode'], int):
        inode_type = 0
    elif isinstance(ticket['wrapper']['inode'], type(None)):
        inode_type = None
    ...
    for key in encp_keys:
        if key not in ticket['encp']:
            return key

    for key in fc_keys:
        if key not in ticket['fc']:
            return key

    for key in vc_keys:
        if key not in ticket['vc']:
            return key

    for key in wrapper_keys:
        if key not in ticket['wrapper']:
            return key
        else:
            if not isinstance(ticket['wrapper'][key], type(wrapper_keys[key])):
...
if __name__ == "__main__":
    ...
    w_ticket = {'status': ('ok', None), 'routing_callback_addr': ('131.225.202.12', 32970),
                ...
                'callback_addr': ('131.225.202.12', 54874), 'file_size': 36864036,
                'lm': {'address': ('131.225.13.4', 7522)},
                'unique_id': 'dellquad2.fnal.gov-1075133530-841-0',
                'fc': {'size': 36864036, 'external_label': 'VO5154', 'address': ('131.225.13.59', 7501)},
                'route_selection': 0,
                ...
                'at_the_top': 6,
                'mover': {'status': ('ok', None), 'check_written_file': 100, 'max_buffer': 1363148800,
                ...
                'wrapper': {'major': 0, 'rminor': 0,
                            ...
                            'sanity_size': 65536, 'pstat': (16877, 145747256, 9, 1, 2937, 1540, 512,
                                                            1075133530, 1075133530, 1075123605),
                            'mode': 32782, 'rmajor': 0, 'size_bytes': 36864036,
    ...
    r_ticket = {'status': ('ok', None),
                'vc': {'status': ('ok', None), 'comment': '', 'storage_group': 'selex',
                       'si_time': [993676719.0, 997734795.0], 'capacity_bytes': 20401094656,
                       ...
                       'wrapper': 'cpio_odc', 'remaining_bytes': 364952576, 'sum_mounts': 231,
                ...
                'fc': {'status': ('ok', None), 'complete_crc': 3020422051,
                       ...
                       'sanity_cookie': (65536, 2090556793), 'size': 80907408},
                'encp': {'agetime': 0, 'curpri': 1, 'adminpri': -1, 'delpri': 0, 'basepri': 1,
                         'delayed_dismount': None},
                'file_size': 80907408, 'at_the_top': 3,
                'wrapper': {'major': 0, 'rminor': 0,
                            ...
                            'uname': 'jurgen', 'pstat': (33188, 185388456, 1310720, 1, 1727, 1747,
                                                         80907408, 1059340564, 997734565, 997734522),
                            'gid': 1747, 'mode': 33252, 'gname': 'e781', 'size_bytes': 80907408,
