...
PLOT_INFO = [[enstore_constants.MPH_FILE, "Mounts/Hour (no null mvs)"],
             [enstore_constants.D_MPD_FILE,
              "Mounts/Day (no null mvs as of 8/1/01)"],
             [enstore_constants.MPD_FILE,
              "Mounts/Day (no null mvs as of 8/1/01)"],
             [enstore_constants.MPD_MONTH_FILE,
              "Mounts/Day (30 days) (no null mvs)"],
             [enstore_constants.MLAT_FILE, "Mount Latency (no null mvs)"],
             [enstore_constants.BPD_MONTH_FILE_W,
              "Bytes Written/Day (30 days) (no null mvs)"],
             ...
             [enstore_constants.XFERLOG_FILE,
              "Transfer Activity (log - no null mvs))"],
             [enstore_constants.XFER_FILE, "Transfer Activity (no null mvs)"],
             [enstore_constants.NULL_RATES, "Null Terabytes/Day"],
             [enstore_constants.REAL_RATES,
              "Real Terabytes/Day (no null mvs)"],
...
class EnBaseHtmlDoc(HTMLgen.SimpleDocument):
    ...
    def nav_table(self):
        ...
        tr.append(HTMLgen.TD(HTMLgen.Bold(HTMLgen.Font(
                  HTMLgen.Href(
                      "%sstatus_enstore_system.html" %
                      (self.nav_link,), SERVERS),
                  size="+2")), bgcolor=NAV_TABLE_COLOR))
        tr.append(HTMLgen.TD(HTMLgen.Bold(HTMLgen.Font(
                  HTMLgen.Href(
                      "%sencp_enstore_system.html" %
                      (self.nav_link,), 'Encp'),
                  size="+2")), bgcolor=NAV_TABLE_COLOR))
        if self.help_file:
...
class EnMoverStatusPage(EnBaseHtmlDoc):
    ...
    def add_bytes_volume_info(self, moverd, tr, mvkey):
        if enstore_constants.VOLUME in moverd:
    ...
    def add_bytes_eod_info(self, moverd, tr, mvkey):
        if enstore_constants.EOD_COOKIE in moverd:
            tr.append(HTMLgen.TD(moverd[mvkey]))
        ...
        elif enstore_constants.LOCATION_COOKIE in moverd:
            tr.append(HTMLgen.TD(moverd[mvkey]))
    ...
    def add_files(self, moverd, table):
        if enstore_constants.FILES in moverd:
    ...
    def mv_row(self, mover, table):
        ...
        if moverd:
            ...
            if enstore_constants.STATE in moverd and \
               moverd[enstore_constants.STATE]:
                ...
                if enstore_constants.LAST_READ in moverd:
                    tr = HTMLgen.TR(HTMLgen.TD(HTMLgen.Font(
                ...
                elif enstore_constants.CUR_READ in moverd:
                    tr = HTMLgen.TR(HTMLgen.TD(HTMLgen.Font(
    ...
    def migrator_row(self, migrator, table):
        ...
        if md:
            for k in md:
                # we may have gotten an error when trying to get it,
                # so look for a piece of it.  (efb used to be 'or')
                if isinstance(md[k], dict) and enstore_constants.STATE in md[k] and \
                   md[k][enstore_constants.STATE]:
                    ...
                    m_table.append(tr)
                    if enstore_constants.LAST_READ in md[k]:
                        tr = HTMLgen.TR(HTMLgen.TD(HTMLgen.Font(
                            "Last%sRead%s(bytes)" % (NBSP,
                                                     NBSP),
                            color=BRICKRED,
                    ...
                    elif enstore_constants.CUR_READ in md[k]:
                        tr = HTMLgen.TR(HTMLgen.TD(HTMLgen.Font(
                                                   "Current%sRead%s(bytes)" % (NBSP,
                                                                               NBSP),
                                                   color=BRICKRED,
...
class EnLmStatusPage(EnBaseHtmlDoc):
    ...
    def __init__(self, lm, refresh=60, system_tag="",
                 max_lm_rows=DEFAULT_LM_ROWS):
        EnBaseHtmlDoc.__init__(self, refresh=refresh,
    ...
    def parse_queues(self):
        ...
        self.vols = {}
        if enstore_constants.WORK not in self.data_dict or \
        ...
        if not self.data_dict[enstore_constants.WORK] == \
            ...
            for mover in self.data_dict[enstore_constants.WORK]:
                if mover[enstore_constants.WORK] == enstore_constants.READ:
                    ...
                    # this is a write
                    if mover[enstore_constants.FILE_FAMILY] in self.ff:
                        self.ff[mover[enstore_constants.FILE_FAMILY]].append(
                            self.write_q_list(mover))
                    else:
                        self.w_ff.append(self.write_q_list(mover))
                        self.ff[mover[enstore_constants.FILE_FAMILY]] = []
        # parse the pending read queue
        if enstore_constants.PENDING in self.data_dict and \
           not self.data_dict[enstore_constants.PENDING] == enstore_constants.NO_PENDING:
            for mover in self.data_dict[enstore_constants.PENDING][enstore_constants.READ]:
                if mover[enstore_constants.DEVICE] in self.vols:
                    self.vols[mover[enstore_constants.DEVICE]].append(
                        self.read_q_list(mover))
                else:
                    self.r_vols.append(self.read_q_list(mover))
                    self.vols[mover[enstore_constants.DEVICE]] = []
    ...
    def read_row(self, elem, print_device=0):
        ...
        if print_device:
            ...
            # we do not need to print the volume label, again, must have done
            # it before
            tr.append(empty_data())
    ...
    def write_row(self, elem, print_ff=0):
        ...
        if print_ff:
            ...
            # we do not need to print the file family, again, must have done it
            # before
            tr.append(empty_data())
        if elem[3] is not enstore_constants.NOMOVER:
            tr.append(HTMLgen.TD("[at%s%s]" % (NBSP,
    ...
    def get_in_vol_order(self, table):
        ...
        for elem in self.r_vols:
            if num_done > self.max_lm_rows and not self.max_lm_rows == DEFAULT_ALL_ROWS:
                # we have put the max number on the main page, now make an
                # additional page
    ...
    def other_vol_info(self, table):
        if enstore_constants.ACTIVE_VOLUMES not in self.data_dict or \
    ...
    def main_table(self, table):
        self.busy_movers = []
        # add the status line
        str1 = HTMLgen.Font("Status%s:" %
                            (NBSP,), size="+1", html_escape='OFF')
...
class EnLmFullStatusPage(EnBaseHtmlDoc):
    ...
    def __init__(self, lm, refresh=60, system_tag="",
                 max_lm_rows=DEFAULT_FULL_LM_ROWS):
        EnBaseHtmlDoc.__init__(self, refresh=refresh,
        ...
        if enstore_constants.FILE_FAMILY in qelem:
            ...
            if work and work == enstore_constants.WRITE:
                ...
                tr.append(HTMLgen.TD(
                    qelem[enstore_constants.FILE_FAMILY_WIDTH]))
        elif enstore_constants.VOLUME_FAMILY in qelem:
    ...
    def first_queue_row(self, qelem, intro):
        ...
        tr = HTMLgen.TR(HTMLgen.TD(HTMLgen.Font(text, color=BRICKRED,
                                                html_escape='OFF')))
        if enstore_constants.MOVER in qelem and \
        ...
        tr.append(HTMLgen.TD(enstore_functions2.strip_node(
            qelem[enstore_constants.NODE])))
    ...
    def make_lm_queue_rows(self, qelems, intro):
        ...
        for qelem in qelems:
            ...
            if enstore_constants.REJECT_REASON in qelem:
    ...
    def lm_queue_rows(self, qelems, queue, intro):
        qlen = len(qelems)
        tr0 = None
        if (not self.max_lm_rows == DEFAULT_ALL_ROWS) and (
    ...
    def work_at_movers_row(self, cols):
        ...
        if not the_work == enstore_constants.NO_WORK:
            rows = self.lm_queue_rows(
                the_work, enstore_constants.WORK, AT_MOVERS)
        else:
    ...
    def pending_work_row(self, cols):
        ...
        if the_work and not the_work == enstore_constants.NO_PENDING and \
           (the_work[enstore_constants.READ] or the_work[enstore_constants.WRITE]):
            ...
            rows = self.lm_queue_rows(
                qelems, enstore_constants.PENDING, PENDING)
        else:
    ...
    def lm_rows(self, table):
        ...
        if enstore_constants.LMSTATE in self.data_dict:
...
class EnFileListPage(EnBaseHtmlDoc):
    ...
    def __init__(self, refresh=60, system_tag="",
                 max_rows=DEFAULT_FILE_LIST_ROWS):
        EnBaseHtmlDoc.__init__(self, refresh=refresh,
                               help_file="fileHelp.html",
                               system_tag=system_tag)
    ...
    def main_list(self, filelist, table):
        ...
        if (not self.max_rows == DEFAULT_ALL_ROWS) and flen > self.max_rows:
            ...
            flen = self.max_rows
            new_key = "file_list"
            self.extra_queue_pages[new_key] = (
                EnExtraFileListPages(self), filename)
            self.extra_queue_pages[new_key][0].body(
                self.file_list_table(filelist[flen:]))
        table.append(HTMLgen.TR(HTMLgen.TD(
            self.file_list_table(filelist[0:flen], tr))))
...
class EnSysStatusPage(EnBaseHtmlDoc):
    ...
    def shortcut_table(self):
        ...
        if got_unmonitored_server:
            tr, num_tds_so_far = add_to_scut_row(num_tds_so_far, tr, table,
                                                 '#%s' % (
                                                     first_unmonitored_server,),
                                                 UNMONITORED_SERVERS)
        # add a link to the full file list page
        tr, num_tds_so_far = add_to_scut_row(num_tds_so_far, tr, table,
                                             '%s' % (
                                                 enstore_constants.FILE_LIST_NAME),
                                             "Full File List")
    ...
    def generic_server_rows(self, table):
        for server in enstore_constants.GENERIC_SERVERS:
            if server in self.data_dict and self.data_dict[server]:
    ...
    def xfer_row(self, lm):
        ...
        tr.append(HTMLgen.TD(
            self.data_dict[lm][enstore_constants.TOTALONXFERS]))
        tr.append(HTMLgen.TD(HTMLgen.Font("Pending%sTransfers" % (NBSP,),
                                          color=BRICKRED, html_escape='OFF')))
        tr.append(HTMLgen.TD(
            self.data_dict[lm][enstore_constants.TOTALPXFERS]))
    ...
    def pending_work_row(self, lm, cols):
        ...
        if the_work and isinstance(the_work, dict):
    ...
    def lm_rows(self, lm):
        ...
        if lm_d and string.find(lm_d[enstore_constants.STATUS][0], NBSP) == -1:
            if enstore_constants.LMSTATE in lm_d and \
        ...
        # tell if this is a bad state or not
        if enstore_constants.LMSTATE in lm_d and \
           isinstance(lm_d[enstore_constants.LMSTATE], bytes):
        ...
        # we may have gotten an error while trying to get the info,
        # so check for a piece of it first
        if enstore_constants.LMSTATE in lm_d:
            # the rest of the lm information is in a separate table, it starts
            # with the suspect volume info
    ...
    def mover_row(self, server):
        ...
        name = self.server_url(server, enstore_functions2.get_mover_status_filename(),
                               server)
        if enstore_constants.STATE in mover_d and \
    ...
    def migrator_row(self, server):
        ...
        for k in m_d:
            if isinstance(m_d[k], dict) and enstore_constants.STATE in m_d[k] and \
...
class EnLogPage(EnBaseHtmlDoc):
    ...
    def __init__(self, refresh=600, system_tag=""):
        ...
        self.source_server = THE_INQUISITOR
        self.description = "Enstore log files may also be %s" % (
    ...
    def body(self, http_path, logs, user_logs, www_host):
        table = self.table_top()
        # now add the data, first the table with the user specified log files
        # in it
...
class EnAlarmPage(EnBaseHtmlDoc):
    ...
    def sort_by_latest_time(self, alarms):
        # return a list of keys to the alarms hash, sorted from most recent
        # alarmed to earliest
        keys = alarms.keys()
        # build array
        latest_time_keys = []
    ...
    def alarm_table(self, alarms):
        ...
        for akey in akeys:
            # pick out the actual key to the alarms hash. 0 is the latest time
            akey = akey[1]
            ...
            td = HTMLgen.TD("%s" %
                            (enstore_functions2.format_time(float(alarm[0]))))
            ...
            if isinstance(alarm[11], dict):
                if enstore_constants.RA in alarm[11]:
        ...
        for file_label in PLOT_INFO:
            ...
            if not index == -1:
                ...
                if file_label[0] == enstore_constants.UTIL_FILE:
                    ...
                    just_mover = string.replace(
                        text[index:], file_label[0], "")
                    ...
                    return "%s %s" % (
                        text[index:-1].split('_')[0], file_label[1])
                else:
                    return file_label[1]
        else:
    ...
    def add_stamp(self, jpgs, stamps, pss, trs, trps, trps2):
        ...
        if stamps:
            ...
            text = "%s (%s)%s" % (self.find_label(stamp[0]),
                                  ...
                                  NBSP * 2)
            # if self.outofdate and stamp[1] < self.yesterday: (commented by
            # Dmitry)
            today = time.time()
            day_secs = 26. * 60. * 60.
...
class EnActiveMonitorPage(EnBaseHtmlDoc):
    ...
    def __init__(self, headings, refresh, system_tag=""):
        ...
        self.description = "%s%sRecent network monitoring results." % (
            NBSP, NBSP)
...
class EnSaagPage(EnBaseHtmlDoc):
    ...
    # the  alt_key is used to specify a more explicit data item than just the
    # key
    def get_element(self, dict, key, out_dict, offline_dict, alt_key="",
        ...
        if make_link != 0:
            ...
            if alt_key in enstore_constants.SERVER_NAMES:
                name = enstore_constants.SERVER_NAMES[alt_key]
            else:
                name = alt_key
            h_alt_key = HTMLgen.Href("%s#%s" %
                                     (self.status_file_name, name), alt_key)
        else:
            h_alt_key = alt_key
        if key in offline_dict:
    ...
    def check_for_red(self, enstat_d, table, empty_rows=4):
        ...
        if ball_gif.filename == self.redball_gif:
            ...
            rb_tr.append(
                HTMLgen.TD(
                    HTMLgen.Image("enstore_is_red.gif"),
                    align="LEFT"))
    ...
    def add_to_row(self, tr, val, dict, outage_d, offline_d):
        ...
        if enstore_constants.URL in dict and dict[enstore_constants.URL]:
    ...
    def make_server_table(self, dict, out_dict, offline_dict):
       ...
       # now add the migrator information
        ...
        while len(migrator_keys) > 0:
            tr = HTMLgen.TR()
            migrator_keys = self.add_data(
                migrators, migrator_keys, tr, out_dict, offline_dict)
            migrator_keys = self.add_data(
                migrators, migrator_keys, tr, out_dict, offline_dict)
            migrator_keys = self.add_data(
                migrators, migrator_keys, tr, out_dict, offline_dict)
    ...
    def body(self, enstat_d, other_d, medstat_d, alarms, nodes, outage_d,
        ...
        # add the section where nodes are reported and which servers run on
        # them
        table.append(
            HTMLgen.TR(
                HTMLgen.TD(
                    self.make_node_server_table(nodes))))
        table_spacer(table)
...
class EnStatusOnlyPage(EnSaagPage):
    ...
    def __init__(self, title="Mass Storage Production System's Status",
        ...
        sys.stdout.flush()
        EnBaseHtmlDoc.__init__(
            self,
            refresh=370,
            background="enstore_background.gif")
    ...
    def add_to_row(self, tr, val, dict, outage_d, offline_d, alt_txt=None):
        ...
        if enstore_constants.URL in dict:
            txt = HTMLgen.Href(dict[enstore_constants.URL], txt)
    ...
    def add_status_row(self, estatus, txt, table):
        ...
        if not self.check_for_red(enstat_d, table, 0):
            ...
            # enstore ball is red need to identify it better on the page as to
            # which one
            txt = HTMLgen.Font(txt, size="+2", color=BRICKRED)
            if enstore_constants.URL in enstat_d:
    ...
    def body(self):
        ...
        for stamp in stamps:
            ...
            if lib not in libraries:
                libraries[lib] = []
            if image in images:
                libraries[lib].append(
                    (sg, os.path.join(
                        plots, stamp), os.path.join(
                        plots, image)))
