...
# (1 hr) how often it checks the override file
OVERRIDE_UPDATE_INTERVAL = 3600
...
class EventRelay:
    ...
    def __repr__(self):
        ...
        return "event_relay : %s\n%s" % (
            pprint.pformat(self.__dict__), DIVIDER)
...
class InquisitorMethods(dispatching_worker.DispatchingWorker):
    ...
    # received from the event_relay for this server after server.hung_interval
    # time
    ...
    def ok_to_monitor(self, config_dict):
        return "inq_ignore" not in config_dict
    ...
    def is_server_known_down(self, server):
        ...
        if offline_d and server.name in offline_d:
            ...
            while i < 3:
                ...
                if self.event_relay.is_alive() and (not known_down):
                    if first_time:
                        ...
                        first_time = 0
                    os.system(
                        'enstore Estop %s "--just %s"' %
                        (node[0], server.name))
                    time.sleep(20)
                    os.system(
                        'enstore Estart %s "--just %s"' %
                        (node[0], server.name))

                # check if now alive - to do this, wait the equivalent of hung_interval
                # for this server and then see if an event relay message has
                # arrived
                k = 15
                j = 0
                while j < server.hung_interval:
                    time.sleep(k)
                    j = j + k
                if server.check_recent_alive(
                        self.event_relay) == monitored_server.NO_TIMEOUT:
                    Trace.log(
                        e_errors.INFO, "%s: Restarted %s" %
                        (prefix, server.name))
                    break
                else:
                    i = i + 1
                ...
                if known_down:
                    ...
                    if not server.name == enstore_constants.ALARM_SERVER:
                        Trace.alarm(
                            e_errors.ERROR, e_errors.CANTRESTART, alarm_info)
                    else:
    ...
    # this restarting may be overridden in the config file on a per server
    # basis.
        ...
        if server.no_thread():
            ...
            if server.norestart:
                if not server.did_restart_alarm:
                    ...
                    if not server.name == enstore_constants.ALARM_SERVER:
                        Trace.alarm(
                            e_errors.ERROR, e_errors.SERVERDIED, alarm_info)
                    else:
                ...
                # do not attempt to do the restart if the event relay is not
                # alive.
        ...
        try:
            for key in log_dirs.keys():
                if log_dirs[key][0:5] not in ["http:", "file:"]:
                    log_dirs[key] = "%s%s" % (
                        self.inquisitor.www_host, log_dirs[key])
    ...
    def add_new_server(self, key, config_d):
        if enstore_functions2.is_mover(key):
            ...
            # this is a generic server - make sure we are now monitoring this
            # server
            cdict = config_d[key]
            if self.ok_to_monitor(cdict) and key in self.servers_by_name:
                self.er_lock.acquire()
    ...
    def update_variables_from_config(self, config):
        ...
        for skey in self.server_d.keys():
            ...
            if new_server_config:
                if self.ok_to_monitor(new_server_config):
                    ...
                    server.hung_interval = self.inquisitor.get_hung_interval(
                        server.name)
                else:
        ...
        for skey in config.keys():
            if skey not in self.server_d:
                self.add_new_server(skey, config)

        self.www_server = config.get(enstore_constants.WWW_SERVER, {})
        # only update the following values if they were not set on the command
        # line
        key = 'alive_rcv_timeout'
        self.alive_rcv_timeout = self.get_value(
            key, self.got_from_cmdline[key])
        key = 'alive_retries'
        ...
        # get the thresholds which determine when we need an extra web page or
        # two
        self.page_thresholds = self.get_value(
            enstore_constants.PAGE_THRESHOLDS, NOVALUE)
    ...
    def make_config_html_file(self):
        ...
        if e_errors.is_timedout(d):
            ...
            return
        # we may not have gotten the dict so check for it first before writing
        # it.
        self.config_d = d.get('dump', {})
        self.update_config_page(self.config_d)

        # now update all of the internal information based on the new config
        # info.
    ...
    def suspect_vols(self, lib_man, time):
        ...
        try:
            ...
            return None
        except errno.errorcode[errno.ETIMEDOUT] as detail:
            message = "Error while getting suspect vols from %s (%s)" \
                      % (lib_man.name, str(detail))
            Trace.log(e_errors.ERROR, message, e_errors.IOERROR)
            return None
    ...
    def default_suspect_vols(self, lib_man):
        if enstore_constants.SUSPECT_VOLS not in self.serverfile.text[lib_man.name]:
            self.serverfile.text[lib_man.name][enstore_constants.SUSPECT_VOLS] = [
                "None"]
    ...
    def already_sent_mail(self, server_name, ff, node):
        ...
        if key not in self.sent_stalled_mail:
    ...
    def clear_sent_stalled_mail(self, server_name, node, ff=""):
        if ff:
            key = self.get_stalled_q_mail_key(server_name, node, ff)
            if key in self.sent_stalled_mail:
    ...
    def queue_is_stalled(self, server, node, ff, pend_num, wam_num):
        ...
        if self.lm.is_really_stalled(
                node, ff) and not self.already_sent_mail(server.name, ff, node):
    ...
    def check_for_stalled_queue(self, lib_man):
        ...
        for node in self.check_nodes:
            ...
            # the pending queue.  if there is nothing, ff_width  = 0, but we
            # don't care
            ...
            for ff in keys:
                if ff in wam_dict:
                    ...
                    self.queue_is_stalled(
                        lib_man, node, ff, pend_dict[ff][NUM_IN_Q], 0)
    ...
    def work_queue(self, lib_man, time):
        ...
        self.serverfile.output_lmqueues(
            self.lm_queues[lib_man.name], lib_man.name)
    ...
    def default_work_queue(self, lib_man):
        if enstore_constants.WORK not in self.serverfile.text[lib_man.name]:
            self.serverfile.text[lib_man.name][enstore_constants.WORK] = enstore_constants.NO_WORK
        if enstore_constants.PENDING not in self.serverfile.text[lib_man.name]:
    ...
    def active_volumes(self, lib_man, time):
        ...
        self.serverfile.output_lmactive_volumes(
            lib_man.active_volumes, lib_man.name)
        if e_errors.is_timedout(self.lm_queues[lib_man.name]):
            self.serverfile.output_etimedout(lib_man.host,
    ...
    def default_active_volumes(self, lib_man):
        if enstore_constants.ACTIVE_VOLUMES not in self.serverfile.text[lib_man.name]:
    ...
    def update_mover(self, mover):
        ...
        self.serverfile.output_moverstatus(
            self.mover_state[mover.name], mover.name)
    ...
    def update_migrator(self, migrator):
        ...
        for k in mig_state:
            if isinstance(mig_state[k], dict):
                migrator.server_status[k] = (
                    mig_state[k]['state'], mig_state[k]['internal_state'])
    ...
    def make_encp_html_file(self, now):
        ...
        encplines = sorted(
            enstore_pg.acc_encp_lines(
                self.csc, self.max_encp_lines))
        enstore_functions.inqTrace(enstore_constants.INQERRORDBG,
    ...
    def check_for_bad_writes(self, server):
        ...
        if lm.wam_queue:
            for qelem in lm.wam_queue:
                ...
                node = enstore_functions2.strip_node(
                    qelem['wrapper']['machine'][1])
                vc = qelem['vc']
                   ...
                   node in node_d_keys:
                    ...
                    ff = vc.get('file_family',
                                volume_family.extract_file_family(vc.get('volume_family',
                                                                         "")))
                    key = "%s-%s" % (node, ff)
                    if key not in bad_movers:
                        bad_movers[key] = [
                            [vc.get('file_family_width', None), node, ff]]
                    Trace.trace(enstore_constants.INQWORKDBG,
                ...
                if now - lm.time_bad >= time_bad:
                    ...
                    for action in action_l:
                        if action == enstore_constants.ALARM:
                            ...
                            Trace.alarm(
                                e_errors.ERROR,
                                "file family %s, from node %s has exceeded the file_family_width of %s %s" %
                                (ff,
                                 node,
                                 ffw,
                                 mover_info))
                        if action == enstore_constants.RED:
                            ...
                            # set the lm ball to red, this will set the enstore
                            # ball to red
    ...
    def get_server_info(self, servers_just_done, success_servers):
        ...
        for aServer in lms + servers:
            if aServer not in servers_just_done:
                ...
                # we did this one this time through already.  do not do it
                # again
                continue
            if aServer in self.server_d:
                self.er_lock.acquire()
                ...
                # if server is a library_manager, we need to get some extra
                # status
                if enstore_functions2.is_library_manager(aServer):
    ...
    def process_event_message(self, reason_called=TIMEOUT):
        ...
        if self.er_alive_event.isSet():
            ...
            while (time.time() - start_time > 10) and iterations < 3:
                ...
                servers_just_done, success_servers = self.get_server_info(
                    servers_just_done, success_servers)
    ...
    def periodic_tasks(self, reason_called=TIMEOUT):
        ...
        # just output the inquisitor alive info, if we are doing this, we are
        # alive.
    ...
    def encp_periodic_tasks(self, reason_called=TIMEOUT):
        ...
        if self.er_encp_event.isSet():
            ...
            if now - self.last_encp_update > ENCP_UPDATE_INTERVAL:
                # it has been more than update interval since the last update,
                # so do it.
    ...
    def time_for_override_mail(self, now, element):
        if element not in self.override_mail_sent:
            # mail was never sent
            return 1
    ...
    def saag_periodic_tasks(self, reason_called=TIMEOUT):
        ...
        for element in elements:
            ...
            if isinstance(elist, list):
                origin_date = elist[1]
                if now - \
                        origin_date > interval and self.time_for_override_mail(now, element):
    ...
    def do_dump(self):
        keys = sorted(self.server_d.keys())
        for skey in keys:
            server = self.server_d[skey]
            print(repr(server))
    ...
    def is_valid(self, server):
        ...
        if server in self.config_d:
            ...
            for key in keys:
                ...
                if "other_saag_links" in self.config_d and \
                   server in self.config_d["other_saag_links"]:
                    return 1
                return 0
            ...
            for key in server_l:
                ...
                if key is not None:
                    ...
                    if func == UP:
                        ...
                        override_d[key] = [
                            ticket["saagStatus"], time.time(), reason]
    ...
    def get_last_heartbeat(self, ticket):
        ...
        if isinstance(ticket["servers"], bytes):
            server_l = string.split(ticket["servers"], ',')
        else:
            server_l = ticket["servers"]
        last_time = {}
        for server in server_l:
            if server in self.server_d:
                heartbeat_time = self.server_d[server].last_heartbeat()
                # we may not be watching this server, do not return a time if
                # this is so
                if heartbeat_time:
        ...
        for func, time_data in self.interval_funcs.items():
            ...
            if now - last_called > interval:
                ...
                Trace.trace(
                    ...
                    (func.__name__,))
                self.run_in_thread(
                    func, (), after_function=self._done_cleanup, thread_name="%s.%s" %
    ...
    def process_request(self, request, client_address):
        ...
        ticket = udp_server.UDPServer.process_request(self, request,
                                                      client_address)
        Trace.trace(
            6, "inquisitor:process_request %s; %s" %
            (request, ticket,))
        if not ticket:
            return
        try:
        ...
        if c < self.max_threads:
            Trace.trace(5, "threads %s" % (c,))
            self.run_in_thread(
                function, (ticket,), after_function=self._done_cleanup)
        else:
            t = time.time()
            function(ticket)
            Trace.trace(
    ...
    def read_er_messages(self):
        ...
        while True:
            try:
                ...
                r, w, x = select.select([self.erc.sock, ], [], [
                                        self.erc.sock, ], rcv_timeout)
                # enstore_functions.inqTrace(enstore_constants.INQERTHREAD,
                #                           "thread  %s %s %s"%(r, w, x))
                ...
                if msg:
                    if msg.type == event_relay_messages.ALIVE and \
                        ...
                        self.er_lock.acquire()
                        if msg.server in self.server_d:
                ...
                # and carry on.
                Trace.handle_error()
                # we may be here because of an error while quitting, check
                # again
                if self.exit_now_event.isSet():
                    return
                #
...
class Inquisitor(InquisitorMethods, generic_server.GenericServer):
    ...
    def __init__(self, csc, html_file="", update_interval=NOVALUE, alive_rcv_to=NOVALUE,
        ...
        # these are the servers we will be monitoring, always get the
        # inquisitor first
        ...
        self.accounting_server = monitored_server.MonitoredAccountingServer(
            cdict)
        ...
        self.drivestat_server = monitored_server.MonitoredDrivestatServer(
            cdict)
        ...
        self.add_interval_func(
            self.saag_periodic_tasks,
            OVERRIDE_UPDATE_INTERVAL)
