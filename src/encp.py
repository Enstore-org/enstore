...
FAIL_1ST_DATA_SOCKET_LATER = bool(os.environ.get('FAIL_1ST_DATA_SOCKET_LATER',
                                                 False))
# Set to True after error is simulated.
fail_1st_data_socket_later_state = False
...
def encp_client_version():
    ...
    if encp_file:
        version_string = version_string + \
            " $Revision$ " + os.path.basename(encp_file)
    else:
...
def format_class_for_print(object, name):
    ...
    for var in dir(object):
        ...
        if isinstance(ret, bytes):  # if a string, make it look like one.
            ret = "'" + ret + "'"
...
def is_layer_access_name(filepath):
    ...
    access_match = re.compile(r"\.\(access\)\([0-9A-Fa-f]+\)\([1-8]\)")
        ...
        try:
            ...
            f = file_utils.open(
                os.path.join(
                    ...
                    ".(nameof)(%s)" %
                    parent_id))
            parent_name = file_utils.readline(
                f, unstable_filesystem=True).strip()
            f.close()
...
def get_enstore_pnfs_path(filepath):
    ...
    if dirname[:13] == "/pnfs/fs/usr/":
        ...
        raise EncpError(None,
                        "Unable to return enstore pnfs pathname: %s" % (
                            filepath,),
                        e_errors.WRONGPARAMETER)
...
def get_enstore_fs_path(filepath):
    ...
    if dirname[:13] == "/pnfs/fs/usr/":
        ...
        raise EncpError(None,
                        "Unable to return enstore pnfs admin pathname: %s" % (
                            filepath,),
                        e_errors.WRONGPARAMETER)
...
def get_enstore_canonical_path(filepath):
    ...
    if dirname[:len(canonical_pathbase)] == canonical_pathbase:
        ...
        raise EncpError(None,
                        "Unable to return enstore pnfs canonical pathname: %s" % (
                            filepath,),
                        e_errors.WRONGPARAMETER)
...
def stat_decode(statinfo):
    ...
    if not isinstance(statinfo, tuple):
    ...
    try:
        ...
        mode = (statinfo[stat.ST_MODE] % 0o777) | 0o100000
...
def update_modification_time(output_path, time_now=None):
    ...
    try:
        # Update the last modified time; set last access time to existing
        # value.
        file_utils.utime(output_path,
                         (file_utils.get_stat(output_path)
...
def update_last_access_time(input_path, time_now=None):
    ...
    try:
        # Update the last access time; set last modified time to existing
        # value.
        file_utils.utime(input_path,
                         (time_now, file_utils.get_stat(
...
def is_read(ticket_or_interface):
    ...
    if isinstance(ticket_or_interface, types.InstanceType):
    ...
    elif isinstance(ticket_or_interface, dict):
...
def requests_outstanding(request_list):
    ...
    for i in range(len(request_list)):
        ...
        if completion_status is None:
            if request_list[i].get('copy', None) is not None:
...
def get_request_index(request_list, request):
    ...
    for i in range(len(request_list)):
        if unique_id == request_list[i]['unique_id']:
            return i, request_list[i].get(
                'copy', 0)  # file number, copy number
...
def _get_csc_from_bfid(bfid):  # Should only be called from get_csc().
    ...
    if __csc is not None:
        test_fcc = file_clerk_client.FileClient(
            __csc, logc=__logc, alarmc=__alarmc,
            rcv_timeout=5, rcv_tries=20)
        if test_fcc.server_address is None:
...
def __get_csc(parameter=None):
    ...
    if isinstance(parameter, dict):  # If passed a ticket with bfid.
        ...
        volume = parameter
            ...
            and isinstance(parameter[0], bytes) \
            and isinstance(parameter[1], int):
        address = parameter
...
def __get_fcc(parameter=None):
    ...
    elif isinstance(parameter, dict):  # If passed a ticket with bfid.
        bfid = parameter.get('fc', {}).get("bfid",
                                           parameter.get("bfid", None))
    ...
    if bfid is None:
        if __fcc is not None:  # No bfid, but have cached fcc.
            return __fcc, None
        else:
            # Get the csc to use.
            if __csc is None:
    ...
    if __fcc is not None and match_fc(__fcc.server_address):
    ...
    # Loop through systems for the brand that matches the one we're looking
    # for.
    for server in config_servers.keys():
        try:
            ...
            if fcc_test.server_address is not None:
    ...
    __fcc = fcc
    if __fcc.server_address is not None and bfid is not None:
        # If the fcc has been initialized correctly; use it.
...
def match_fc(match_fc_addr):
    if __csc is not None and __fcc is not None and match_fc_addr is not None:
...
def match_vc(match_vc_addr):
    if __csc is not None and __fcc is not None and match_vc_addr is not None:
...
def __get_vcc(parameter=None):
    ...
    elif isinstance(parameter, dict):  # If passed a ticket with volume.
        if 'fc' in parameter:
            volume = parameter.get('fc', {}).get('external_label',
                                                 parameter.get('volume', None))
            match_fc_addr = parameter.get(
                'fc', {}).get(
                'address', (None, None))
        elif 'volume' in parameter:
            volume = parameter.get('volume', None)
            match_fc_addr = None
        elif 'external_label' in parameter:  # fc_ticket
    ...
    if volume is None:
        if __vcc is not None:  # No volume, but have cached vcc.
            return __vcc, None
        else:
            # Get the csc to use.
            if __csc is None:
    ...
    for server in config_servers.keys():
        try:
            ...
            vcc_test = volume_clerk_client.VolumeClerkClient(
                csc_test, logc=__logc, alarmc=__alarmc,
                rcv_timeout=5, rcv_tries=3)
            if vcc_test.server_address is None:
    ...
    if __vcc and __vcc.server_address is not None and volume is not None:
        ...
        for dictkey, value in csc.saved_dict.items():
            if dictkey[-len(".mover"):] == ".mover" \
               ...
               and value.get('driver', None) == "NullDriver":
                ...
                if isinstance(value['library'], list):
...
def print_data_access_layer_format(inputfile, outputfile, filesize, ticket):
    ...
    if not isinstance(mover_ticket, dict):
        Trace.log(e_errors.WARNING,
    ...
    if not isinstance(time_ticket, dict):
        Trace.log(e_errors.WARNING,
    ...
    if paranoid_crc is not None:
        crc = paranoid_crc
    elif encp_crc is not None:
        crc = encp_crc
    elif mover_crc is not None:
        crc = mover_crc
    else:
        crc = ""

    if isinstance(outputfile, list) and len(outputfile) == 1:
    ...
    if filesize is None and ticket.get('filesize', None):
...
def get_callback_addresses(encp_intf):
    ...
    # for clients running on dual stack hosts choose protocol version based on
    # configuration server IP
    my_ips = socket.getaddrinfo(socket.getfqdn(), None)
    cs_addrinfo = socket.getaddrinfo(
        __csc.get_server_address(
            __csc.server_address)[0], None)
    ...
    if hasattr(encp_intf, 'put') or hasattr(encp_intf, 'get'):
        # Get an ip and port to listen for the mover address for routing
        # purposes.
        udp_callback_addr, udp_serv = get_udp_callback_addr(encp_intf)
        # If the socket does not exist, do not continue.
        if udp_serv.server_socket is None:
            done_ticket = {'status': (e_errors.NET_ERROR,
        ...
        try:
            ...
            if e.get_bfid:
                ...
                if e_errors.is_ok(fcc_response):
                    if fcc_response['original'] is not None:
                        is_copy = True
            ...
            if not (e.override_deleted and
                ...
                # Determine if the pnfs layers and the file data are
                # consistant.
            ...
            if not (e.override_deleted and request['fc']['deleted'] != 'no') \
                ...
                if db_bfid != sfs_bfid and not is_copy:
                    ...
                    rest['bfid'] = "db_bfid differs from %s_bfid" \
                                   % (sfs_name,)
                # Origdrive has not always been recored.
                if (sfs_crc != namespace.UNKNOWN) and (db_crc is not None) \
                        and (db_crc != sfs_crc):
                ...
                if (dcache_size is not None and dcache_size != db_size):
                    rest['db_size'] = db_size
                ...
                if len(rest.keys()) > 0:
                    ...
                    Trace.alarm(e_errors.ERROR, e_errors.CONFLICT,
                                conflict_ticket)
                    raise EncpError(None,
                                    "Probable database conflict with pnfs: %s" % str(
                                        rest),
                                    e_errors.CONFLICT, request)
    ...
    for i in range(len(work_list)):
        ...
        # elif not access_check(outputlist[i], os.F_OK) and dcache:
            ...
            directory_listing = os.listdir(
                get_directory_name(outputfile_use[i]))
...
###                        file_utils.end_euid_egid(reset_ids_back = True)
                ...
                raise EncpError(error,
                                "Unable to verify output file: %s" % (
                                    msg.filename),
                                enstore_error)
...
def create_zero_length_local_files(filenames):
    ...
    for f in filenames:
        ...
        fd = file_utils.open_fd(fname, os.O_CREAT | os.O_EXCL | os.O_RDWR,
                                mode=0o666)  # raises OSError on error.
...
def get_uinfo(e=None):
    ...
    if uinfo['uid'] == 0 and uinfo['gid'] == 0 and \
       e is not None and (e.put_cache or e.migration_or_duplication):
    ...
    if uinfo['uid'] == 0 and uinfo['gid'] == 0 \
       and e is not None and (e.put_cache or e.migration_or_duplication):
        # For the case of dcache writes into enstore; we should use the
...
def open_udp_server(udp_serv, unique_id_list, encp_intf):
    ...
    while(time.time() - start_time < encp_intf.resubmit_timeout):
        ...
        if not isinstance(udp_ticket, dict):
            continue
        ...
        elif 'unique_id' in udp_ticket and \
...
def open_control_socket(listen_socket, mover_timeout):
    ...
    if fds:
        try:
            bytes_queued = callback.get_socket_read_queue_length(
                control_socket)
        ...
        if bytes_queued > 0:
            ...
            except (select.error, socket.error, e_errors.EnstoreError) as msg:
                raise EncpError(
                    msg.errno,
                    str(msg),
                    e_errors.NET_ERROR,
...
def mover_handshake_part1(listen_socket, udp_serv, work_tickets, encp_intf):
    ...
    while time.time() < start_time + encp_intf.resubmit_timeout:
        ...
        except (socket.error, select.error, EncpError) as msg:
            if msg.args[0] in [errno.EINTR, errno.EAGAIN]:
...
def receive_final_dialog_2(udp_serv, e):
    ...
    while time.time() < final_dialog_2_start_time + e.mover_timeout:
        ...
        if mover_udp_done_ticket is None:  # Something happened, keep trying.
            continue
...
def close_descriptors(*fds):
    ...
    for fd in fds:
        ...
        except (OSError, IOError, AttributeError, socket.error) as msg:
            pass
    ...
    for route in routes_to_close.keys():
        try:
            ...
            pass
        except (OSError, IOError, socket.error) as msg:
            Trace.log(e_errors.WARNING,
...
def submit_one_request_send(ticket, encp_intf):
    ...
    if e_errors.is_ok(lm_config) and \
        ...
        if lmd_name:
            lmd = library_manager_director_client.LibraryManagerDirectorClient(
                csc, lmd_name)
            Trace.message(
                TICKET_1_LEVEL, "LMD SUBMISSION TICKET\n%s:" %
            ...
            if orig_library != t["vc"]["library"]:
                ticket.update(
                    dict(
                        filter(
                            lambda i: i[0] not in ("work"),
                            t.iteritems())))

            Trace.message(
                TICKET_1_LEVEL, "LMD REPLY TICKET\n%s:" %
                (pprint.pformat(ticket),))
    ...
    try:
        ...
        try:
            ...
            pass
        raise_(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2])
    except BaseException:  # Un-anticipated errrors.
        # This is used to keep multiple encps in a migration in sync.
        # Otherwise, it should have no effect.
        try:
            start_lock.release()
    ...
    while not response_ticket:
        try:
            ...
            if not response_ticket:
                Trace.log(
                    e_errors.ERROR, "about to hang in submit_all_request_recv: %s" %
                    (response_ticket,))
...
def submit_one_request_recv(transaction_id, work_ticket, lmc, encp_intf):
    ...
    while not response_ticket:
        try:
            ...
            if msg.errno in [errno.ETIMEDOUT]:
                transaction_id = lmc.u.send_deferred(
                    work_ticket, lmc.server_address)
                count = count + 1
                if count <= max_count:
                    continue
...
def adjust_resubmit_request(ticket, encp_intf):
    ...
    if is_read(ticket) and ticket.get('method', None) is not None:
        ticket['method'] = "read_tape_start"
    # These two lines of code are for put retries to work properly.
    if is_write(ticket) and ticket.get('method', None) is not None:
    ...
    if is_write(encp_intf) and (retries or resubmits):
        ...
        if not encp_intf.output_file_family_width:
            try:
                # only for writes.
                dname = get_directory_name(ticket['outfile'])
...
def open_local_file(work_ticket, tinfo, e):
    ...
    if is_read(e) and current_size != 0:
...
def transfer_file(input_file_obj, output_file_obj, control_socket,
    ...
    if not e_errors.is_retriable(done_ticket) and \
       not e_errors.is_ok(done_ticket):
        rtn_ticket = combine_dict(
            done_ticket, {
                'exfer': EXfer_ticket}, request)
    elif not e_errors.is_retriable(EXfer_ticket) and \
            not e_errors.is_ok(EXfer_ticket):
        rtn_ticket = combine_dict(
            {'exfer': EXfer_ticket}, done_ticket, request)
...
def check_crc_readback(done_ticket, fd):
    ...
    if fd and (isinstance(fd, int)) and \
...
def check_crc_layer2(done_ticket, encp_crc_1_seeded):
    ...
    if dcache_crc_long is not None and dcache_crc_long != encp_crc_1_seeded:
    ...
    if dcache_size_long is not None and dcache_size_long != encp_size_long:
...
def verify_file_size(ticket, encp_intf=None):
    ...
    try:
        full_stat = file_utils.get_stat(
            ticket['wrapper'].get('fullname', None))
    ...
    try:
        # Handle making sure the local inode did not change during the
        # transfer.
        ...
        # Handle making sure the pnfs file inode not change during the
        # transfer.
                ...
                if is_write(ticket):
                    ...
                    if (encp_intf.get_bfid or encp_intf.get_bfids) and \
                       encp_intf.skip_pnfs:
                        ...
                        # this is needed in case if time is appended by ms
                        update_t = ticket['fc']['update'].split(".")[0]
                        ...
                        in_stat_info = (ticket['wrapper']['mode'],
                                        ...
                                        ticket['wrapper']['uid'],
                                        ticket['wrapper']['gid'],
                                        # when merging head as SFA notices
                                        # incosistency between filesize and
                                        # file_size, this should take care of
                                        # it
                                        ticket.get(
                            'filesize', ticket.get(
                                'file_size', 0)),
...
def handle_retries(request_list, request_dictionary, error_dictionary,
    ...
    if USE_FIRST_REQUEST and 'infile' not in request_dictionary and \
            'outfile' not in request_dictionary:
    ...
    # If there is a control socket open and there is data to read, then read
    # it.
    ...
    if local_filename:
        ...
        try:
            ...
            if original_inode is not None and \
    ...
    if is_write(encp_intf) and isinstance(pnfs_filename, bytes) \
    ...
    if max_submits is not None and resubmits >= max_submits \
    ...
    # If we can an error, that does not result
    ...
    if request_dictionary.get('unique_id', None) is None \
            and e_errors.is_retriable(status) \
            and status[0] != e_errors.RESUBMITTING:
        Trace.message(ERROR_LEVEL,
    ...
    # actuall error, not just a timeout.  Also, increase the retry count by 1.
        ...
        if 'retried_unique_ids' not in request_dictionary:
    ...
    if e_errors.is_ok(done_ticket['status'][0]):
        ...
        try:
            if transfer_time == 0.0:
                ...
                acc_transfer_rate = int(
                    done_ticket['file_size'] / transfer_time)
        except OverflowError:
            acc_transfer_rate = -1
...
def calculate_final_statistics(bytes, number_of_files, exit_status, tinfo):
    ...
    for key, value in tinfo.iteritems():
        ...
        if index != -1:
            id = key[:index]
            statistics['transfer_size'] = statistics.get(
                'transfer_size', 0) + value
    ...
    statistics['MB_per_S_overall'] = statistics['transfer_size'] / \
        statistics['overall_time'] if statistics.get('overall_time') else 0
    statistics['MB_per_S_transfer'] = statistics['transfer_size'] / \
        statistics['transfer_time'] if statistics.get('transfer_time') else 0
    statistics['MB_per_S_network'] = statistics['transfer_size'] / \
        statistics['network_time'] if statistics.get('network_time') else 0
    statistics['MB_per_S_drive'] = statistics['transfer_size'] / \
        statistics['drive_time'] if statistics.get('drive_time') else 0
    statistics['MB_per_S_disk'] = statistics['transfer_size'] / \
        statistics['disk_time'] if statistics.get('disk_time') else 0
...
def set_sfs_settings(ticket, intf_encp):
    ...
    try:
        ...
        fc_ticket["fc"]['original_library'] = ticket.get(
            'original_library', None)
        fc_ticket["fc"]['file_family_width'] = ticket['vc'].get(
            'file_family_width', 1)
    ...
    if not ticket.get('copy', None):  # Don't set size if copy.
        # Start time of setting the filesize.
...
def create_write_requests(callback_addr, udp_callback_addr, e, tinfo):
    ...
    if use_copies > 0:
        ...
        if e.output_library:
            ...
            libraries = t.get_library(
                os.path.dirname(
                    work_ticket['outfile'])).split(",")
        ...
        for n_copy in range(1, use_copies + 1):
            ...
            for work_ticket in request_list:
                ...
                # this encp.
                if start_copy_number is not None:
                ...
                # Make the transfer id unique, but also keep the original
                # around.
...
def create_write_request(work_ticket, file_number,
    ...
    if ifullname in ["/dev/zero", "/dev/random", "/dev/urandom"]:
        bound_limits = sorted([long(os.environ.get('ENSTORE_RANDOM_LB', 0)),
...
def submit_write_request(work_ticket, encp_intf):
    ...
    while encp_intf.max_retry is None or \
...
def write_hsm_file(work_ticket, control_socket, data_path_socket,
    ...
    if e_errors.is_non_retriable(
            done_ticket.get('status', (e_errors.OK, None))):
        return combine_dict(result_dict, work_ticket)
...
def prepare_write_to_hsm(tinfo, e):
    ...
    try:
        ...
        raise_(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2])
            ...
            socket.error, select.error) as msg:
        Trace.handle_error(severity=99)
        if isinstance(msg, EncpError):
            e_ticket = msg.ticket
            if e_ticket.get('status', None) is None:
    ...
    Trace.message(TRANSFER_LEVEL, message)
    for i in range(len(request_list)):
        if e.put_cache or request_list[i].get('copy', None) is not None:
            ...
            except (OSError, IOError, EncpError) as msg:
                if msg.args[0] == getattr(errno, str("EFSCORRUPTED"), None) \
...
def write_to_hsm(e, tinfo):
    ...
    while requests_outstanding(request_list):
        ...
        while requests_outstanding([work_ticket, original_ticket]):
            ...
            # happens.
            done_ticket = combine_dict(
                result_dict, work_ticket, request_ticket)
            ...
            # the error occured.
            exfer_ticket = done_ticket.get('exfer', {'bytes_transfered': 0})
    ...
    for vol in vols:
        ...
        if request['outfile'] not in ["/dev/null", "/dev/zero",
            ...
            for quota in fs_quotas:
                ...
                if quota[EXfer.BLOCK_HARD_LIMIT] > 0 \
                ...
                if quota[EXfer.FILE_HARD_LIMIT] > 0 \
...
def get_file_clerk_info(bfid_or_ticket, encp_intf=None):
    ...
    if fc_ticket is None or not e_errors.is_ok(fc_ticket):
        fc_status = fc_ticket.get('status', (e_errors.EPROTO, None))
        fc_error_ticket = {'fc': fc_ticket}

        if encp_intf is not None and encp_intf.check \
...
def get_volume_clerk_info(volume_or_ticket, encp_intf=None):
    ...
    if vc_ticket is None:
        if isinstance(volume_or_ticket, bytes):
            volume_name = volume_or_ticket
        elif isinstance(volume_or_ticket, dict):
    ...
    if inhibit in (e_errors.NOACCESS, e_errors.NOTALLOWED):
        ...
        raise EncpError(None,
                        "Volume %s is marked %s." % (
                            vc_ticket['external_label'], inhibit),
...
def create_read_requests(callback_addr, udp_callback_addr, tinfo, e):
    ...
    for i in range(number_of_files):
        ...
        if e.volume and hasattr(e, "get") and e.list:
            ...
            use_infile = request['fc']['pnfs_name0']
        elif e.volume:
            try:
        ...
        try:
            # Moved this inside the loop to pass in the filename to
            # Pnfs/Chimera
...
def create_read_request(request, file_number,
    ...
    if e.volume:
        ...
        if deleted_status is not None and deleted_status != "no":
        ...
        if ifullname is None and sfsid is not None:
            try:
        ...
        if fc_reply.get('pnfs_name0', None) is None:
        ...
        if e.shortcut and e.override_path:
            ...
            try:
                ...
                if msg.args[0] not in [errno.ENODEV] \
                        or not isinstance(ifullname_list, list) \
    ...
    if not isinstance(file_size, type(None)):
...
def submit_read_requests(requests, encp_intf):
    ...
    for req in requests:
        ...
        while req.get('completion_status', None) is None:
            try:
...
def stall_read_transfer(data_path_socket, control_socket, work_ticket, e):
    ...
    if work_ticket['file_size'] == 0:
        ...
        target_size = 0
    elif work_ticket['file_size'] is None:
...
def read_hsm_file(request_ticket, control_socket, data_path_socket,
    ...
    try:
        ...
        done_ticket['status'] = (
            e_errors.UNKNOWN, "transfer_file(): (%s, %s)" %
            (str(exc), str(msg)))
...
def prepare_read_from_hsm(tinfo, e):
    ...
    try:
        ...
        raise_(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2])
    except (OSError, IOError, AttributeError, ValueError, EncpError) as msg:
        Trace.handle_error(severity=99)
        if isinstance(msg, EncpError):
            e_ticket = msg.ticket
            e_ticket['status'] = (msg.type, str(msg))
                ...
                if not should_skip_deleted:
                    ...
                    if os.getuid() == 0:
                        ...
                        if e.get_bfid and e.skip_pnfs:
                            ...
                            in_file_stats = get_stat(
                                requests_per_vol[vol][i]['infile'])
    ...
    for vol in vols:
        ...
        if submitted > 0:
            while requests_outstanding(request_list):
                ...
                # If USE_NEW_EVENT_LOOP is true, we need these ids.
                transaction_id_list = result_dict.get(
                    'transaction_id_list', [])
            ...
            exit_status = 1
            # If all submits fail (i.e using an old encp), this avoids
            # crashing.
...
class EncpInterface(option.Interface):
    ...
    def __str__(self):
        ...
        the_list = sorted(the_list + [('input', {}), ('output', {}),
                                      ('intype', {}), ('outtype', {}),
                                      ])
    ...
    def parse_options(self):
        ...
        # processed correctly.
        if not isinstance(self.max_retry, bytes):
        ...
        if not isinstance(self.max_resubmit, bytes):
            pass  # Skip the int and None cases.  Only consider string cases.
        ...
        if self.copies is not None and self.copies < 0:
        ...
        for i in [1, 2, 3]:
            try:
                this_host = socket.getaddrinfo(
                    socket.getfqdn(), None)[0][4][0], [], []
            except (socket.error, socket.herror, socket.gaierror):
                time.sleep()
        else:
            try:
...
def final_say(intf, done_ticket):
    ...
    try:
        ...
        status = done_ticket.get(
            'status', (e_errors.UNKNOWN, e_errors.UNKNOWN))
