...
class WebServer:
    def __init__(self, timeout=1, retry=0):
        ...
        if self.system_name:
            ...
            try:
                ...
                print(
                    "Failed to connect to config server, using configuration file %s" %
                    (configfile,))
                try:
                    f = open(configfile, 'r')
                ...
                for item in ret.items():
                    if socket.getfqdn(
                            item[1][0]) == socket.getfqdn(def_addr[0]):
                        match = True
    ...
    def read_httpd_conf(self):
        ...
        if not os.path.exists("%s%s" % (self.config_file, SUFFIX)):
            if self.move_httpd_conf(self.config_file, "%s%s" %
                                    (self.config_file, SUFFIX)):
                return 1
                        ...
                        if indx == 0 and key == line.split()[0].strip():
                            if key == "CustomLog":
                                ...
                                for k in self.server_dict[key].keys():
                                    ...
                                    if index != -1:
                                        custom_txt = key + " " + \
                                            str(self.server_dict[key]
                                                [k]) + " " + k + "\n"
                                        break
                            elif key == "ScriptAlias":
                                for k in self.server_dict[key].keys():
                                    txt = key + " " + \
                                        str(self.server_dict[key]['fake']) + " " + \
                                        str(self.server_dict[key]
                                            ['real']) + "\n"
                                    txt = txt + "<Directory \"" + \
                                        str(self.server_dict[key]
                                    ...
                                    for e in [
                                            "PYTHONINC", "PYTHONPATH", "PYTHONLIB", "PATH"]:
                                        txt = txt + "SetEnv " + e + \
                                            " " + os.getenv(e) + "\n"
                                    txt = txt + "</Directory>\n"
                                    break
    ...
    def get_document_root(self):
        if not self.is_ok:
            return None
        if 'DocumentRoot' in self.server_dict:
    ...
    def get_pid_file(self):
        if not self.is_ok:
            return None
        if 'PidFile' in self.server_dict:
    ...
    def get_server_name(self):
        if not self.is_ok:
            return None
        if 'ServerName' in self.server_dict:
    ...
    def get_server_host(self):
        if not self.is_ok:
            return None
        if 'ServerHost' in self.server_dict:
    ...
    def get_server_root(self):
        if not self.is_ok:
            return None
        if 'ServerRoot' in self.server_dict:
    ...
    def get_error_log(self):
        if not self.is_ok:
            return None
        if 'ErrorLog' in self.server_dict:
    ...
    def get_custom_log(self, name):
        if not self.is_ok:
            return None
        if 'CustomLog' in self.server_dict:
    ...
    def get_cgi_directory(self):
        if not self.is_ok:
            return None
        if 'ScriptAlias' in self.server_dict:
...
def install():
    ...
    try:
        ...
        if server.this_host_name != server.get_server_host():
            print(
                "You are installing RPM on host which is not intended to run apache server ")
            return 0
        ...
        print(
            "exists", os.path.exists(
                os.path.join(
                    server.get_document_root(), "enstore")), os.path.join(
                server.get_document_root(), "enstore"))
...
def erase():
    server = WebServer()
    rc = 0
    server.move_httpd_conf(
        "%s%s" %
        (server.config_file, SUFFIX), server.config_file)
...
if __name__ == "__main__":
    try:
        do_erase = False
        do_install = False
        opts, args = getopt.getopt(sys.argv[1:], "hs:is:es:", [
                                   "help", "install", "erase"])
        for o, a in opts:
            if o in ("-h", "--help"):
                usage(sys.argv[0])
