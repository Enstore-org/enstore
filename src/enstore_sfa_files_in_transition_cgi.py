...
def get_environment():
    ...
    if os.environ.get('ENSTORE_CONFIG_HOST', None) is None:
...
def main():
    ...
    if 'status' in result and result['status'][0] == e_errors.OK:
        ...
        for pool in keys:  # pools are keys of the dictionary
            if pool == 'migration_pool' and len(
                ...
                for k in result['pools'][pool].keys():
                    ...
                    time_qd = time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(time_qd))
                    times[k] = time_qd
                    if t != "CACHE_WRITTEN":
                        continue
                    if policy not in files_pool_policy:
                        files_pool_policy[policy] = {}
                    if pool not in files_pool_policy[policy]:
                        files_pool_policy[policy][pool] = []
                    if policy not in files:
                        #files[policy] = {"migration_pool" : [] , "cache_written" : [] }
                        files[policy] = {}
                    if pool not in files[policy]:
                        files[policy][pool] = {}
                    if k not in files[policy][pool]:
                        files[policy][pool][k] = []
                ...
                for k, value in result['pools'][pool].iteritems():
                    ...
                    if pool not in files[policy]:
                        files[policy][pool] = {}
                    if value.list_id not in files[policy][pool]:
                        files[policy][pool][value.list_id] = []
                    times[value.list_id] = time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(value.creation_time))
    ...
    if not files:
        ...
        for policy, poollists in files.iteritems():
            ...
            for pool, list in poollists.iteritems():
                ...
                Q1 = "select count(*), sum(size)/1024. as total from file where bfid in ('%s')" % (
                    string.join(files_in_pool_policies, "','"))
                res = select(dbinfo, Q1)
                count = int(res[0]["count"])
                size = long(res[0]["total"])

                print(
                    "<h2>pool=%s, total=%d, size=%d (kB), #of lists=%d</h2>" %
                    (pool, count, size, len(list)))
                for key, value in list.iteritems():
                    ...
                    size = long(res[0]["total"])
                    print(
                        "list id=%s, total=%d,size=%d (kB), time_qd=%s</h2>" %
                        (key, count, size, times[key]))
                    print("<pre>")
                    print(
                        "cache_mod_time storage_group file_family volume location_cookie bfid size crc pnfs_id pnfs_path archive_status")
                    nfiles = len(value)
                    start = 0
                    end = start
                    while (end < nfiles):
