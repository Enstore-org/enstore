...
def ftt_mode_select_6_bytes(driver,
    ...
    Trace.trace(
        32, "ftt_mode_select_6_bytes: Sending Mode Select: %s" %
        (sprint_list(cdb_mode_select),))
    Trace.trace(
        32, "ftt_mode_select_6_bytes: Mode select data: %s" %
        (sprint_list(data_block),))
...
def t10000c_amc(driver,
    ...
    data[2] = 0x10
    data[3] = 0x00
    data[MS_HEADER_DATA_LENGTH +
         OFFSET_TO_AMC] = data[MS_HEADER_DATA_LENGTH +
                               OFFSET_TO_AMC] & 0xfe | amc
    ...
    if sense_data[MS_HEADER_DATA_LENGTH +
                  OFFSET_TO_AMC] != data[MS_HEADER_DATA_LENGTH + OFFSET_TO_AMC]:
...
def set_lfa_fsa(driver,
    ...
    data[3] = 0x00
    data[MS_HEADER_DATA_LENGTH +
         OFFSET_TO_DFSA] = data[MS_HEADER_DATA_LENGTH +
                                OFFSET_TO_DFSA] & 0xc0 | byte8
    ...
    if sense_data[MS_HEADER_DATA_LENGTH +
                  OFFSET_TO_DFSA] != data[MS_HEADER_DATA_LENGTH + OFFSET_TO_DFSA]:
...
def t10000c_dce(driver,     # ftt driver
    ...
    data[3] = 0x00
    data[MS_HEADER_DATA_LENGTH +
         OFFSET_TO_DCE] = data[MS_HEADER_DATA_LENGTH +
                               OFFSET_TO_DCE] & 0x40 | dce
    ...
    if sense_data[MS_HEADER_DATA_LENGTH +
                  OFFSET_TO_DCE] & 0xff != data[MS_HEADER_DATA_LENGTH + OFFSET_TO_DCE]:
...
def t10000c_sdca(driver,
    ...
    if sense_data[MS_HEADER_DATA_LENGTH +
                  OFFSET_TO_SDCA] != data[MS_HEADER_DATA_LENGTH + OFFSET_TO_SDCA]:
        Trace.log(e_errors.ERROR, "AMC setting failed: out %0x in %0x" %
...
def ftt_scsi_verify(driver,
    ...
    Trace.trace(
        32, "ftt_scsi_verify: Sending SCSI Verify Command: %s" %
        (sprint_list(cdb_verify),))
...
def ftt_request_sense(driver,
    ...
    Trace.trace(
        32, "ftt_request_sense: Sending Request Sense: %s" %
        (sprint_list(cdb_request_sense),))
...
def page_25_test(driver):
    ...
    res = raw_input(
        "enter 1 to set AMC, 2 to set FSA or LFA, or just press Enter: ")
    if res:
        res = int(res)
    if res == 1:
