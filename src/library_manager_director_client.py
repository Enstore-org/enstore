...
class LibraryManagerDirectorClient(generic_client.GenericClient):
    def __init__(self, csc, server_name="", flags=0, logc=None, alarmc=None,
        ...
        # name - name of the Library Manager Director in configuration
        # dictionary
...
class LibraryManagerDirectorClientInterface(
...
def unit_test(intf):
    ...
    if lm:
        # this is the keyword, that must be defined in LM configuration to
        # refer to library manager director
        lmdname = lm.get('use_LMD', None)
        if lmdname:
