...
def ago(year, mon, day):
    ...
    return time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime(
        time.mktime((t[0] - year, t[1] - mon, t[2] - day, 0, 0, 0, 0, 0, 1))))
...
def last_weekday(d):
    ...
    if tt <= 0:
        tt = tt + 7
    return time.strftime("%Y-%m-%d:%H:%M:%S", time.localtime(
        time.mktime((t[0], t[1], t[2] - tt, 0, 0, 0, 0, 0, 1))))
...
class EcronData:
    ...
    def get_all_names_and_nodes(self):
        if self.all_names_and_nodes is None:
    ...
    def get_result(self, name, node, start=None):
        if start:
            q = "(select start as time, 10 from event where name = '%s' and split_part(node,'.',1) = '%s' and start >= '%s' and comment is null) union (select finish as time, status from event where name = '%s' and split_part(node,'.',1) = '%s' and not finish is null and start >= '%s' and comment is null) order by time;" % (
                name, node.split('.')[0], start, name, node.split('.')[0], start)
        else:
            q = "(select start as time, 10 from event where name = '%s' and split_part(node,'.',1) and comment is null) union (select finish as time, status from event where name = '%s' and split_part(node,'.',1) and not finish is null and comment is null) order by time;" % (
