...
def swap_volume(vol):
    ...
    for i in range(len(res)):
        ...
        q2 = "select label,storage_group,file_family,wrapper from file,volume,file_copies_map where file.volume = volume.id and file.bfid = file_copies_map.alt_bfid and file_copies_map.bfid = '%s'" % (
            bfid,)
    ...
    if e_errors.is_ok(vol_info):
        ...
        if is_normal_file_family(file_family):
            new_file_family = get_copy_file_family(
                file_family, highest_copy_count)
