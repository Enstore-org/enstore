...
# Not equals to (==) safe.
migrate_chimera.MIGRATION_FILE_FAMILY_KEY = "_copy_%s"
...
migrate_chimera.LOG_FILE = migrate_chimera.LOG_FILE.replace(
...
def search_order_duplication(src_bfid, src_file_record, dst_bfid,
    ...
    # arguments is_it_copied and is_it_swapped used by migrate_chimera.py
    # version.
...
def migration_file_family_duplication(
        bfid, ff, fcc, intf, deleted=migrate_chimera.NO):
    ...
    if deleted == migrate_chimera.YES:
        return migrate_chimera.DELETED_FILE_FAMILY + \
            migrate_chimera.MIGRATION_FILE_FAMILY_KEY % (count,)
    else:
        if intf.file_family:
            return intf.file_family + \
                migrate_chimera.MIGRATION_FILE_FAMILY_KEY % (count,)
        else:
...
def is_migration_file_family_duplication(ff):
    if re.search("_copy_[0-9]*", ff) is None:
        return False
...
def log_uncopied_duplication(src_bfid, dst_bfid, fcc, db):
    rtn_val = migrate_chimera.log_uncopied_migration(
        src_bfid, dst_bfid, fcc, db)
    if rtn_val:
        return rtn_val  # Error.
...
def _duplicate_metadata(my_task, job, fcc, db):
    ...
    # SFA: if duplicated file is a package and not empty, create children for
    # new package
    if src_is_a_package and package_files_count > 0:
        rc = migrate_chimera.dup_packaged_meta_one(
            my_task, src_file_record, dst_bfid, fcc)
        if rc:
            return "failed to create children bfids for package %s" % (
                dst_bfid,)
...
if __name__ == '__main__':
    ...
    intf_of_migrate = migrate_chimera.MigrateInterface(
        sys.argv, 0)  # zero means admin
