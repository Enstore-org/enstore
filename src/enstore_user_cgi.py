...
def go():
    ...
    try:
        ...
        if main_cbox_key in form:
            main_cbox = form[main_cbox_key]
            if isinstance(main_cbox, type([])):
        ...
        if main_opt_key in form:
            main_opt = form[main_opt_key].value
        else:
            # the user did not select a command
            print(
                "ERROR: Please select an option (and value) for this command (e.g. bfid).")
            raise SystemExit
