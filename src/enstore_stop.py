...
def find_servers_by_type(csc, type):
    ...
    try:
        if csc:
            ...
            if e_errors.is_ok(config_dict) and "dump" in config_dict:
...
def kill_process(pid):
    ...
    if detect_process(pid):
        ...
        try:
            ...
            # If the SIGTERM succeded, give the kernel a moment...
            time.sleep(1)
        except OSError as msg:
            if hasattr(msg, "errno") and msg.errno != errno.EPERM:
                return 1
...
def stop_server(gc, servername):
    ...
    if servername.find("mover"):
        ...
        try:
            rtn1 = u.send({'work': "status"},
                          gc.server_address, SEND_TO, SEND_TM)
        except (socket.error, select.error, e_errors.EnstoreError) as msg:
            if msg.errno == errno.ETIMEDOUT:
...
def check_event_relay(csc):
    ...
    if csc is None or not e_errors.is_ok(info):
        info = enstore_functions.get_dict_from_config_file(name, None)

    # If we still do not have the necessary info, skip it.
    if info is None or not e_errors.is_ok(info):
        return
    # If the process is running on this host continue, if not running on
    # this host return.
...
def do_work(intf):
    ...
    proxy_servers = find_servers_by_type(
        csc, enstore_constants.UDP_PROXY_SERVER)
