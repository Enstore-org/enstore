...
while True:
    ...
    if l:
        ...
        # 23:59:48 stkenmvr205a.fnal.gov 006277 root I LTO4_105MV  drive stats after write. Tape VOY318 position 180 block 0 block_size 0 bloc_loc 2835120 tot_blocks 762939000 BOT 0 read_err 0 write_err 0 bytes 2119911936 block_write_tot 19.2541582584 tape_rate 110101511.972 Thread tape_thread
        t = a[0]
        mv = a[5]
        vol = a[12]
        pos = int(a[14])
        ...
        tp = (t, vol, pos, read_err, write_err, bytes_written, write_time, tape_rate)
        if not (wr_stats.has_key(mv)):
            wr_stats[mv] = {"write_count": 0L}
        if not (wr_stats[mv].has_key(vol)):
            wr_stats[mv][vol] = []
...
for mv in wr_stats.keys():
    ...
    for vol in  wr_stats[mv].keys():
        if vol != "write_count":
            ...
            print "Vol. %s writes %s write_errors %s"%(vol, vol_writes, vol_wr_errors)
