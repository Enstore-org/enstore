...
def paranoid_db(system):
    ...
    cmd = 'rsh ' + backup_host + ' ls -1td ' + \
        os.path.join(database_backup_path, database_pattern)
    ...
    paranoid_db_path = os.path.join(
        os.path.join(
            paranoid_backup_path,
            system),
        'db')
    ...
    for i in database_file:
        cmd = 'rcp ' + backup_host + ':' + \
            os.path.join(dbase_path, i) + ' ' + paranoid_db_path
        print(cmd)
        if do_it:
            os.system(cmd)
    if do_it:
...
def paranoid_pnfs(system):
    ...
    for i in result:
        ...
        if len(token) == 3 and token[2] == 'Z':
            ...
            if file not in files:
                files[file] = i
    paranoid_pnfs_path = os.path.join(
        os.path.join(
            paranoid_backup_path,
            system),
        'pnfs')
    ...
    for i in files.keys():
        cmd = 'rcp %s:%s %s' % (backup_host, os.path.join(
            pnfs_backup_path, files[i]), paranoid_pnfs_path)
        print(cmd)
        if do_it:
            os.system(cmd)
