...
class GenericDriver:
        ...
        def __setattr__(self, attr, val):
            if attr=='cur_loc_cookie':
                ...
                Trace.log(e_errors.INFO,
                          "Setting cur_loc to %s from %s" %
                          (val,tb.tb_frame.f_back.f_code.co_name))
	...
	# middle of a tape.
        ...
        ## cgw says - what kind of tape drive will let you write to the middle
        ## of tape anyway?  that comment above makes no sense to me.
	...
	if loc_cookie==None or loc_cookie=='None' or loc_cookie=='none':
                                         loc = 0
	elif type(loc_cookie)==type(""): loc = atoi( loc_cookie )
...
class  FTTDriver(GenericDriver) :
    ...
    def loc2int( self, loc ):
        if loc==None or loc=='None' or loc=='none':
            ...
            xx = re.split( '_', loc )
            part, block_loc, filenum = ( atoi(xx[0]),
                                         atoi(xx[1]),
                                         atoi(xx[2]) )
	...
	# bound again as the next volume, the position could be checked to
	...
	if statistics['remain_tape'] == None:
	    ...
	    rb = atoi(statistics['remain_tape'])*1024L
	...
	else:                         rb = atoi(ss['remain_tape'])*1024L
	...
	if ss['bloc_loc'] != None: b = atoi(ss['bloc_loc'])
	...
	if block_loc and 0:
		...
		FTT.locate( block_loc )
		ss = FTT.get_stats()
		if atoi(ss['bloc_loc']) == block_loc: break
		xx = xx - 1
		pass
		...
		if ss['bloc_loc'] != None:
		    ...
		    # IS BOGUS.
		    if block_loc and block_loc != atoi(ss['bloc_loc']):
                        if debug_paranoia: print block_loc, "!=", atoi(ss['bloc_loc'])
			raise SeekError
		    block_loc = atoi(ss['bloc_loc'])
		    pass
		else:
		    Trace.log( e_errors.ERROR, "FTT.get_stats - no block_loc" )
		    pass
...
class  DelayDriver(RawDiskDriver) :
	...
	if loc_cookie==None or loc_cookie=='None' or loc_cookie=='none':
	      loc = 0
	else: loc = atoi( loc_cookie )
	bytesskipped = abs( atoi(self.cur_loc_cookie) - loc );
...
class NullDriver( GenericDriver):
    ...
    def seek(self, loc_cookie):
        loc = self.loc2int(loc_cookie)
        self.loc=loc

    def tell(self):
        self.cur_loc_cookie = self.int2loc(self.loc)
        return self.cur_loc_cookie
    
    def loc_compare(self,loc1,loc2):
        return 1 #XXX HACK to make mover happy
