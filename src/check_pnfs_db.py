...
def change_file_name(file):
    ...
    # this is not always true, depends on the name of the mount point
    new = ['/pnfs', 'fs', 'usr']
...
def check_volume(label, selected_library=""):
    ...
    for line in lines:
        ...
        if len(field) == 6:
            ...
            Print(
                "Error EFL: File record field length is not 6 (%d), record '%s'\n" %
    ...
    for entry in list:
        ...
        if (len(layer1) != 1):
            err = err + 1
            Print(
                "Error L1: wrong L1 layer length= %d, layer=%s " %
                (len(layer1), layer1), ferr)
            Print(
                "invt bf_id %s  file '%s'\n" %
        ...
        if bfid_l1 != bfid_inv:
            ...
            if(OK_rdL4 and len(layer) > 8 and (bfid_l1 == layer[8][:-1])):
                same = " (same as in L4)"
            msg = "Wrong BF_ID pnfs: %s -> inv: %s file '%s' del='%s'%s." % (
                bfid_l1, bfid_inv, entry["fname"], entry["del_flag"], same)
            Print("%s: %s\n" % (err_kind, msg), ferr)
        ...
        if (len(layer) != 9) and (len(layer) != 10):
            err = err + 1
            Print(
                "Error L4: wrong L4 layer length= %d, layer=%s " %
                (len(layer), layer), ferr)
            Print(
                "invt bf_id %s  file '%s'\n" %
                (entry["bf_id"], entry["fname"]), ferr)
            continue
        ...
        if layer[8][:-1] != entry["bf_id"]:
            msg = "Wrong BF_ID pnfs: %s -> inv: %s file '%s' del='%s'." % (
                bfid_l1, bfid_inv, entry["fname"], entry["del_flag"])
            if layer[0][:-1] == label:
                #err = err +1
                #err_kind = "Error L4"
                warn = warn + 1
...
if __name__ == "__main__":
    ...
    if args:
        ...
        volume_list = generate_volume_list(
            "enstore/tape_inventory/VOLUMES_DEFINED", library)
    t1 = time.time()
