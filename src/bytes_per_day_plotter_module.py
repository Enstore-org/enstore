...
class BytesPerDayPlotterModule(enstore_plotter_module.EnstorePlotterModule):
    ...
    def write_plot_file(self, plot_filename, data_filenames,
        ...
        EMPTY_AGGREGATES = {'sum' : 0L, 'average' : 0L, 'count' : 0L}

        #Make these lists for processing.
        if type(keys) != types.ListType:
            keys = [keys]
        if type(data_filenames) != types.ListType:
        ...
        for i in range(len(data_filenames)):
            ...
            if writes_only and total_only:
                ...
                rl = '"%s" using 1:3 t "reads" with impulses linewidth 18, ' % (data_filename,)
                wl = '"%s" using 1:4 t "writes" with impulses linewidth 20 ' % (data_filename,)
    ...
    def _fill(self, sql_cmd, adb):
        ...
        for row in adb_res:
            ...
            if not self.pts_files_dict.has_key(row[1]):
            ...
            if not self.store_dict.has_key(row[1]):
                self.store_dict[row[1]] = {}
            if not self.store_dict[row[1]].has_key(row[0]):
        ...
        for key in self.store_dict.keys():
            ...
            for day in self.store_dict[key].keys():
                read_sum = self.store_dict[key][day].get('r', 0L)
                write_sum = self.store_dict[key][day].get('w', 0L)
    ...
    def _fill2(self, sql_cmd, adb):
        ...
        for row in adb_res:
            ...
            if not self.total_values.has_key(row[0]):
    ...
    def plot(self):
        ...
        for key in self.pts_files_dict.keys():
            ...
            if self.pts_files_dict.has_key(key):
