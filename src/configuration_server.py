...
class ConfigurationDict:
    ...
    def get_copy_level(self):
        self.member_lock.acquire()
        try:
            copy_level = self.__get_config_value(
                self.do_copies, self.do_copies)
        except BaseException:
            self.member_lock.release()
            raise_(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2])
        self.member_lock.release()
    ...
    def _get_domains(self):
        ...
        self.member_lock.acquire()
        if getattr(self, "cached_domains", None) is None:
            should_set_now = True
        else:
        ...
        if should_set_now:
            ...
            if isinstance(domains, dict):
                ...
                try:
                    cached_domains['domains']['valid_domains'].append(
                        hostaddr.getdomainaddr())
                except KeyError:
                    pass
            ...
            Trace.trace(
                25, "_get_domains: extract time: %f" %
                (time.time() - t0,))
...
class ConfigurationServer(ConfigurationDict, dispatching_worker.DispatchingWorker,
    ...
    def _reinit(self):
        Trace.log(
            e_errors.INFO,
            "Configuration server reloading configuration")
        configfile = enstore_functions2.default_file()
    ...
    def invoke_function(self, function, args=(), after_function=None):
        ...
        if function.__name__ in ['dump', 'dump2']:
    ...
    def lookup(self, ticket):
        ...
        try:
            ...
            ticket['status'] = (e_errors.KEYERROR,
                                ...
                                + repr(lookup))
        # Determine on what IP type to communicate depending on the address
        # type of the client.
        server_ipv4 = None
        if (len(ticket['r_a'][0][0].split('.')) == 4 and
            ...
            hostinfo = socket.getaddrinfo(
                ticket[lookup]['host'], socket.AF_INET)
            for e in hostinfo:
    ...
    def get_keys(self, ticket):
        ...
        skeys = sorted(self.get_config_keys())
    ...
    def __make_dump(self, ticket):
        ...
        my_address_family = socket.getaddrinfo(
            socket.gethostname(), None)[0][0]
        if (len(ticket['r_a'][0][0].split('.'))) == 4 and (
                my_address_family == socket.AF_INET6):
            # convert all 'hostip' to IPV4
            for key in reply['dump']:
                if 'host' in reply['dump'][key]:
                    server_ipv4 = None
                    hostinfo = socket.getaddrinfo(
                        reply['dump'][key]['host'], socket.AF_INET)
                    for e in hostinfo:
                        if e[0] == socket.AF_INET:
                            # see python socket module documentation.
                            server_ipv4 = e[4][0]
    ...
    def dump(self, ticket):
        ...
        try:
            ...
            if r:
                Trace.log(
                    e_errors.ERROR, "Error calling write_tcp_obj. Callback addr. %s" %
                    (addr,))
    ...
    # Instead of being sent to log or alarm server they just get written to
    # stdout.
