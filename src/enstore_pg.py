...
def acc_encp_lines(csc, numEncps=100):
    ...
    res_err = db.query("select date, node, pid, username, encp_id, version, type, error, src, dst, size, storage_group, file_family, wrapper,  mover, drive_id,  drive_sn,  rw, volume from encp_error order by date desc limit %s;"%(numEncps,))
    # combine the 2 lists and take the top numEncps entries to return
    res_total = res.dictresult() + res_err.dictresult()
    res_total.sort()
