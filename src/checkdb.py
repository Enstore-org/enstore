...
def configure(configuration=None):
    ...
    if mynode != backup_node and backup_fqdn != mynode_fqdn:
        print(timeofday.tod(), "ERROR Backups are stored", backup_node, end=' ')
...
def check_backup(backup_dir, backup_node):
    ...
    if len(bdirs) == 0:
        print(timeofday.tod(), "ERROR NO Backups found on ", backup_node, end=' ')
    ...
    if time.time() - os.stat(container)[stat.ST_MTIME] < 60:
        ...
        print(timeofday.tod(), "Checking container", container, end=' ')
        print(
            "last modified on",
            time.ctime(
                os.stat(container)[
...
def start_postmaster(db_path):
    cmd = "ps axw| grep postmaster | grep %s | grep -v grep | awk '{print $1}'" % (
        db_path)
    ...
    if pid:
        print(
            timeofday.tod(),
            "postmaster has already been started -- pid =",
            pid)
        sys.exit(1)
    else:
...
def stop_postmaster(db_path):
    pid = os.popen(
        "ps axw| grep postmaster | grep %s | grep -v grep | awk '{print $1}'" %
        (db_path)).readline()
    pid = string.strip(pid)
    if pid:
...
def extract_backup(check_dir, container):
    print(timeofday.tod(), "Extracting database files from backup container", end=' ')
    print(container)
    ...
    os.system(
        "pg_restore -d backup -v -s -t no_flipping_storage_group " +
        container)
    ...
    os.system(
        "pg_restore -d backup -v -s -t no_flipping_file_family " +
        container)
    os.system("pg_restore -d backup -v -s -t file_copies_map " + container)
    os.system("pg_restore -d backup -v -a " + container)
    os.system(
        "psql backup -c 'alter table only volume add constraint volume_pkey primary key (id);'")
    os.system(
        "psql backup -c 'create index volume_storage_group_idx on volume(storage_group);'")
    os.system(
        "psql backup -c 'create index volume_system_inhibit_0_idx on volume(system_inhibit_0);'")
    os.system(
        "psql backup -c 'alter table only file add constraint file_pkey primary key (bfid);'")
    os.system(
        "psql backup -c 'CREATE INDEX package_id_fk_idx ON file USING btree (package_id);'")
    os.system(
        "psql backup -c 'CREATE INDEX file_volume_idx ON file USING btree (volume);'")
    ...
    while True:
        ...
        for row in res:
            ...
            if sg in EXCLUDED_STORAGE_GROUP:
                continue
            complete_file_listing_fd.write(
            ...
            if sg not in output_files:
                ...
                output_files[sg].write(
                    complete_file_listing_header.format(
                        time_stamp, sg))
            ...
            if sg not in recent_output_files:
                ...
                recent_output_files[sg].write(
                    recent_files_listing_header.format(
                        time_stamp, os.path.basename(
                            sys.argv[0])))
                head = "Recent (packed and package) written to tape in the last {} hours for storage group {}".format(
                    DURATION, sg)
                recent_output_files[sg].write("\n{}\n\n".format(head))
                recent_output_files[sg].write(
                    "update_time storage_group file_family volume location_cookie bfid size crc pnfs_id pnfs_path archive_status package_pnfsid\n")
...
if __name__ == "__main__":
    ...
    (backup_dir, check_dir, current_dir, backup_node,
     dest_path, db_path) = configure(1)  # non-None argument!
