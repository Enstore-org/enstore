...
def check_db(check_dir):
	...
	print timeofday.tod(), "Listing all files ... (old style)"
        query = "SELECT f.bfid, "+\
                ...
                "f.pnfs_path from file f, volume v WHERE f.volume=v.id and v.system_inhibit_0 != 'DELETED' and f.deleted = 'n' and  f.package_id!=f.bfid "+\
        ...
        sg_query = "SELECT v.storage_group, v.file_family, "+\
                   ...
                   "ELSE f.location_cookie "+\
                   "END as location_cookie, "+\
                   "f.bfid, f.size, f.crc, f.pnfs_id, f.pnfs_path "+\
                   "FROM file f, volume v "+\
                   "WHERE f.volume=v.id and v.system_inhibit_0 != 'DELETED' and f.deleted = 'n' and v.storage_group='%s' and f.package_id != f.bfid "+\
                   "ORDER by v.file_family, volume, location_cookie"
