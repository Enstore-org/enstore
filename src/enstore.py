...
# the tee is not robust - need to add code to check if we can write to tty
# (that is connected to console server)
...
class EnstoreInterface:
    ...
    def get_valid_servers(self):
        servers = sorted(server_functions.keys())
        if self.user_mode:
            allowed_servers = []
            for server in servers:
    ...
    def get_all_servers(self):
        if not self.user_mode:
            scripts = sorted(local_scripts.keys() + remote_scripts.keys())
        else:
            scripts = []
...
class Enstore:
    ...
    def get_config_from_file(self):
        ...
        return rtn
    """
    # make sure the user wanted to start d0en nodes while on stken and vice
    # versa
        ...
        if VERIFY in command:
            ...
            if gang in clusters:
                ...
                if thisgang not in clusters:
                    ...
                    return 1
                # if we are trying to execute a command from a node in the same
                # cluster, just do it
                if thisgang == gang:
                    return 1
                ...
                print(
                    "You want to execute a command on",
                    node,
