...
class Restrictor:
    ...
    def match_found(self, ticket):
        ...
        # what to do with request ("ignore", "reject"), None if fuction was not
        # found
        ...
        for key in self.discipline_dict.keys():
            ...
            if self.ticket_match(key, host) == 1:
                # use deep copy to make sure that the original set of arguments
                # is returned
                match = 1, self.discipline_dict[key]['function'], copy.deepcopy(
                    self.discipline_dict[key]['args']), self.discipline_dict[key]['action']
                break
