...
class TestEnstoreError(unittest.TestCase):
    ...
    def setUp(self):
        self.e_errors_list = [ABOVE_THRESHOLD, ALARM, BAD_FILE_SIZE, BELOW_THRESHOLD, BFID_EXISTS, BROKEN, CANTRESTART,
                             CLEANUDP_EXCEPTION, CONFIGDEAD, CONFLICT, CRC_DCACHE_ERROR, CRC_ECRC_ERROR, 
                             CRC_ENCP_ERROR, CRC_ERROR, CRC_ERROR_IN_WRITE_CLIENT, DATABASE_ERROR, DEFAULT_ROOT_ERROR, 
                             DEFAULT_SEVERITY, DELETED, DEVICE_ERROR, DISMOUNTFAILED, DOESNOTEXIST, DOESNOTEXISTSTILLDONE, 
                             DRIVEERROR, EMAIL, ENCP_GONE, ENCP_STUCK, ENSTOREBALLRED, EOV1_ERROR, EPROTO, ERROR, 
                             EnstoreError, FILESYSTEM_CORRUPT, FILE_CLERK_ERROR, FILE_MODIFIED, IGNORE, INFO, 
                             INFO_SERVER_ERROR, INPROGRESS, INVALID_ACTION, INVALID_WRAPPER, IOERROR, KEYERROR, 
                             LMD_WRONG_TICKET_FORMAT, LOCKED, MALFORMED, MC_DRVNOTEMPTY, MC_DRVNOTFOUND, MC_FAILCHKDRV, 
                             MC_FAILCHKVOL, MC_NONE, MC_QUEUE_FULL, MC_VOLNOTFOUND, MC_VOLNOTHOME, MEDIAERROR, MEDIA_IN_ANOTHER_DEVICE, 
                             MEMORY_ERROR, MISC, MOUNTFAILED, MOVERLOCKED, MOVER_BUSY, MOVER_CRASH, MOVER_STUCK, NET_ERROR, 
                             NOACCESS, NOALARM, NOMOVERS, NOREAD, NOSPACE, NOTALLOWED, NOT_ALWD_EXCEPTION, NOT_SUPPORTED, 
                             NOVOLUME, NOWORK, NOWRITE, NO_CRC_RETURNED, NO_FC_EXCEPTION, NO_FILE, NO_FILES, 
                             NO_PNFS_EXCEPTION, NO_SG, NO_VOLUME, OK, OSERROR, PAUSE, PNFS_ERROR, POSITIONING_ERROR, POSIT_EXCEPTION, 
                             QUOTAEXCEEDED, READ_BADLOCATE, READ_BADMOUNT, READ_BADSWMOUNT, READ_EOD, READ_EOT, READ_ERROR, READ_NODATA, 
                             READ_NOTAPE, READ_TAPEBUSY, READ_VOL1_MISSING, READ_VOL1_READ_ERR, READ_VOL1_WRONG, RECYCLE, REJECT, RESTRICTED_SERVICE, 
                             RESUBMITTING, RETRY, SERVERDIED, TCP_EXCEPTION, TCP_HUNG, TIMEDOUT, TOOMANYSUSPVOLS, TOO_MANY_FILES, 
                             TOO_MANY_RESUBMITS, TOO_MANY_RETRIES, TOO_MANY_VOLUMES, TestEnstoreError, UNCAUGHT_EXCEPTION, 
                             UNKNOWN, UNKNOWNMEDIA, UNLOCKED, USERERROR, USER_ERROR, VERSION_MISMATCH, VM_CONF_EXCEPTION, 
                             VM_ENSTORE_EXCEPTION, VM_PNFS_EXCEPTION, VOLUME_CLERK_ERROR, VOLUME_EXISTS, VOL_SET_TO_FULL, WARNING, WRITE_BADMOUNT, 
                             WRITE_BADSPACE, WRITE_BADSWMOUNT, WRITE_EOT, WRITE_ERROR, WRITE_NOTAPE, WRITE_TAPEBUSY, WRITE_VOL1_MISSING, 
                             WRITE_VOL1_READ_ERR, WRITE_VOL1_WRONG, WRONGPARAMETER,
                             WRONG_FORMAT]
    ...
    def test_init(self):
        ...
        self.assertEqual(self.e0.ticket, {'status': ('UNKNOWN', 'test error 0')})
    ...
    def test_is_retriable(self):
        ...
        #self.assertEqual(is_retriable(OK), 1, "expected %s got %s" % (1,is_retriable(OK)) )
    ...
    def test_is_non_retriable(self):
        ...
        self.assertEqual(is_non_retriable(OK), 0, "expected %s got %s" % (0,is_non_retriable(OK)) )
        for e in non_retriable_errors:
    ...
    def test_retriable_discrepancy(self):
        ...
        for e in self.e_errors_list:
            ...
            if e not in fail_list:
                ...
                self.assertEqual(r+n, 1, "expected %s got %s testing %s" % (1,r+n, e) )
    ...
    def test_is_media(self):
        ...
        media_errors = [WRITE_NOTAPE, WRITE_TAPEBUSY, WRITE_BADMOUNT,
                   WRITE_BADSWMOUNT, WRITE_BADSPACE, WRITE_ERROR, WRITE_EOT]
        media_errors.extend([READ_NOTAPE, READ_TAPEBUSY, READ_BADMOUNT,
                   READ_BADSWMOUNT, READ_BADLOCATE, READ_ERROR, READ_EOT,
                   READ_EOD, READ_NODATA])        
        media_errors.extend([READ_VOL1_READ_ERR, WRITE_VOL1_READ_ERR, READ_VOL1_MISSING,
