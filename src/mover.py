...
class Mover(dispatching_worker.DispatchingWorker,
    ...
    def check_sched_down(self):
        ...
        if not host:
            return 0
        cmd = 'rsh %s \' su -c ". /usr/local/etc/setups.sh; setup enstore; enstore sched --show" enstore\'' % (host,)
        p = os.popen(cmd, 'r')
        r = p.read()
        s = p.close()
        if s:
    ...
    def set_sched_down(self):
        ...
        if not host:
            return 0
        cmd = 'rsh %s \' su -c ". /usr/local/etc/setups.sh; setup enstore; enstore sched --down=%s; enstore system" enstore\'' % (
            host, self.name)
        p = os.popen(cmd, 'r')
        r = p.read()
        s = p.close()
