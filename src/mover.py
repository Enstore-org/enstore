...
# error sources
TAPE, ROBOT, NETWORK, DRIVE, USER, MOVER, UNKNOWN = [
    'TAPE', 'ROBOT', 'NETWORK', 'DRIVE', 'USER', 'MOVER', 'UNKNOWN']
...
class Buffer:
    ...
    def __init__(self, blocksize, min_bytes=0, max_bytes=1 * MB, crc_seed=1):
        """
...
# self._freelist = []   keep this around to save on malloc's
    ...
    def empty(self):
        # this means that a stream write would fail - we have no data at all to
        # send
        self._lock.acquire()
    ...
    def __repr__(self):
        return "Buffer %s  %s  %s" % (
            self.min_bytes, self._buf_bytes, self.max_bytes)
    ...
    def block_read(self, nbytes, driver, fill_buffer=1):
        ...
        t2 = time.time()
        # Trace.trace(222,"block_read: bytes_read: %s in
        # %s"%(bytes_read,t2-t1)) # COMMENT THIS
        ...
        if self.first_block:  # Handle variable-sized cpio header
            ...
            if len(data) >= self.wrapper.min_header_size:
                try:
                    ...
                    Trace.log(e_errors.ERROR, "Invalid header %s" %
                              (data[:self.wrapper.min_header_size]))
        ...
        if do_crc:
            crc_error = 0
            try:
                # Trace.trace(22,"block_read: data_ptr %s, bytes_for_cs %s" %
                # (data_ptr, bytes_for_cs)) #COMMENT THIS
        ...
        t4 = time.time()
        self.read_stats[0] = self.read_stats[0] + \
            t1 - t0   # total time in get_space
        self.read_stats[1] = self.read_stats[1] + \
            t2 - t1   # total time in read
        self.read_stats[2] = self.read_stats[2] + \
            t3 - t2   # total time in check CRC
        self.read_stats[3] = self.read_stats[3] + \
            t4 - t3   # total time in push
        self.read_stats[4] = self.read_stats[4] + \
            t4 - t0   # total time in block_read
    ...
    def block_write(self, nbytes, driver):
        ...
        if len(data) != nbytes:
            raise_(
                ValueError, "asked to write %s bytes, buffer has %s" %
                (nbytes, len(data)))
        ...
        if bytes_written == nbytes:  # normal case
            ...
            if do_crc:
                ...
                if self.bytes_written >= self.trailer_pnt:
                    ...
                    number_to_skip = number_to_skip + self.bytes_written + \
                        bytes_written - self.trailer_pnt
                ...
                if bytes_for_cs:
                    try:
                        ...
                        # if self.first_block and self.sanity_bytes <
                        # SANITY_SIZE:
                        if self.sanity_bytes < SANITY_SIZE:
                            nbytes = min(
                                SANITY_SIZE - self.sanity_bytes, bytes_for_cs)
                            self.sanity_crc = checksum.adler32_o(self.sanity_crc,
                                                                 data,
                                                                 data_ptr, nbytes)
                            self.sanity_bytes = self.sanity_bytes + nbytes
            ...
            self._freespace(data)
        self.write_stats[0] = self.write_stats[0] + \
            t1 - t0   # total time in pull
        self.write_stats[1] = self.write_stats[1] + \
            t2 - t1   # total time in write
        self.write_stats[2] = self.write_stats[2] + \
            t3 - t2   # total time in check CRC
        self.write_stats[3] = self.write_stats[3] + \
            t4 - t3   # total time in freespace
        self.write_stats[4] = self.write_stats[4] + \
            t4 - t0   # total time in block_write
    ...
    def stream_write(self, nbytes, driver):
        ...
        if not self._writing_block:
            ...
            self._write_ptr = 0
        bytes_to_write = min(len(self._writing_block) -
                             self._write_ptr, nbytes)
        Trace.trace(
            135, "bytes_to_write %s write_ptr %s" %
            (bytes_to_write, self._write_ptr))

        if driver:
            bytes_written = driver.write(
                self._writing_block, self._write_ptr, bytes_to_write)
            Trace.trace(124, "BYTES WRITTEN %s" % (bytes_written,))
            if bytes_written != bytes_to_write:
                msg = "encp gone? bytes to write %s, bytes written %s" % (
        ...
        Trace.trace(135, "write_ptr %s len w_b %s" %
                    (self._write_ptr, len(self._writing_block)))
        if self._write_ptr == len(
                self._writing_block):  # finished sending out this block
...
# cookie is such a silly term, but I guess we're stuck with it :-(
def cookie_to_long(cookie):
    ...
    if not isinstance(cookie, str):
        raise_(
            TypeError, "expected string or integer, got %s %s" %
            (cookie, type(cookie)))
    if '_' in cookie:
...
class Mover(dispatching_worker.DispatchingWorker,
    ...
    def __init__(self, csc_address, name, logclient=None,
        ...
        # used to bypass GIL problem detected in disk movers after kernel
        ...
        # allow logging additional mover information (configurable)
        ...
        # use this to synchronize read and network threads
        self.read_tape_done = threading.Event()
        self.stream_w_flag = 0    # this flag is set when before stream_write is called
        # volume clerk address. Used in LM to identify volume clerk. Needed for
        # sharing movers and
        self.vc_address = None
        # LMs across systems

        self.dont_update_lm = 0   # if this flag is set do not update LM to avoid mover restart
        self.initial_crc_seed = 1  # adler 32 default seed
        # crc seed to use in adler32 (0 or 1)
        ...
        # this is to indicate that the new volume will be mounted and the
        # current dismounted (HIPR)
        self.will_mount = None
        self.last_absolute_location = 0
        # set this to True before tape write, if write comletes successfuly it
        ...
        # maximal number of consecutive errors (configurable)
        self.max_consecutive_failures = 2
        # consecutive error count for media changer requests
        self.media_changer_consecutive_failures = 0
        self.local_mcc = False
        # maximal number of failures before going OFFLINE (configurable)
        self.max_failures = 3
        # offline mover if number of failures>self.max_failures within this
        ...
        if self.config['status'][0] != 'ok':
            raise MoverError(
                'could not start mover %s: %s' %
                (self.name, self.config['status']))
    ...
    def __setattr__(self, attr, val):
        ...
        Trace.trace(
            10, "set state %s %s" %
            (state_name(val), self.__dict__['state_change_time']))
    ...
    def set_new_bitfile(self, fc_ticket, vc_ticket):
        ...
        if fc_ticket.get(
                'original_library') and fc_ticket['original_library'] == vc_ticket['library']:  # no redirection was needed
            # request will not be put into files_in_transition table
            fc_ticket['original_library'] = None
            ...
            if fcc_reply['status'][0] == e_errors.OK:
                ...
                if fcc_reply['status'][0] != e_errors.TIMEDOUT:
                    ...
                    if i == 0:
                        Trace.log(
                            e_errors.INFO,
                            "re-trying new bitfile request %s " %
                            (request))
                    else:
                        Trace.log(
                            e_errors.INFO,
        ...
        if self.log_mover_state or logit:
            ...
            for thread in threads:
                if thread.isAlive():
                    ...
                    Trace.log(
                        e_errors.INFO, "LOG(%s): Thread is dead" %
                        (thread_name,))
    ...
    def memory_usage(self):
        ...
        try:
            ...
            Trace.log(
                e_errors.WARNING, 'unexpected command return: %s' %
                (result,))
            return 1
        #print z
        if z[0] == 'root':
        ...
        if mem_u > self.max_idle_mem:
            Trace.log(
                e_errors.WARNING,
                "Memory usage %s approaches a limit %s. %s. Will restart the mover" %
                (mem_u,
                 self.max_idle_mem,
                 result,
                 ))
    ...
    def watch_syslog(self):
        ...
        if self.syslog_match:
            try:
                cmd = "$ENSTORE_DIR/src/match_syslog.py '%s'" % (
                    self.syslog_match)
                result = shell_command(cmd)
                if result:
                    for l in result:
                        Trace.log(e_errors.INFO, "SYSLOG Entry:[%s] %s" % (
                            l[:-1], self.current_volume))
            except BaseException:  # do not know what kind of exception it may be
                Trace.handle_error()
    ...
    def check_sched_down(self):
        ...
        if s:
            Trace.log(
                e_errors.ERROR, "error getting outage file (%s)" %
                (self.name,))
    ...
    def init_stat(self, drive_name):
        ...
        try:
            self.dsc = drivestat_client.dsClient(
                self.csc, drive_name, logc=self.logclient)
            self.stats_on = 1
        except BaseException:
            Trace.handle_error()
            pass
    ...
    def set_volume_noaccess(self, volume, reason=None):
        ...
        Trace.alarm(
            e_errors.ALARM, "marking %s NOACCESS. Mover: %s. Reason: %s" %
            (volume, self.shortname, reason))
    ...
    def update_tape_stats(self):
        ...
        try:
            ...
            if isinstance(self.stats[self.ftt.MEDIA_END_LIFE], type(None)):
                media_end_life = 0
            else:
                media_end_life = int(self.stats[self.ftt.MEDIA_END_LIFE])
            if isinstance(
                    self.stats[self.ftt.NEARING_MEDIA_END_LIFE], type(None)):
    ...
    def update_stat(self):
        ...
        if self.stats_on and self.tape_driver and self.tape_driver.ftt:
            try:
                ...
                Trace.log(e_errors.ERROR, 'Type Error getting USER_READ: %s %s' % (
                    detail, stats[self.ftt.USER_READ]))
                user_read = -1

            try:
                user_write = long(stats[self.ftt.USER_WRITE]) / 1024.
            except TypeError as detail:
                Trace.log(e_errors.ERROR, 'Type Error getting USER_WRITE: %s %s' % (
                    detail, stats[self.ftt.USER_WRITE]))
                user_write = -1

            try:
                read_count = long(stats[self.ftt.READ_COUNT]) / 1024.
            except TypeError as detail:
                Trace.log(e_errors.ERROR, 'Type Error getting READ_COUNT: %s %s' % (
                    detail, stats[self.ftt.READ_COUNT]))
                read_count = -1
            try:
                write_count = long(stats[self.ftt.WRITE_COUNT]) / 1024.
            except TypeError as detail:
                Trace.log(e_errors.ERROR, 'Type Error getting WRITE_COUNT: %s %s' % (
                    detail, stats[self.ftt.WRITE_COUNT]))
                write_count = -1
            try:
                read_errors = long(stats[self.ftt.READ_ERRORS])
            except TypeError as detail:
                Trace.log(e_errors.ERROR, 'Type Error getting READ_ERRORS: %s %s' % (
                    detail, stats[self.ftt.READ_ERRORS]))
                read_errors = -1
            try:
                write_errors = long(stats[self.ftt.WRITE_ERRORS])
            except TypeError as detail:
                Trace.log(e_errors.ERROR, 'Type Error getting WRITE_ERRORS: %s %s' % (
                    detail, stats[self.ftt.WRITE_ERRORS]))
                write_errors = -1
            try:
                write_prot = int(stats[self.ftt.WRITE_PROT])
            except TypeError as detail:
                Trace.log(e_errors.ERROR, 'Type Error getting WRITE_PROT: %s %s' % (
                    detail, stats[self.ftt.WRITE_PROT]))
                write_prot = -1
    ...
    def fetch_tape_device(self):
        ...
        if result[0] == 0:
            ...
            if len(lines) == 1:
                ...
                Trace.alarm(
                    e_errors.ERROR, "Non existing or ambiguous device in /dev/rmt: %s" %
                    (result,))
        else:
            Trace.alarm(
                e_errors.ERROR, "Device search returned %s" %
                (result[2],))
    ...
    def get_tape_device(self):
        ...
        if 'type' in self.mc_keys and self.mc_keys['type'] == 'STK_MediaLoader':
            # drive_info example:
            # {'Wwn': '50.01.04.f0.00.a2.b4.b2', 'drive': '2,3,1,15'}
            wwn = int(
                "0x%s" %
                (mcc_reply['drive_info']['Wwn'].replace(
                    '.',
                    '')),
                16)  # node name identifier wwn
        else:  # here should be other MC types
            # just set it the same as for STK library so far
            wwn = int(
                "0x%s" %
                (mcc_reply['drive_info']['Wwn'].replace(
                    '.', '')), 16)
        ...
        if result[0] == 0:
            ...
            if not os.path.exists(rmt_dir):
                try:
                    ...
                    Trace.handle_error()
                    Trace.alarm(
                        e_errors.ERROR, "Can not create %s" %
                        (rmt_dir,))
                    return None
            try:
                os.remove(ftt_tape)
            ...
            Trace.log(
                e_errors.INFO, 'symlinking: /dev/nst%s to %s' %
                (nst_num, ftt_tape))
    ...
    def start(self):
        ...
        if self.logclient and isinstance(
                self.logclient, log_client.TCPLoggerClient):
        ...
        cmd = "EPS | grep %s | grep %s | grep -v grep" % (
            self.name, "mover.py")
        result = shell_command(cmd)
        if result:
            if len(result) > 1:
                Trace.alarm(
                    e_errors.ERROR, "mover is already running, can not restart: %s" %
                    (result,))
        ...
        Trace.log(
            e_errors.INFO, "starting mover %s. MAX_BUFFER=%s MB" %
            (self.name, MAX_BUFFER / MB))
        ...
        try:
            self.vcc = volume_clerk_client.VolumeClerkClient(
                self.csc, logc=self.logclient)
        except BaseException:
            self.vcc is None
        try:
            self.fcc = file_clerk_client.FileClient(
                self.csc, logc=self.logclient)
        except BaseException:
            self.fcc is None
        ...
        if isinstance(self, DiskMover):
            ...
            self.mover_type = self.config.get(
                'type', enstore_constants.DISK_MOVER)
            ...
            if self.config.get('device'):
                if not os.path.exists(self.config['device']):
                    ...
                    try:
                        os.makedirs(self.config['device'], 0o755)
                    except OSError as detail:
                        if detail[0] != errno.EEXIST:
                            raise OSError(detail)
            else:
                Trace.alarm(
                    e_errors.ALARM, "Cannot start. Device %s does not exist" %
                    (self.config['device'],))
                sys.exit(-1)
            ...
            try:
                ...
                if detail[0] != errno.EEXIST:
                    Trace.alarm(
                        e_errors.ALARM, "Cannot start. OSError %s" %
                        (str(detail),))
                    sys.exit(-1)
        ...
        if 'do_cleaning' in self.config:
            if self.config['do_cleaning'][0] in ('n', 'N'):
                self.do_cleaning = 0

        self.rem_stats = 1
        if 'get_remaining_from_stats' in self.config:
            if self.config['get_remaining_from_stats'][0] in ('n', 'N'):
                self.rem_stats = 0

        self.mc_device = self.config.get('mc_device', 'UNDEFINED')
        self.media_type = self.config.get(
            'media_type', default_media_type)  # XXX
        ...
        # pattern to match in syslog for scsi error
        self.syslog_match = self.config.get("syslog_entry", None)
        ...
        self.max_idle_mem = self.config.get('max_idle_mem', 10)

        if self.check_sched_down() or self.check_lockfile():
            self.state = OFFLINE

        self.asc = accounting_client.accClient(
            self.csc, self.logname, logc=self.logclient)
        ...
        self.failure_interval = self.config.get(
        ...
        if not isinstance(lib_list, type([])):
            lib_list = [lib_list]
        for lib in lib_list:
            lib_config = self.csc.get(lib)
            if 'mover_port' in lib_config:
        ...
        self.check_written_file_period = self.config.get(
            'check_written_file', 0)
        self.files_written_cnt = 0
        # maximal time allowed in a certain states
        self.max_time_in_state = self.config.get('max_time_in_state', 600)
        ...
        elif crc_control is not None:
            Trace.log(
                e_errors.ERROR,
                "Ignoring invalid 'read_crc_control' value of %s found in configuration. Allowed values are 0 or 1. Using default value of 0 ('calculate CRC when reading from memory')." %
                (crc_control,
                 ))
        self.read_error_recovery_timeout = float(
            self.config.get("read_error_recovery_timeout", 20 * 60))
        ...
        if not self.config.get('device'):
            ...
            if self.config['device']:
                Trace.log(
                    e_errors.INFO, "Dynamically picked device %s" %
                    (self.config['device'],))
            else:
                Trace.alarm(
                    e_errors.ALARM,
                    "Cannot start. No device was found")
                sys.exit(-1)
        ...
        if drive_rate_threshold:
            exit_flag = False
            if isinstance(drive_rate_threshold, tuple) and len(
                    drive_rate_threshold) == 2:
                try:
                    self.drive_rate_threshold = (
                        long(
                            drive_rate_threshold[0]), long(
                            drive_rate_threshold[1]))
                except BaseException:
                    exit_flag = True
            else:
                exit_flag = True
            if exit_flag:
                Trace.log(
                    e_errors.ERROR,
                    "Wrong format for drive_rate_threshold. Check configuration")
                sys.exit(-1)

        Trace.log(e_errors.INFO, "Starting in state %s" %
        ...
        if self.driver_type == 'NullDriver':
            ...
            self.device = None
            # need this to cause EOD cookies to update.
            self.single_filemark = 1
            ...
            if self.state == IDLE:
                good_label = 1
                try:
                    have_tape = self.tape_driver.open(
                        self.device, mode=0, retry_count=3)
                except self.ftt.FTTError as detail:
                    Trace.alarm(
                        e_errors.ERROR,
                        "Supposedly a serious problem with tape drive: %s %s. Will terminate" %
                        (self.ftt.FTTError,
                         detail))
                    time.sleep(5)
                    sys.exit(-1)
                ...
                try:
                    ...
                    Trace.alarm(
                        e_errors.ALARM, "Can not start: %s" %
                        (detail,))
                    print("Can not start: %s" % (detail,))
                    sys.exit(-1)
                if use_local_mc:
                    # Find and set mc drive address
                    self.mc_device, self.mc_device_phys_location = self.find_mc_drive_address(
                        self.config['serial_num'])
                    if not self.mc_device:
                        Trace.alarm(
                            e_errors.ERROR,
                            'Can not find media changer address for this mover. Will terminate')
                        sys.exit(-1)
                    Trace.log(
                        e_errors.INFO, 'MC drive address: %s physical location %s serial number %s' %
                        (self.mc_device, self.mc_device_phys_location, self.config['serial_num']))
                ...
                if self.config['product_id'] in ("T10000C", "T10000D"):
                    ...
                    if not disable_AMC:
                        # enable AMC
                        try:
                            enabled = scsi_mode_select.t10000c_amc(
                                self.tape_driver, 1)
                        except Exception as detail:
                            Trace.alarm(
                                e_errors.ERROR, 'Failed to set  "Allow Maximum Capacity": %s' %
                                (str(detail),))
                            sys.exit(-1)
                        if not enabled:
                            Trace.alarm(
                                e_errors.ERROR, '"Allow Maximum Capacity" was not set')
                            sys.exit(-1)
                        # if AMC is set do not use remaining bytes information
                        # from tape drive
                        self.rem_stats = 0
                    # set initial compression
                    rc = scsi_mode_select.t10000_set_compression(
                        self.tape_driver, compression=self.compression)
                    if not rc:
                        Trace.alarm(
                            e_errors.ERROR, "Compression setting failed")
                        sys.exit(-1)
                    ...
                    if status[0] == e_errors.OK:
                        ...
                        for i in range(2):
                            ...
                            if isinstance(v, type(
                                    {})) and 'status' in v and v['status'][0] == e_errors.OK:
                        ...
                        if not found:
                            Trace.log(
                                e_errors.ERROR, "Can not identify volume %s. Exiting" %
                                (self.current_volume,))

                        if self.state == OFFLINE:
                            Trace.log(
                                e_errors.INFO, "Mover is OFFLINE. Performing dismount at startup")
                            self.dismount_volume(after_function=self.offline)
                            self.current_volume = None
                        else:
                            self.state = HAVE_BOUND
                            Trace.log(
                                e_errors.INFO, "have vol %s at startup" %
                                (self.current_volume,))
                        ...
                        Trace.alarm(
                            e_errors.ERROR,
                            "failed to read volume label on startup")
                        ejected = self.tape_driver.eject()
                        if ejected == -1:
                            if self.can_force_eject:
                                # try to unload tape if robot is STK. It can do
                                # this
                                Trace.log(
                                    e_errors.INFO, "Eject failed. For STK robot will try to unload anyway")
                            else:
                                Trace.alarm(
                                    e_errors.ERROR, "cannot eject tape on startup, will die")
                                time.sleep(2)
                                sys.exit(-1)
                        have_tape = 0
                else:
                ...
                if not have_tape:
                    Trace.log(e_errors.INFO,
                              "performing precautionary dismount at startup")
                    ...
                    for i in range(retry_cnt):
                        ...
                        if 'max_work' not in mcc_reply:
                            Trace.log(
                                e_errors.INFO, "Media Changer returned %s" %
                                (mcc_reply,))
                        else:
                            time.sleep(2)
                            break
                        ...
                        Trace.log(
                            e_errors.ERROR, "Media Changer did not return expected reply: %s" %
                            (mcc_reply,))
                        self.state = OFFLINE
                    if self.state != OFFLINE:
                        if mcc_reply['max_work'] == 0:
                            # media changer would not accept requests. Go
                            # OFFLINE
                            Trace.alarm(
                                e_errors.ERROR, "media changer is locked, going OFFLINE")
                            self.state = OFFLINE
                        else:

                            while True:
                                mcc_reply = self.mcc.unloadvol(
                                    vol_ticket, self.name, self.mc_device)
                                status = mcc_reply.get('status')
                                if status and status[0] == e_errors.MC_QUEUE_FULL:
                                    # media changer responded but could not
        ...
        if not isinstance(self.mount_delay, type(0)):
    ...
    def restart_unlock(self):
        ...
        try:
            ...
            Trace.log(
                e_errors.ERROR, "Cannot unlink %s %s" %
                (filename, detail))
    ...
    def restart(self, do_restart=1):
        ...
        if restart_allowed == 0:
            Trace.log(
                e_errors.ERROR, "Can not restart there is a lock file %s preventing restart" %
                (self.restart_lockfile_name(),))
            sys.exit(-1)
        ...
        if cur_thread_name:
            if cur_thread_name in ('net_thread', 'MainThread'):
                ...
                if thread and thread.isAlive():
                    Trace.log(
                        e_errors.INFO,
                        "waiting for tape thread to finish")
    ...
    def _reinit(self):
        ...
        if encp_dict:
            self.crc_seed = long(encp_dict.get("crc_seed", 1))
    ...
    def check_drive_rate(self, bfid, file_size, drive_rate,
                         rw, read_err, write_err, drive_stats):
        """
        ...
        if hasattr(self, "drive_rate_threshold") and isinstance(
                self.drive_rate_threshold, tuple):
            # self.drive_rate_threshold[0] - cut off file size
            if file_size > self.drive_rate_threshold[0]:
                # self.drive_rate_threshold[1] - cut off drive rate
                if drive_rate < self.drive_rate_threshold[1]:
                    ...
                    Trace.alarm(
                        e_errors.ERROR, "Slow drive rate detected. Drive %s rate %3.2f MB/s" %
                        (self.logname, drive_rate / MB))
    ...
    def handle_mover_error(self, exc, msg, tb):
        ...
        if self.current_work_ticket:
            try:
                Trace.trace(
                    10, "handle error: calling transfer failed, str(msg)=%s" %
                    (str(msg),))
    ...
    def update_lm(self, state=None, reset_timer=None, error_source=None):
        ...
        if self.state in (IDLE, HAVE_BOUND):
            ...
            if n_thread and n_thread.isAlive():
                ...
                if not hasattr(self, 'restarting'):
                    ...
                    if self.state == HAVE_BOUND:
                        Trace.log(
                            e_errors.INFO, "Trying to dismount volume (1) %s" %
                            (self.current_volume,))
                        self.run_in_thread(
                            'media_thread',
                            self.dismount_volume,
                            after_function=self.send_error_and_restart)
                ...
                if self.config['product_id'].find("DLT") == -1:
                    ...
                    if self.delay_alarm_for_tape_thread:
                        Trace.log(
                            DEBUG_LOG,
                            'delaying alarm for tape thread running in %s for %s s' %
                            (state_name(
                                self.state),
                                t_in_state))
                        Trace.trace(10, 'delaying alarm for tape thread running in %s for %s s' % (
                            state_name(self.state), t_in_state))  # repeat for stdio
                        time.sleep(1)
                        ...
                        Trace.alarm(e_errors.ALARM,
                                    ...
                                    (state_name(self.state),))
                        Trace.trace(
                            10, 'tape thread is running in %s for %s s' %
                            (state_name(
                                self.state), t_in_state))
                        self.watch_syslog()
                        self.log_state(logit=1)
                        Trace.log(
                            e_errors.INFO, "Trying to dismount volume %s" %
                            (self.current_volume,))
                        self.dismount_volume(after_function=self.offline)
                        return
                else:
        ...
        if (thread_name == 'MainThread'):
            ...
            if (((time_in_state - self.time_in_state) > self.max_time_in_state) and
                ...
                if self.state == ACTIVE:
                    Trace.trace(
                        8, "bytes read last %s bytes read %s" %
                    ...
                    if self.bytes_read_last == self.bytes_read:
                        if self.mode in (WRITE, ASSERT):
                            ...
                            Trace.log(
                                e_errors.INFO,
                                "net thread %s tape thread %s" %
                                (n_thread.isAlive(),
                                 t_thread.isAlive()))  # Remove this when problem is fixed !!!!!
                ...
                if not hasattr(self, 'too_long_in_state_sent') and send_alarm:
                    if (self.state != ERROR and
                        ...
                        try:
                            ...
                            try:
                                Trace.log(
                                    e_errors.ERROR, "error sending alarm %s %s" %
                                    (exc, msg))
                                Trace.log(
                                    e_errors.INFO, "state %s" %
                                    (self.state,))
                                Trace.log(
                                    e_errors.INFO, "volume %s" %
                                    (self.current_volume,))
                                Trace.log(
                                    e_errors.INFO, "host %s" %
                                    (self.current_work_ticket['wrapper']['machine'][1],))
                            except TypeError:
                                Trace.log(
                                    e_errors.ERROR, "wrong ticket format? %s" %
                                    (self.current_work_ticket))
                ...
                Trace.trace(
                    8, "in_state_to_cnt %s max_in_state_cnt %s" %
                    (self.in_state_to_cnt, self.max_in_state_cnt,))
                ...
                if ((self.in_state_to_cnt >= self.max_in_state_cnt) and
                    ...
                    if self.state != ACTIVE:
                        ...
                        # offline it
                        msg = "mover is stuck in %s. Client host %s" % (
                            self.return_state(), self.current_work_ticket['wrapper']['machine'][1])
                        Trace.alarm(e_errors.ERROR, msg)
                        if self.state == SEEK:
                            self.set_volume_noaccess(
                                self.current_volume, "Mover stuck. Seelog for details")
                        self.transfer_failed(
                            e_errors.MOVER_STUCK, msg, error_source=TAPE, dismount_allowed=0)
                        self.offline()
                        return

                    else:
                        if transfer_stuck:
                            msg = "data transfer to or from client stuck. Client machine %s. Breaking connection." % (
                                self.current_work_ticket['wrapper']['machine'][1],)
                            if self.mode == READ:
                                msg1 = "Stream write flag %s. Bytes %s/%s" % (
                                    self.stream_w_flag, self.bytes_written, self.bytes_to_write)
                                msg = msg + msg1
                            self.transfer_failed(
                                e_errors.ENCP_STUCK, msg, error_source=NETWORK)
                            return
        ...
        Trace.trace(20, "BEFORE: STATE %s udp_sent %s" %
                    (state_name(self.state), self.udp_cm_sent))
        Trace.trace(20, "send_update_cnt %s method %s" %
                    (self.send_update_cnt, self.method))
        send_rq = 1
        use_state = 1
            ...
            for lib, addr in libraries:
                if use_state:
                    ticket = self.format_lm_ticket(
                        state=state, error_source=error_source)
                else:
                    ticket = self.format_lm_ticket(
                        state=self.state, error_source=error_source)
                ...
                if ((thread_name == 'MainThread') and
                    ...
                    # XXX Sasha - this is an experiment - not sure this is a
                    # good idea!
                    if addr != self.lm_address and self.state == HAVE_BOUND:
                        ...
                        if addr == self.udp_control_address and ticket['work'] == 'mover_busy':
                            ...
                            try:
                                ...
                                request_from_lm = self.udpc.send(
                                    ticket, addr, rcv_timeout=to, max_send=retry)
                                #request_from_lm = self.udpc.send(ticket, addr, rcv_timeout=30)
                                Trace.trace(
                                    41, "Request turn around time %s" %
                                    (time.time() - t0,))
                                ...
                                if exc == errno.errorcode[errno.ETIMEDOUT]:
                                    ...
                                    if addr == self.udp_control_address:
                                        ...
                                        if not msg:
                                            msg = "%s sending to %s" % (
                                                e_errors.TIMEDOUT, addr)
                                        Trace.log(
                                            e_errors.INFO, "ENCP_GONE1 %s" %
                                            (msg,))  # remove after fixing ENCP_GONE
                                        self.transfer_failed(
                                            e_errors.ENCP_GONE, msg, error_source=NETWORK)
    ...
    def check_dismount_timer(self):
        ...
        if self.state is HAVE_BOUND and self.dismount_time and now > self.dismount_time:
            ...
            self.unlock_state()
            Trace.trace(
                10, "Dismount time expired %s" %
                (self.current_volume,))
            Trace.log(
                e_errors.INFO, "Trying to dismount volume (2) %s" %
                (self.current_volume,))
            self.run_in_thread(
                'media_thread',
                self.dismount_volume,
                after_function=self.idle)
        else:
            self.unlock_state()

    def send_error_msg(self, error_info=(None, None),
                       error_source=None, returned_work=None):
        """
        ...
        if self.lm_address != (
                'none', 0):  # send error message only to LM that called us
    ...
    def return_work_to_lm(self, ticket):
        ...
        ticket = self.format_lm_ticket(state=ERROR,
                                       error_info=(
                                           e_errors.MOVER_BUSY, state_name(
                                               self.state)),
    ...
    def read_client(self):
        ...
        Trace.trace(
            8, "read_client starting,  bytes_to_read=%s" %
            (self.bytes_to_read,))
        ...
        while self.state in (ACTIVE,) and self.bytes_read < self.bytes_to_read:
            ...
            try:
                ...
                exc, detail, tb = sys.exc_info()
                msg = "ENCP_GONE(2) exc %s detail %s. bytes_read %s" % (
                    exc, detail, bytes_read)
                #Trace.handle_error(exc, detail, tb)
                self.transfer_failed(
                    e_errors.ENCP_GONE, msg, error_source=NETWORK)
                return
        ...
        if self.bytes_read == self.bytes_to_read:
            if self.trailer:
                ...
                while trailer_bytes_read < self.buffer.trailer_size:
                    ...
                    bytes_read = self.buffer.stream_read(
                        bytes_to_read, trailer_driver)
                    trailer_bytes_read = trailer_bytes_read + bytes_read
                    Trace.trace(
                        8, "read %s bytes of trailer" %
                        (trailer_bytes_read,))
            if self.tr_failed:
                driver.close()
                return
        ...
        self.bytes_read_last = self.bytes_read
        Trace.trace(
            8, "read_client exiting, read %s/%s bytes" %
            (self.bytes_read, self.bytes_to_read))
    ...
    def position_for_crc_check(self):
        ...
        try:
            have_tape = self.tape_driver.open(
                self.device, self.mode, retry_count=30)
            self.tape_driver.set_mode(
                compression=self.compression, blocksize=0)

        except self.ftt.FTTError as detail:
            Trace.alarm(
                e_errors.ERROR,
                "Supposedly a serious problem with tape drive while checking a written file: %s %s" %
                (self.ftt.FTTError,
                 detail))
            self.vcc.set_system_readonly(self.current_volume)
            Trace.alarm(
                e_errors.ERROR, "Serious FTT error %s on %s. Volume is set readonly" %
                (detail, self.current_volume))
            self.transfer_failed(
                e_errors.WRITE_ERROR, "Serious FTT error %s" %
            ...
            self.set_volume_noaccess(
                self.current_volume,
                "Write error. See log for details")
        ...
        if have_tape != 1:
            ...
            self.transfer_failed(
                ...
                "error positioning tape for selective CRC check",
                error_source=DRIVE)
            return
        try:
            ...
            Trace.alarm(e_errors.ERROR, "error positioning tape %s for selective CRC check. Position %s. Volume is set readonly" %
                        (self.current_volume, save_location))
            self.transfer_failed(
                e_errors.POSITIONING_ERROR, 'positioning error %s' %
                (detail,), error_source=DRIVE)
            # also set volume to NOACCESS, so far
            # no alarm is needed here because it is send by volume clerk
            # when it sets a volume to NOACCESS
            self.set_volume_noaccess(
                self.current_volume,
                "Positioning error. See log for details")
            # log all running proceses
            self.log_processes(logit=1)
            return
    ...
    def client_update_enabled(self, ticket):
        ...
        rc = False
        c_legal_version = enstore_functions2.convert_version(
            legal_encp_version)
    ...
    def send_client_update(self):
        ...
        if self.crc_check_percent_completed <= 0:
            ...
            # self.crc_check_percent_complete can not be 0. If it is, encp
            # interrupts transfer.

            self.crc_check_percent_completed -= 1
        Trace.trace(
            20, "send_client_update: percent done %s" %
            (self.crc_check_percent_completed,))
        self.current_work_ticket['status'] = (
            e_errors.MOVER_BUSY, self.crc_check_percent_completed)
        Trace.log(e_errors.INFO, "Sending update to client: %s" %
                  (self.current_work_ticket['status'],))
        try:
            callback.write_tcp_obj(
                self.control_socket,
                self.current_work_ticket)
        except BaseException:
            exc, detail, tb = sys.exc_info()
            Trace.log(
                e_errors.ERROR, "error in send_client_update: %s" %
                (detail,))
    ...
    def selective_crc_check(self):
        ...
        self.bytes_read = self.bytes_read_last = 0
        Trace.trace(
            20, "selective_crc_check: header size %s" %
        ...
        Trace.trace(
            22, "selective_crc_check: starting check after write, bytes_to_read=%s" %
            (bytes_to_read,))
        driver = self.tape_driver
        first_block = 1
        block_counter = 0
        total_block_counter = 0
        ...
        while self.bytes_read < bytes_to_read:
            ...
            try:
                ...
                if detail == e_errors.CRC_ERROR:
                    ...
                    self.transfer_failed(
                        e_errors.WRITE_ERROR, detail, error_source=DRIVE)
                else:
            ...
            if block_counter == 2000:
                self.crc_check_percent_completed = int(
                    self.bytes_read * 100 / self.bytes_to_write)
                ...
                block_counter += 1
            Trace.trace(
                22, "selective_crc_check: bytes_read %s bytes_read_last %s" %
                (self.bytes_read, self.bytes_read_last))
        # end while
        Trace.trace(
            22, "selective_crc_check: total blocks %s" %
            (total_block_counter,))
        ...
        if self.read_errors > 0:
            # number of read error per current file
            self.read_errors = self.read_errors - read_errors_1
        if self.write_errors > 0:
            # number of read error per current file
            self.write_errors = self.write_errors - write_errors_1

        # if self.buffer.read_stats[4] > 0.:
        #    rates = bytes_read/self.buffer.read_stats[4]
        if self.buffer.read_stats[1] > 0.:
            # read rates are file_size / time_in_driver.read
            rates = self.bytes_written / self.buffer.read_stats[1]
        else:
            rates = 0.
    ...
    def write_tape(self):
        ...
        Trace.log(
            e_errors.INFO, "write_tape starting, bytes_to_write=%s" %
            (self.bytes_to_write,))
        Trace.trace(8, "bytes_to_transfer=%s" % (self.bytes_to_transfer,))
        driver = self.tape_driver
        if self.config['product_id'] in (
                "T10000C", "T10000D") and self.compression:
            # special code for setting compression for T10000C tape drives
            rc = scsi_mode_select.t10000_set_compression(
                driver, compression=True)
            if not rc:
        ...
        Trace.log(
            e_errors.INFO, 'Write starting. Tape %s absolute location in blocks %s' %
            (self.current_volume, self.bloc_loc))
        ...
        if self.driver_type == 'FTTDriver':
            if self.write_counter == 0:  # this is a first write since tape has been mounted
                ...
                if self.initial_abslute_location == 0:
                    # tape is at BOT
                    # something wrong with positioning.
                    self.transfer_failed(
                        e_errors.WRITE_ERROR, "Tape %s at BOT, can not write" %
                        (self.current_volume,), error_source=TAPE)
                    self.set_volume_noaccess(
                        self.current_volume, "Tape is at BOT, can not write")
                    return
            else:
                Trace.trace(
                    31,
                    "cur %s, initial %s, last %s" %
                    (self.bloc_loc,
                     self.initial_abslute_location,
                     self.last_absolute_location))
                if (self.bloc_loc <= self.initial_abslute_location) or (
                        self.bloc_loc != self.last_absolute_location):
                    Trace.alarm(
                        e_errors.ERROR, "Write error on %s. Wrong position. See log for details" %
                        (self.current_volume,))
                    self.log_processes(logit=1)
                    ...
                    self.set_volume_noaccess(
                        self.current_volume, "Wrong position. See log for details")
                    return
        ...
        try:
            bytes_written = self.net_driver.write("B",  # write anything
                                                  0,
                                                  1)  # just 1 byte
        except generic_driver.DriverError as detail:
            Trace.log(
                e_errors.INFO, "ENCP_GONE3 detail %s. bytes_written %s" %
                (detail, bytes_written))  # remove after fixing ENCP_GONE
            self.transfer_failed(
                e_errors.ENCP_GONE,
                detail,
                error_source=NETWORK)
            ...
            Trace.log(
                e_errors.INFO, "ENCP_GONE4 exc %s detail %s. bytes_written %s" %
                (exc, detail, bytes_written))  # remove after fixing ENCP_GONE
            self.transfer_failed(
                e_errors.ENCP_GONE,
                detail,
                error_source=NETWORK)
        ...
        if self.header_labels:
            ...
            try:
                ...
                if self.driver_type == 'FTTDriver':
                    import ftt
                    self.ftt._ftt.ftt_set_last_operation(
                        self.tape_driver.ftt.d, 0)
                # initiate cleaning
                self.force_clean = 1
                self.transfer_failed(
                    e_errors.WRITE_ERROR, detail, error_source=TAPE)
                return
            ...
            try:
                ...
                self.transfer_failed(
                    e_errors.WRITE_ERROR, detail, error_source=TAPE)
                return
            self.media_transfer_time = self.media_transfer_time + \
                (time.time() - t1)
        ...
        while self.state in (
                ACTIVE, ) and self.bytes_written < self.bytes_to_write:
            loop_start = time.time()

            Trace.trace(
                133, "total_bytes %s total_bytes_written %s" %
                (self.bytes_to_write, self.bytes_written))
            if self.tr_failed:
            ...
            if (empty or
                ...
                now = time.time()
                if (int(now - buffer_empty_t) >
                        self.max_time_in_state) and int(buffer_empty_t) > 0:
                    if not hasattr(self, 'too_long_in_state_sent'):
                        if not self.state == ERROR:
                            ...
                            Trace.log(
                                e_errors.INFO,
                                "write:now %s t %s max %s empty %s defer %s br %s btr %s" %
                                (now,
                                 buffer_empty_t,
                                 self.max_time_in_state,
                                 empty,
                                 defer_write,
                                 self.bytes_read,
                                 self.bytes_to_read))  # !!! REMOVE WHEN PROBLEM is fixed
                            self.too_long_in_state_sent = 0  # send alarm just once
                        else:
                            return
                    ...
                    buffer_empty_t = now
                    Trace.trace(
                        9, "buf empty cnt %s max %s" %
                        (buffer_empty_cnt, self.max_in_state_cnt))
                    if buffer_empty_cnt >= self.max_in_state_cnt:
                        msg = "data transfer from client stuck. Client host %s. Breaking connection" % (
                            self.current_work_ticket['wrapper']['machine'][1],)
                        self.tape_driver.flush()  # to empty buffer and to release device from this thread
                ...
                if (defer_write and (self.bytes_read ==
                                     self.bytes_to_read or not self.buffer.low())):
                    defer_write = 0
                continue
            else:
                buffer_empty_t = 0
            ...
            try:
                ...
                nblocks = nblocks + 1
                Trace.trace(
                    133, "bytes_to_write %s bytes_written %s" %
                ...
                if str(detail) == 'FTT_ENOSPC':
                    ...
                    if ret['status'][0] != e_errors.OK or ret['eod_cookie'] != self.vol_info['eod_cookie']:
                        Trace.alarm(
                            e_errors.ERROR, "set_remaining_bytes failed", ret)
                        self.set_volume_noaccess(
                            self.current_volume,
                            "Failed to update volume information. See log for details")
                    ...
                    # when it sets a volume to NOACCESS
                    self.set_volume_noaccess(
                        self.current_volume, "Write error. See log for details")
                    # log all running proceses
                    self.log_processes(logit=1)
        ...
        Trace.log(
            e_errors.INFO, "written bytes %s/%s, blocks %s header %s trailer %s" %
            (self.bytes_written, self.bytes_to_write, nblocks, len(
                self.header), len(
                self.trailer)))

        if failed:
            # to empty buffer and to release device from this thread
            rc = self.tape_driver.flush()
            if rc[0] == -1:
                self.transfer_failed(
                    e_errors.WRITE_ERROR, "Serious FTT error %s" %
                    (rc[1],), error_source=DRIVE)
            return
        ...
        if self.tr_failed:
            Trace.log(
                e_errors.ERROR,
                "Transfer failed just before writing file marks")
            self.tape_driver.flush()  # to empty buffer and to release devivice from this thread
            # AM: DO NOT REWIND TAPE
            # if self.rewind_tape:
            ##    Trace.log(e_errors.INFO, "To avoid potential data overwriting will rewind tape");
        ...
        if self.bytes_written == self.bytes_to_write:
            try:
                ...
                # of them.
                self.eof_labels = self.wrapper.eof_labels(
                    self.buffer.complete_crc)
                if self.single_filemark or self.eof_labels:
                    Trace.trace(
                        23, "single fm %s eof labels %s" %
                        (self.single_filemark, self.eof_labels))
                ...
                if self.eof_labels:
                    bytes_written = driver.write(
                        self.eof_labels, 0, len(self.eof_labels))
                    if bytes_written != len(self.eof_labels):
                        ...
                        Trace.alarm(
                            e_errors.ERROR, "Short write on %s. Volume is set readonly. See log for details" %
                            (self.current_volume,))
                        # also set volume to NOACCESS, so far
                        # no alarm is needed here because it is send by volume clerk
                        # when it sets a volume to NOACCESS
                        self.set_volume_noaccess(
                            self.current_volume, "Short write. See log for details")
                        # log all running proceses
                        self.log_processes(logit=1)
                        return
                    ...
                    if not self.single_filemark:
                        ...
                        self.tape_driver.skipfm(-1)
                        Trace.trace(23, "fm done")
                # get location info before calling tape_driver.flush() as it
                # will clear stats
                ...
                if self.driver_type == 'FTTDriver':
                    ...
                    if self.read_errors > 0:
                        # number of read error per current file
                        self.read_errors = self.read_errors - read_errors_0
                    if self.write_errors > 0:
                        # number of read error per current file
                        self.write_errors = self.write_errors - write_errors_0

                    Trace.log(
                        e_errors.INFO, 'filemarks written. Tape %s absolute location in blocks %s' %
                        (self.current_volume, new_bloc_loc,))
                    ...
                    if self.buffer.write_stats[1] > 0.:
                        # write rates are file_size / time_in_driver.write
                        rates = self.bytes_written / self.buffer.write_stats[1]
                    else:
                        rates = 0.
                    ...
                    self.media_transfer_time = self.media_transfer_time + \
                        (time.time() - t1)  # include filemarks into drive time
                    Trace.trace(
                        31, "cur %s, initial %s, last %s, blocks %s, headers %s trailers %s" %
                        (new_bloc_loc, self.initial_abslute_location, self.current_absolute_location, self.last_blocks_written, len(
                            self.header_labels), len(
                            self.eof_labels)))
                ...
                if self.header_labels and self.eof_labels:
                    ...
                    extra = 0
                if self.driver_type == 'FTTDriver':
                    if new_bloc_loc != self.current_absolute_location + \
                            self.last_blocks_written + 1 + extra:
                ...
                Trace.alarm(
                    e_errors.ERROR,
                    "Write error on %s. Volume is set readonly. See log for details" %
                    (self.current_volume,
                     ))
                Trace.handle_error(exc, detail, tb)
                self.transfer_failed(
            ...
            if self.check_written_file() and self.driver_type == 'FTTDriver':
                Trace.log(
                    e_errors.INFO,
                    "selective CRC check after writing file")
                self.crc_check_percent_completed = 0
                # send more often than encp retry interval (15 min)
                self.add_interval_func(self.send_client_update, 14 * 60)
    ...
    def read_tape(self):
        ...
        Trace.log(
            e_errors.INFO, "read_tape starting, bytes_to_read=%s" %
            (self.bytes_to_read,))
        ...
        while self.state in (
                ACTIVE, ) and self.bytes_read < self.bytes_to_read:
            loop_start = time.time()
            Trace.trace(
                133, "total_bytes_to_read %s total_bytes_read %s" %
                (self.bytes_to_read, self.bytes_read))
            ...
            if self.buffer.full():
                ...
                Trace.trace(9, "read_tape: time in state %s max %s, bf %s sent %s bf_c %s m_c %s" %
                            (int(now - buffer_full_t), self.max_time_in_state,
                             int(buffer_full_t), hasattr(
                                 self, 'too_long_in_state_sent'),
                ...
                if (((int(now - buffer_full_t) > self.max_time_in_state) and int(buffer_full_t) > 0) or
                    ...
                    if (buffer_full_cnt >= self.max_in_state_cnt) or network_slow:
                        msg = "data transfer to client stuck. Client host %s. Breaking connection" % (
                            self.current_work_ticket['wrapper']['machine'][1],)
            ...
            try:
                ...
                idle_time = idle_time + t1 - loop_start
                Trace.trace(
                    133, "bytes to read %s, bytes read %s" %
                    (nbytes, bytes_read))
            except MemoryError:
                # raise exceptions.MemoryError # to test this thread
                exc, detail, tb = sys.exc_info()
                ...
                if self.method == 'read_next':
                    ...
                    if detail == 'FTT_SUCCESS':
                        ...
                        Trace.log(e_errors.INFO, "perhaps EOT.Current Location %s Previous location %s" %
                                  (self.current_location, prev_loc))
                        Trace.log(
                            e_errors.INFO, 'ftt errors %s' %
                            (self.ftt._ftt.ftt_get_error(),))
                        self.read_tape_done.set()
                        self.transfer_failed(
                            e_errors.READ_ERROR, e_errors.READ_EOD, error_source=TAPE)
                        failed = 1
                        break
                        ...
                        self.transfer_failed(
                            e_errors.READ_ERROR, detail, error_source=TAPE)
                        failed = 1
                        break
                    ...
                    Trace.trace(
                        33, "Exception %s %s" %
                        (e_errors.READ_ERROR, str(detail)))
                    self.read_tape_done.set()
            ...
            if self.bytes_read == 0:  # Handle variable-sized cpio header
                ...
                try:
                    ...
                    self.transfer_failed(
                        e_errors.READ_ERROR, "%s" %
                        (detail,), error_source=TAPE)
                    failed = 1
                    break
                if len(b0) >= self.wrapper.min_header_size:
                    try:
                        ...
                        Trace.log(e_errors.ERROR, "Invalid header %s" %
                                  (b0[:self.wrapper.min_header_size]))
                        self.read_tape_done.set()
                        self.transfer_failed(
                            e_errors.READ_ERROR,
                            "Invalid file header",
            ...
            if bytes_read < nbytes and self.method == 'read_next':
                ...
                if bytes_read == 0:
                    ...
                    Trace.log(
                        e_errors.INFO, "location %s block %s cur_loc %s" %
                        (location, block, self.current_location))
                    break_here = 1
                    break
        ...
        Trace.log(
            e_errors.INFO, "read bytes %s/%s, blocks %s header %s" %
            (self.bytes_read, self.bytes_to_read, nblocks, header_size))
        try:
            location, block = self.tape_driver.tell()
        except self.ftt.FTTError as detail:
            self.transfer_failed(e_errors.READ_ERROR, 'Can not get drive info %s' % (detail,),
        ...
        block_n = tot_blocks = bloc_loc = block_size = bot = 0
        if self.driver_type == 'FTTDriver':
            self.read_errors = -1
            self.write_errors = -1
            self.update_tape_stats()
            ...
            if self.buffer.read_stats[1] > 0.:
                # read rates are file_size / time_in_driver.read
                rates = self.bytes_read / self.buffer.read_stats[1]
            else:
                rates = 0.
        ...
        if do_crc:
            ...
            if self.buffer.complete_crc != complete_crc:
                ...
                if complete_crc is None:

                    sanity_cookie = (
                        self.buffer.sanity_bytes,
                        self.buffer.sanity_crc)
                    if self.method == 'read_next' and bfid is None:
                        # must be tape ingest case
                        if self.bytes_to_write != self.bytes_read:
                            self.bytes_to_write = self.bytes_read
                        Trace.log(e_errors.WARNING,
                                  "updating file db entry for the tape ingest")
                        ...
                        self.file_info['drive'] = "%s:%s" % (
                            self.current_work_ticket['mover']['device'], self.config['serial_num'])
                        self.file_info['pnfsid'] = None
                        # it may later come in get ticket
                        self.file_info['pnfs_name0'] = None
                        ...
                        Trace.log(
                            e_errors.INFO, "updated file info %s" %
                            (ret,))  # Remove when fixed
                        if ret['status'][0] != e_errors.OK:
                            Trace.log(
                                e_errors.ERROR, "cannot assign new bfid %s" %
                                (ret,))
                            self.read_tape_done.set()
                            self.transfer_failed(
                                e_errors.ERROR, "Cannot assign new bit file ID")
                            return
                        if ret['fc']['bfid'] is None:
                            Trace.log(
                                e_errors.ERROR, "FC returned None for bfid %s" %
                                (ret,))
                            self.read_tape_done.set()
                            self.transfer_failed(
                                e_errors.ERROR, "FC returned None for bfid")
                        ...
                        Trace.trace(
                            98, "updated file db %s" %
                            (self.current_work_ticket['fc'],))
                        # update volume DB and volume info
                        self.vcc.update_counts(
                            self.current_volume, wr_access=1)
                        self.vol_info['eod_cookie'] = loc_to_cookie(
                            self.current_location)
                        ...
                        if self.driver_type == 'FTTDriver' and self.rem_stats:
                            ...
                            if remaining is not None:
                                ...
                                self.vol_info['remaining_bytes'] = remaining * 1024
                        ...
                        if ret['status'][0] != e_errors.OK:
                            ...
                            Trace.alarm(e_errors.ALARM, "Failed to update volume information on %s, EOD %s. May cause a data loss." %
                                        (self.current_volume, self.vol_info['eod_cookie']))
                            self.transfer_failed(
                                ret['status'][0], ret['status'][1], error_source=TAPE)
                            return
                        self.vol_info.update(
                            self.vcc.inquire_vol(
                                self.current_volume))
                        self.current_work_ticket['vc'].update(self.vol_info)
                        Trace.trace(
                            98, "updated volume db %s" %
                            (self.current_work_ticket['vc'],))
                    ...
                    if crc_error:
                        ...
                        self.read_tape_done.set()
                        self.transfer_failed(
                            e_errors.CRC_ERROR, error_source=TAPE)
                        return
    ...
    def write_client(self):
        ...
        Trace.trace(
            8, "write_client starting, bytes_to_write=%s" %
            (self.bytes_to_write,))
        ...
        if self.bytes_to_write > 0 and self.bytes_written == 0 and self.wrapper and self.wrapper.__name__ != "null_wrapper":  # Skip over cpio or other headers
            while self.buffer.header_size is None and self.state in (ACTIVE, ):
                Trace.trace(
                    8, "write_client: waiting for read_tape to set header info")
                self.buffer.write_ok.clear()
                self.buffer.write_ok.wait(1)
            ...
            Trace.trace(
                8, "write_client: discarded %s bytes of header info" %
                (self.buffer.header_size))
        failed = 0
        ...
        while True:
            ...
            Trace.trace(133, "bytes_written %s bytes to write %s" %
                        (self.bytes_written, self.bytes_to_write))
            if self.state in (
                    ACTIVE, ) and self.bytes_written < self.bytes_to_write:
                    ...
                    if detail == e_errors.CRC_ERROR:
                        Trace.alarm(e_errors.WARNING, "CRC error in write client",
                                    {'outfile': self.current_work_ticket['outfile'],
                                     ...
                                     'external_label': self.current_work_ticket['vc']['external_label']})
                        self.transfer_failed(
                            e_errors.CRC_ERROR_IN_WRITE_CLIENT, error_source=TAPE)
                    else:
                ...
                if bytes_written < 0:
                    self.transfer_failed(
                        e_errors.ENCP_GONE, "write returns %s" %
                        (bytes_written,))
                    failed = 1
                    break
                if bytes_written != nbytes:
                    Trace.trace(
                        22, "write_client: !!! bytes written %s bytes to write %s" %
        ...
        Trace.log(e_errors.INFO, "write_client: wrote %s/%s bytes" %
                  (self.bytes_written, self.bytes_to_write))
        ...
        if self.bytes_written == self.bytes_to_write:
            # check crc
            # we do not calculate crc for 0 length file.
            if do_crc and self.file_info['size'] != 0:
        ...
        if self.mover_type != enstore_constants.DISK_MOVER and self.read_tape_done.wait(
                self.read_error_recovery_timeout):
            Trace.log(
                e_errors.INFO,
                "write_client waits for tape thread to exit it may take up to %s min" %
                (self.read_error_recovery_timeout /
                 60,
                 ))
            ...
            Trace.log(
                e_errors.INFO,
                "write_client detected tape theard termination signal")
        else:
            Trace.log(
                e_errors.ERROR,
                "write_client exits, but tape thread termination signal was not recieved")
    ...
    def write_to_hsm(self, ticket):
        ...
        Trace.log(
            e_errors.INFO,
            "WRITE_TO_HSM. Mover state %s" %
            (state_name(
                self.state),
             ))
        Trace.trace(10, "State %s" % (state_name(self.state),))
        if 'copy' in ticket and 'original_bfid' not in ticket['fc']:
            # this is a file copy request
            self.transfer_failed(
                e_errors.ERROR,
                "Cannot assign new bit file ID. No original_bfid key in ticket")
            return
    ...
    def update_volume_info(self, ticket):
        ...
        if not self.vol_info:
            ...
            if self.vol_info['external_label'] is not ticket['external_label']:
                Trace.log(
                    e_errors.ERROR,
                    "Library manager asked to update iformation for the wrong volume: %s, current %s" %
                    (ticket['external_label'],
    ...
    def setup_transfer(self, ticket, mode):
        ...
        for thread in threads:
            if thread.isAlive():
                ...
                Trace.trace(
                    87, "setup_transfer: Thread %s is dead" %
                    (thread_name,))
        ...
        Trace.trace(24, "setup_transfer: save_state %s" %
                    (state_name(self.save_state),))
        self.udp_cm_sent = 0
        ...
        # NB: encp v2_5 supplies this information for writes but not reads.
        # Somebody fix this!
        try:
        ...
        if self.save_state not in (IDLE, HAVE_BOUND):
            Trace.log(e_errors.ERROR, "Not idle %s" %
                      (state_name(self.state),))
        ...
        if (self.save_state == HAVE_BOUND and self.single_filemark and self.mode == WRITE and self.setup_mode == READ
            ...
            Trace.log(
                e_errors.INFO,
                "writing a tape mark before switching to READ")
            if self.driver_type == 'FTTDriver':
                bloc_loc = 0
                if self.write_in_progress:
                    ...
                    try:
                        ...
                        exc, detail, tb = sys.exc_info()
                        self.transfer_failed(
                            e_errors.POSITIONING_ERROR, 'positioning error %s %s' %
                            (exc, detail,), error_source=DRIVE)
                        self.unlock_state()
                        return
                ...
                if self.bloc_loc != self.last_absolute_location:
                    ...
                    # when it sets a volume to NOACCESS
                    self.set_volume_noaccess(
                        self.current_volume, "Positioning error. See log for details")
                    # log all running proceses
                    self.log_processes(logit=1)
                    self.unlock_state()
                    return
                try:
                    ...
                    # in this case tape is in the right position for the next
                    # write
                    self.tape_driver.skipfm(-1)
                    ...
                    Trace.alarm(
                        e_errors.ERROR,
                        "error writing file mark, will set volume readonly")
                    # also set volume to NOACCESS, so far
                    # no alarm is needed here because it is send by volume clerk
                    # when it sets a volume to NOACCESS
                    self.set_volume_noaccess(
        ...
        ticket['mover']['device'] = "%s:%s" % (
            self.config['host'], self.config['device'])
    ...
    def check_connection(self):
        ...
        while self.mode == ASSERT:
            Trace.trace(40, "check_connection mode %s" %
                        (mode_name(self.mode),))
            try:
                if self.control_socket:
                    r, w, ex = select.select([self.control_socket], [
                                             self.control_socket], [], 10)
                    Trace.trace(40, "check_connection1 %s %s %s" % (r, w, ex))
                    Trace.trace(40, "r= %s" % (r,))
                    if r:
                        # r - read socket appears when client connection gets
                        # closed
                        Trace.trace(
                            40, "media_validate= %s" %
                            (self.media_validate,))
                        if self.media_validate:
                            ...
                            scsi_mode_select.ftt_scsi_verify(
                                self.tape_driver, VLBPM, STOP_MEDIA_VALIDATION)
            ...
            if self.media_validate:
                ...
                if ret[0]:
                    Trace.log(
                        DEBUG_LOG, "check_scsi_verify %s %s %s" %
                        (ret[1], ret[2], percent_completed))
                    if ret[1]:
                    ...
                    if percent_completed != ret[2]:
                        ...
                        self.bytes_read = int(
                            self.vol_info['active_bytes'] / 100. * percent_completed)
                        # hack to not alarm false transfer stuck
                        self.bytes_read_last = self.bytes_read - 1
    ...
    def assert_vol(self):
        ...
        for retry_open in range(3):
            ...
            have_tape = self.tape_driver.open(
                self.device, READ, retry_count=30)
            ...
            if have_tape == 1:
                ...
                try:
                    ...
                    err = r
                    Trace.log(
                        e_errors.INFO, "rewind/retry: mt rewind returns %s, status %s" %
                        (r, s))
                    if s:
                        self.transfer_failed(
                            e_errors.MOUNTFAILED, 'mount failure: %s' %
                            (err,), error_source=ROBOT)
                        Trace.log(
                            e_errors.INFO, "Trying to dismount volume (3) %s" %
                            (self.current_volume,))
                    ...
                    err = detail
                    Trace.log(
                        e_errors.ERROR, "rewind/retry: %s %s" %
                        (exc, detail))
        else:
            self.transfer_failed(
                e_errors.MOUNTFAILED, 'mount failure: %s' %
                (err,), error_source=ROBOT)
            Trace.log(
                e_errors.INFO, "Trying to dismount volume (4) %s" %
                (self.current_volume,))
            self.dismount_volume(after_function=self.idle)
            return
        eod = self.vol_info['eod_cookie']
        status = self.tape_driver.verify_label(
            ticket['vc']['external_label'], READ)
        if status[0] != e_errors.OK:
            if status[0] == e_errors.READ_VOL1_READ_ERR and eod == 'none':
                Trace.log(
                    e_errors.INFO, "Trying to dismount volume (5) %s" %
                    (self.current_volume,))
                self.dismount_volume(after_function=self.idle)
                return
        ...
        if 'action' in ticket:
            ...
            if (ticket['action'] == 'crc_check'):
                ...
                if 'parameters' in ticket and isinstance(
                        ticket['parameters'], type([])) and ticket['parameters']:
                    # Initialize return list
                    for lc in ticket['parameters']:
                        rec = info_c.find_file_by_location(
                            ticket['vc']['external_label'], lc)
                        Trace.trace(
                            24, "find_file_by_location returned %s" %
                            (rec,))
                        if rec['status'][0] != e_errors.OK:
                            self.transfer_failed(
                                rec['status'][0], rec['status'][1])
                            return
                        if 'file_list' in rec:
                            ...
                            for entry in rec['file_list']:
                                ticket['return_file_list'][entry['location_cookie']
                                                           ] = e_errors.UNKNOWN
                    ...
                    try:
                        ...
                        if file_list['status'][0] != e_errors.OK:
                            self.transfer_failed(
                                file_list['status'][0], file_list['status'][1])
                            return
                    except BaseException:
                        exc, msg, tb = sys.exc_info()
                ...
                keys = sorted(ticket['return_file_list'].keys())
                ...
                for loc_cookie in keys:
                    ...
                    self.current_work_ticket = ticket
                    Trace.trace(24, "t1 %s" %
                                (type(self.current_work_ticket),))
                    Trace.trace(24, "t11 %s" %
                                (type(self.current_work_ticket['fc']),))
                    Trace.trace(24, "t2 %s" % (type(file_info),))
                    Trace.trace(24, "t21 %s" %
                                (type(file_info.get(loc_cookie)),))
                    ...
                    if self.tr_failed:
                        ...
                        Trace.trace(
                            24, "ticket!: %s" %
                            (ticket['return_file_list'][loc_cookie],))
                        break
    ...
    def finish_transfer_setup(self):
        ...
        """
        Trace.trace(10, "client connect returned: %s %s" %
                    (self.control_socket, self.client_socket))
        Trace.trace(
            24, "finish_transfer_setup %s %s" %
            (mode_name(
                self.mode), mode_name(
                self.setup_mode)))
        ticket = self.current_work_ticket
        if not self.client_socket:
            if self.mode != ASSERT:
                Trace.trace(
                    20, "finish_transfer_setup: connection to client failed")
        ...
        # the default setting for read, assert: check crc before writing to
        # network in net thread
        ...
        self.current_work_ticket = ticket
        if 'delayed_dismount' in self.current_work_ticket['encp']:
            if ((isinstance(self.current_work_ticket['encp']['delayed_dismount'], type(0))) or
                    (isinstance(self.current_work_ticket['encp']['delayed_dismount'], type(0.)))):
                delay = 60 * \
                    self.current_work_ticket['encp']['delayed_dismount']
            else:
        ...
        vcc_address_family = socket.getaddrinfo(
            self.vcc.server_address[0], None)[0][0]
        fcc_address_family = socket.getaddrinfo(
            self.fcc.server_address[0], None)[0][0]
        client_reported_vcc_address_family = socket.getaddrinfo(
            vc['address'][0], None)[0][0]
        client_reported_fcc_address_family = socket.getaddrinfo(
            fc['address'][0], None)[0][0]
        try:
        ...
        if volume_label:
            ...
            Trace.log(
                e_errors.ERROR, "setup_transfer: volume label=%s" %
                (volume_label,))
        ...
        if self.vol_info['status'][0] != e_errors.OK:
            ...
            Trace.log(
                e_errors.ERROR, "Volume clerk reply %s %s" %
                (self.vol_info['status'][0], msg,))
            self.transfer_failed(
                msg[0],
                msg[1],
                error_source=TAPE,
        ...
        try:
            ...
            exc, msg, tb = sys.exc_info()
            Trace.log(
                e_errors.ERROR, "error importing wrapper: %s %s" %
                (exc, msg))

        if not self.wrapper:
            msg = e_errors.INVALID_WRAPPER, "Illegal wrapper type %s" % (
                self.wrapper_type)
        ...
        client_filename = self.current_work_ticket['wrapper'].get(
            'fullname', '?')
        pnfs_filename = self.current_work_ticket['wrapper'].get(
            'pnfsFilename', '?')
        ...
        if bytes is None:
            ...
            if self.method == "read_next":
                # we do not know what is the file size, so let's assume that it
                # is big
        ...
        if (self.bytes_to_transfer is None) or (self.bytes_to_transfer < 0):
            self.transfer_failed(
                e_errors.BAD_FILE_SIZE, "bad file size is %s" %
                (self.bytes_to_transfer,), error_source=USER, dismount_allowed=0)
        ...
        if self.wrapper:
            ...
            if self.mode != ASSERT:
                self.wrapper_ticket = self.wrapper.create_wrapper_dict(
                    self.current_work_ticket)
        if self.mode == READ:
            ...
            if self.wrapper:
                self.header_labels = self.wrapper.hdr_labels(
                    self.wrapper_ticket)
                # note! eof_labels will be called when write is done and
                # checksum is calculated
                self.header, self.trailer = self.wrapper.headers(
                    self.wrapper_ticket)
            else:
                self.header = ''
                self.trailer = ''
            ...
            self.bytes_to_write = self.bytes_to_write + \
                len(self.header) + len(self.trailer)
        ...
        if self.mode == ASSERT:
            return
        #######
        Trace.trace(24, "setup_transfer: save_state %s volume %s cur %s " % (
            state_name(self.save_state), volume_label, self.current_volume))
    ...
    def error(self, msg, err=e_errors.ERROR):
        ...
        Trace.log(
            e_errors.ERROR, "error: %s message: %s state=ERROR" %
            (err, msg))
        self.state = ERROR
    ...
    def position_media(self, verify_label=1):
        ...
        for retry_open in range(3):
            ...
            try:
                ...
                if self.driver_type == 'FTTDriver':
                    Trace.log(
                        DEBUG_LOG, 'tape_driver.open errors (Ok, means no errors) %s' %
                        (self.ftt._ftt.ftt_get_error(),))
            except self.ftt.FTTError as detail:
                Trace.alarm(
                    e_errors.ERROR,
                    "Supposedly a serious problem with tape drive positioning the tape: %s %s." %
                    (self.ftt.FTTError,
                     detail))
                self.transfer_failed(
                    e_errors.POSITIONING_ERROR, "Serious FTT error %s" %
                    (detail,), error_source=DRIVE)
                return
            ...
            if have_tape == 1:
                if self.mode == WRITE and self.tape_driver.mode == READ:
                    Trace.alarm(e_errors.ERROR, "tape %s is write protected, will be set read-only" %
                                (self.current_volume,))
                    self.vcc.set_system_readonly(self.current_volume)
                    self.vcc.set_comment(
                        self.current_volume, "write-protected")
                    ...
                    if thread_name and thread_name == 'media_thread':
                        Trace.log(
                            e_errors.INFO, "Trying to dismount volume (6) %s" %
                            (self.current_volume,))
                        self.dismount_volume(after_function=self.idle)
                    else:
                        Trace.log(
                            e_errors.INFO, "Trying to dismount volume (7) %s" %
                            (self.current_volume,))
                        self.run_in_thread(
                            'media_thread',
                            self.dismount_volume,
                ...
                try:
                    ...
                    if s:
                        self.transfer_failed(
                            e_errors.MOUNTFAILED, 'mount failure: %s' %
                            (err,), error_source=ROBOT, dismount_allowed=0)
                        if self.stop:
                            Trace.alarm(
                                e_errors.INFO, 'Tape %s will stay in the drive for investigation' %
                                (self.current_volume,))
                            self.set_volume_noaccess(
                                self.current_volume, "Rewind retry failed. See log for details")
                            self.offline()
                        else:
                            self.unload_volume(
                                self.vol_info, after_function=self.idle)
                        return
        ...
        if self.mode == WRITE and eod is None:
            verify_label = 0
            label_tape = 1

        if self.mode == WRITE:
            Trace.trace(
                24, "target location %s EOD cookie %s" %
            ...
            if label_tape:
                ...
                status = self.tape_driver.verify_label(None)
                Trace.log(
                    e_errors.INFO, "verify label returned %s" %
                    (status,))
                if status[0] == e_errors.OK:  # There is a label present!
                    msg = "volume %s already labeled %s" % (
                        volume_label, status[1])
                    self.set_volume_noaccess(
                        volume_label, "Volume is already labeled")
                    Trace.alarm(e_errors.ERROR, msg)
                    Trace.log(
                        e_errors.ERROR, "marking %s noaccess" %
                        (volume_label,))
                    self.transfer_failed(
                        e_errors.WRITE_VOL1_WRONG, msg, error_source=TAPE)
                    return 0
                    ...
                    if status[1] == 'FTT_EBLANK':
                        if len(status) > 2:
                            if not ((BLANK_RETURN_PATTERN1 in status[2][0]
                                ...
                                msg = "Unexpected return for blank tape for %s, see log for details" % (
                                    volume_label)
                                self.set_volume_noaccess(
                                    volume_label, "Is it a blank tape of failed system call?")
                                Trace.alarm(e_errors.ERROR, msg)
                                Trace.log(
                                    e_errors.ERROR, "marking %s noaccess" %
                                    (volume_label,))
                        ...
                        msg = "Expected tape label %s, read %s" % (
                            volume_label, status[1])
                        ...
                        self.transfer_failed(
                            e_errors.WRITE_VOL1_WRONG, msg, error_source=TAPE)
                        return 0
                elif status[0] == e_errors.READ_VOL1_MISSING and status[1] is None:
                    pass
                else:
                    msg = "expected return code for blank tape is  %s" % (
                ...
                try:
                    ...
                    if self.driver_type == 'FTTDriver':
                        time.sleep(3)
                        stats = self.tape_driver.get_stats()
                        Trace.trace(10, "WRITE_PROT=%s" %
                                    (stats[self.ftt.WRITE_PROT],))
                        write_prot = stats[self.ftt.WRITE_PROT]
                        if isinstance(write_prot, type('')):
                            write_prot = string.atoi(write_prot)
                        if write_prot:
                            # self.vcc.set_system_noaccess(volume_label)
                            self.set_volume_noaccess(
                                volume_label, "Attempt to label write protected tape")
                            Trace.alarm(
                                e_errors.ERROR, "attempt to label write protected tape")
                            self.transfer_failed(e_errors.WRITE_ERROR,
                                                 "attempt to label write protected tape",
                                                 error_source=TAPE)
                            return 0
                    Trace.log(
                        e_errors.INFO, "labeling new tape %s" %
                        (volume_label,))
    ...
    def transfer_failed(self, exc=None, msg=None,
                        error_source=None, dismount_allowed=1):
        """
        ...
        if exc == e_errors.WRITE_ERROR or exc == e_errors.READ_ERROR or exc == e_errors.POSITIONING_ERROR:
            ...
            if (msg.find("FTT_EIO") != -1):
                ...
                if self.stop:
                    Trace.alarm(
                        e_errors.ERROR,
                        "encountered FTT_EBLANK error. Going OFFLINE. Please check the tape drive. The tape %s will stay in the drive" %
                        (self.current_volume,
                         ))
                    self.set_volume_noaccess(
                        volume_label, "encountered FTT_EBLANK error. See log for details")
                ...
                Trace.alarm(
                    e_errors.ERROR,
                    "encountered FTT_EUNRECOVERED error. Going OFFLINE. Please check the tape drive")
                self.set_volume_noaccess(
                    volume_label, "encountered FTT_EUNRECOVERED error. See log for details")
                self.offline()  # stop here for investigation
                self.net_driver.close()
                self.network_write_active = False  # reset to indicate no network activity
                return
        ...
        if self.state == ERROR:
            Trace.log(
                e_errors.ERROR, "Mover already in ERROR state %s, state=ERROR" %
                (msg,))
            self.tr_failed = False
            return
        ...
        if exc not in (e_errors.ENCP_GONE, e_errors.ENCP_STUCK, e_errors.READ_VOL1_WRONG,
                       e_errors.WRITE_VOL1_WRONG, e_errors.MEMORY_ERROR, e_errors.NET_ERROR):
            if msg.find("FTT_EBUSY") != -1:
                # tape thread stuck in D state - offline mover
                after_dismount_function = self.offline
                Trace.alarm(e_errors.ERROR,
                            "tape thread is possibly stuck in D state")
                self.log_state(logit=1)
            if error_source == DRIVE:
                Trace.log(e_errors.ERROR, "MODE %s" % (self.mode,))
                Trace.alarm(
                    e_errors.ERROR, "Possible drive failure %s %s" %
                    (exc, msg))
                if self.mode == WRITE:
            ...
            if self.consecutive_failures >= self.max_consecutive_failures:
                broken = "max_consecutive_failures (%d) reached" % (
                    self.max_consecutive_failures)
            now = time.time()
            self.error_times.append(now)
            while self.error_times and now - \
                    self.error_times[0] > self.failure_interval:
            ...
            if self.mode == WRITE:
                self.vcc.update_counts(
                    self.current_volume, wr_err=1, wr_access=1)
                ...
                try:
                    ...
                    if remaining <= self.remaining_factor * capacity:
                        ...
                        ret = self.vcc.set_remaining_bytes(
                            self.current_volume, 0, eod, None)
                ...
                if self.vcc:
                    self.vcc.update_counts(
                        self.current_volume, rd_err=1, rd_access=1)
        ...
        Trace.trace(
            26, "current thread %s encp_gone %s" %
            (cur_thread_name, encp_gone))
        ...
        if cur_thread_name:
            ...
            if act_thread_name:
                ...
                for wait in range(60):
                    if thread and thread.isAlive():
                        Trace.trace(
                            26, "thread %s is already running, waiting %s" %
                            (alt_thread_name, wait))
                        time.sleep(1)
                    else:
                        break
        Trace.trace(26, "dismount_allowed %s after_dismount %s" %
                    (dism_allowed, after_dismount_function))
        if encp_gone:
            ...
            try:
                ...
                Trace.alarm(
                    e_errors.ERROR, 'Can not get drive info %s' %
                    (detail,))
                if self.mode == WRITE:
                    self.vcc.set_system_readonly(self.current_volume)
                Trace.log(
                    e_errors.INFO, "Trying to dismount volume (8) %s" %
                    (self.current_volume,))
                self.dismount_volume(after_function=self.idle)
                return
            ...
            if (self.header_labels and
                ...
                # data with fm
                Trace.trace(
                    10, "for cern wrapper will set loc to %s" %
                    (last_location,))
        ...
        if ftt_eio and self.mode != WRITE:
            ...
            # action for tape
            self.set_volume_noaccess(
                volume_label, "FTT_EIO error. See log for details")
        if dism_allowed:
            if exc == e_errors.MEMORY_ERROR:
                ...
                self.dump_vars()
                Trace.log(
                    e_errors.INFO, "Trying to dismount volume (9) %s" %
                    (self.current_volume,))
                self.run_in_thread(
                    'media_thread',
                    self.dismount_volume,
                    after_function=self.restart)
            if self.draining:
                Trace.log(
                    e_errors.INFO, "Trying to dismount volume (10) %s" %
                    (self.current_volume,))
                self.run_in_thread(
                    'media_thread',
                    self.dismount_volume,
                ...
                if not after_dismount_function:
                    if not self.maybe_clean():
                        if cur_thread_name and cur_thread_name == 'media_thread':
                            Trace.log(
                                e_errors.INFO, "Trying to dismount volume (11) %s" %
                                (self.current_volume,))
                            self.dismount_volume(after_function=self.idle)
                        else:
                            Trace.log(
                                e_errors.INFO, "Trying to dismount volume (12) %s" %
                                (self.current_volume,))
                            self.run_in_thread(
                                'media_thread', self.dismount_volume, after_function=self.idle)

                else:
                    if cur_thread_name and cur_thread_name == 'media_thread':
                        Trace.log(
                            e_errors.INFO, "Trying to dismount volume (13) %s" %
                            (self.current_volume,))
                        if not (exc in (e_errors.READ_VOL1_READ_ERR,
                                        e_errors.WRITE_VOL1_READ_ERR)):
                        ...
                        Trace.log(
                            e_errors.INFO, "Trying to dismount volume (14) %s" %
                            (self.current_volume,))
                        self.run_in_thread(
                            'media_thread',
                            self.dismount_volume,
                            after_function=after_dismount_function)
    ...
    def transfer_completed(self, error=None):
        ...
        self.timer('transfer_time')
        Trace.trace(
            24, "transfer_completed. ticket %s" %
            (self.current_work_ticket,))
        ticket = self.current_work_ticket
        if 'times' not in ticket:
            ticket['times'] = {}
        ...
        try:
            ...
            self.transfer_failed(e_errors.DRIVEERROR, 'Can not get drive info %s' % (detail,),
                                 error_source=TAPE)
            Trace.log(
                e_errors.INFO, "Trying to dismount volume (15) %s" %
                (self.current_volume,))
            self.dismount_volume(after_function=self.offline)
            return
        except BaseException:
            ...
            self.transfer_failed(e_errors.DRIVEERROR, 'Can not get drive info %s %s' % (exc, detail,),
                                 error_source=TAPE)
            Trace.log(
                e_errors.INFO, "Trying to dismount volume (16) %s" %
                (self.current_volume,))
            self.dismount_volume(after_function=self.offline)
            return
        ...
        if self.draining:
            Trace.log(
                e_errors.INFO, "Trying to dismount volume (17) %s" %
                (self.current_volume,))
            self.run_in_thread(
                'media_thread',
                self.dismount_volume,
    ...
    def maybe_clean(self):
        ...
        if self.force_clean:
            ...
            try:
                ...
                Trace.alarm(
                    e_errors.ALARM, "Possible Drive problem %s %s" %
                    (self.ftt.FTTError, detail))
                self.set_volume_noaccess(
                    self.current_volume,
                    "Possible drive problem. See log for details")
                if not self.stop:  # otherwise do not dismount for further diagnostics
                    Trace.log(
                        e_errors.INFO, "Trying to dismount volume (18) %s" %
                        (self.current_volume,))
                    self.dismount_volume()
                self.offline()
                return 1
        ...
        if needs_cleaning:
            if not self.do_cleaning:
                Trace.log(
                    e_errors.INFO,
                    "cleaning bit set but automatic cleaning disabled")
                return 0
            ...
            if save_state == HAVE_BOUND:
                Trace.log(
                    e_errors.INFO, "Trying to dismount volume (19) %s" %
                    (self.current_volume,))
                self.dismount_volume()
                save_state = IDLE
            self.state = CLEANING
    ...
    def update_after_writing(self):
        ...
        if self.driver_type == 'FTTDriver' and self.rem_stats:
            ...
            try:
                stats = self.tape_driver.get_stats()
                r2 = long(stats[self.ftt.REMAIN_TAPE]) * 1024
            ...
            if failed:
                ...
                self.set_volume_noaccess(
                    self.current_volume,
                    "ftt error on %s after write. See log for details")
                # log all running proceses
                self.log_processes(logit=1)
                self.transfer_failed(
                    e_errors.ERROR, "ftt.get_stats: FTT_ERROR %s" %
                    (detail,), error_source=DRIVE)
                return 0
        ...
        if (r2 > r1):
            ...
            return 0
        '''
        if r1 <= self.remaining_factor * \
                capacity:  # do not allow remaining capacity to decrease in the "near-EOT" regime
            remaining = min(r1, r2)
        ...
        if self.config['driver'] == 'NullDriver':
            fc_ticket['complete_crc'] = 0
            fc_ticket['sanity_cookie'] = (self.buffer.sanity_bytes, 0)
        ...
        if 'copy' in self.current_work_ticket:
        ...
        Trace.log(e_errors.INFO, "set remaining: %s %s %s" %
                  (self.current_volume, remaining, eod))

        self.state = FINISH_WRITE
        finish_writing = 1
        while finish_writing:
            Trace.trace(26, "sending set_remaining_bytes")
            reply = self.vcc.set_remaining_bytes(
                self.current_volume, remaining, eod, bfid)
            ...
            if reply['status'][0] != e_errors.OK or reply['eod_cookie'] != eod:
                if reply['status'][0] == e_errors.TIMEDOUT:
                    ...
                    Trace.alarm(
                        e_errors.ERROR,
                        "Volume Clerk timeout on the final stage of file writing to %s" %
                        (self.current_volume))
                else:
                    self.set_volume_noaccess(
                        self.current_volume,
    ...
    def send_client_done(self, ticket, status, error_info=None):
        ...
        Trace.trace(
            13, "send_client_done status %s error_info %s ticket %s" %
            (status, error_info, ticket))
        if self.control_socket is None:
            return
        ticket['status'] = (status, error_info)
        if 'return_file_list' in ticket and len(
                ticket['return_file_list']) >= enstore_constants.MAX_UDP_PACKET_SIZE:
            print('Volume assert returned %s' % (ticket['return_file_list'],))
            sys.stdout.flush()
            flist_len = len(ticket['return_file_list'])
            del ticket['return_file_list']
            # send just the size of the list
            ticket['return_file_list_lengh'] = flist_len
        Trace.log(e_errors.INFO, "Sending done to client: %s" % (ticket))
        Trace.log(
            e_errors.INFO, "Sending done to client: %s" %
            (ticket['status'],))
        if ticket['status'][0] != e_errors.ENCP_GONE:
            try:
                ...
                exc, detail, tb = sys.exc_info()
                Trace.log(
                    e_errors.ERROR, "error in send_client_done: %s" %
                    (detail,))
        ...
        if ((self.method and self.method != 'read_next') or
            ...
            try:
                ...
                exc, detail, tb = sys.exc_info()
                Trace.log(
                    e_errors.ERROR, "error closing control and listen sockets: %s" %
                    (detail,))
            self.control_socket = None
            self.listen_socket = None
    ...
    def connect_client(self):
        ...
        host_ip = None
        try:
            caller_address_family = socket.getaddrinfo(
                ticket['callback_addr'][0], None)[0][0]
            hostinfo = socket.getaddrinfo(
                socket.gethostname(), caller_address_family)
        except Exception as detail:
            exc, msg, tb = sys.exc_info()
        ...
        try:
            if (not self.method) or self.method and self.method != 'read_next':
                try:
                    host, port, self.listen_socket = callback.get_callback(
                        ip=data_ip if data_ip else host_ip)
                except Exception as detail:
                    exc, msg, tb = sys.exc_info()
                    Trace.log(e_errors.WARNING, "connect_client: Connection to data ip failed:  %s %s %s" %
                              (exc, msg, traceback.format_tb(tb)))
                    Trace.alarm(
                        e_errors.WARNING, "Connection to data ip failed: %s" %
                        (detail,))
                    self.transfer_failed(
                        e_errors.ENCP_GONE, msg, error_source=NETWORK)
            ...
            if (not self.method) or (
                    self.method and self.method != 'read_next'):
                self.listen_socket.listen(1)
                # need a control connection setup
                # otherwise: not because it must be left open
                ticket['mover']['callback_addr'] = (
                    host, port)  # client expects this
                ticket['mover']['mover_address'] = (host, self.config['port'])

                self.control_socket = socket.socket(
                    caller_address_family, socket.SOCK_STREAM)
                flags = fcntl.fcntl(
                    self.control_socket.fileno(), fcntl.F_GETFL)
                fcntl.fcntl(
                ...
                if 'route_selection' in ticket and ticket['route_selection']:
                    ...
                    Trace.trace(
                        10, "callback socket %s" %
                        (u.get_tsd().socket.getsockname(),))
                    try:
                        x = u.send(
                            ticket,
                            ticket['routing_callback_addr'],
                            self.connect_to,
                            self.connect_retry,
                            0)
                    ...
                    if 'callback_addr' in x:
                        ticket['callback_addr'] = x['callback_addr']
                    self.del_udp_client(u)
                    #del u
                if self.method and self.method == 'read_tape_start':
                    self.udp_control_address = ticket.get(
                        'routing_callback_addr', None)
                    if self.udp_control_address:
                        # make sure that
                        self.reset_interval_timer(self.update_lm)
                        self.lm_address_saved = self.lm_address
                ...
                try:
                    ...
                    # get an ISCONN error.
                    if hasattr(
                            errno, 'EISCONN') and detail[0] == errno.EISCONN:
                        pass
                ...
                r, w, ex = select.select([self.control_socket], [self.control_socket], [
                ], self.connect_to * self.connect_retry)

                if r or w:
                    # Get the socket error condition...
                    rtn = self.control_socket.getsockopt(
                ...
                try:
                    # cgw - abstract this to a check_valid_filename method of
                    # the driver ?
                    null_err = 0
                    if self.config['driver'] == "NullDriver":
                        ...
                        for w in string.split(filename, '/'):
                            ...
                            # not found
                            ticket['status'] = (
                                e_errors.USERERROR, "NULL not in PNFS path")
                        ...
                        if not null_err:
                            if ticket['work'] == 'write_to_hsm':
                                ...
                                wrapper_type = volume_family.extract_wrapper(
                                    self.tmp_vf)
                                if wrapper_type != "null":
                                    ticket['status'] = (
                                        e_errors.USERERROR, 'only "null" wrapper is allowed for NULL mover')
                    ...
                    """
                    Trace.log(e_errors.INFO, "SENDING %s" % (ticket,))
                    rtn = callback.write_tcp_obj(
                        self.control_socket, ticket, timeout=10)
                    ...
                    if self.setup_mode == ASSERT:
                        ...
                        if callback_addr:
                            self.client_ip = hostaddr.address_to_name(
                                callback_addr[0])
                        else:
                            self.client_ip = ticket['wrapper']['machine'][1]
                        self.run_in_thread(
                            'volume_assert_thread', self.assert_vol)
                        return
                except BaseException:
                    exc, detail, tb = sys.exc_info()
                    Trace.log(
                        e_errors.ERROR, "error in connect_client: %s" %
                        (detail,))
                    # just for a case
                    try:
                        self.control_socket.close()
            ...
            read_fds, write_fds, exc_fds = select.select(
                [self.listen_socket], [], [], 20)  # 20 s TO
            Trace.trace(
                10, "select returned %s" %
                ((self.listen_socket in read_fds),))
            ...
            if self.listen_socket in read_fds:
                ...
                if data_ip:
                    ...
                    # where mover runs.
                    # this is why we bind to device only if network interfaces
                    # are different

                    host_addr = self.control_socket.getsockname()[0]
                    client_host_addr = hostaddr.name_to_address(
                    ...
                    if ((client_host_addr == host_addr) or  # client comes from the same host
                        ...
                        Trace.trace(
                            10, "bind client socket to %s" %
                            (data_interface,))
                        status = socket_ext.bindtodev(
                            self.client_socket.fileno(), data_interface)
                        if status:
                            Trace.log(
                                e_errors.ERROR, "bindtodev(%s): %s" %
                                (data_interface, os.strerror(status)))
                ...
                Trace.log(
                    e_errors.ERROR,
                    "timeout on waiting for client connect")
                # just for a case
                try:
                    if (not self.method) or (
                            self.method and self.method != 'read_next'):
    ...
    def format_lm_ticket(self, state=None, error_info=None,
                         returned_work=None, error_source=None):
        """
        ...
        if state is IDLE:
            ...
            # XXX If I'm offline should I send mover_error? I don't think
            # so....
            work = "mover_error"
            if self.setup_mode == ASSERT:
                volume_label = self.tmp_vol
        ...
        if work is None:
            Trace.log(
                e_errors.ERROR, "state: %s work: %s" %
                (state_name(state), work))

        if not status:
            status = e_errors.OK, None

        if not isinstance(status, type(())) or len(status) != 2:
            Trace.log(
                e_errors.ERROR, "status should be 2-tuple, is %s" %
                (status,))
            if len(status) == 1:
                status = (status, None)
            else:
    ...
    def run_in_thread(self, thread_name, function,
        ...
        for wait in range(5):
            if thread and thread.isAlive():
                Trace.trace(
                    20, "thread %s is already running, waiting %s" %
                    (thread_name, wait))
                time.sleep(1)
        if thread and thread.isAlive():
            Trace.log(
        ...
        Trace.trace(
            20, "create thread: target %s name %s args %s" %
        ...
        try:
            ...
            exc, detail, tb = sys.exc_info()
            Trace.log(
                e_errors.ERROR, "starting thread %s: %s" %
                (thread_name, detail))
        return 0
    ...
    def dismount_volume(self, after_function=None):
        ...
        if self.current_volume is None:  # no volume - no need to dismount
            if after_function:
                after_function()
            return
        Trace.trace(10, "state %s" % (state_name(self.state),))
                ...
                if self.driver_type == 'FTTDriver':
                    ...
                    if self.write_in_progress:
                        ...
                        if self.current_location - eod == 1:
                            ...
                            # Set it back to eod
                            Trace.log(
                                e_errors.WARNING, 'tape location %s is ahead of EOD %s. Will set to EOD' %
                        ...
                        try:
                            ...
                            exc, detail, tb = sys.exc_info()
                            Trace.log(
                                e_errors.ERROR, "error positioning to last fm %s %s" %
                                (exc, detail,))
                            return
                    self.update_tape_stats()
                    if self.write_counter != 0:
                        ...
                        if bloc_loc != self.last_absolute_location:
                            ...
                            try:
                                ...
                                Trace.alarm(
                                    e_errors.ERROR, "error writing file mark, will set %s readonly" %
                                    (self.current_volume,))
                                Trace.handle_error()
                                self.vcc.set_system_readonly(
                                    self.current_volume)
                                # also set volume to NOACCESS, so far
                                # no alarm is needed here because it is send by volume clerk
                                # when it sets a volume to NOACCESS
                                self.set_volume_noaccess(
                                    self.current_volume, "Error writing file mark. See log for details")
                                # log all running proceses
                                self.log_processes(logit=1)
        ...
        if self.stop and self.last_error[0] in (e_errors.WRITE_VOL1_READ_ERR,
            ...
            Trace.alarm(
                e_errors.INFO,
                'Tape %s will stay in the drive for investigation' %
                (self.current_volume))
            self.offline()
            return
        ...
        if ejected == -1:
            ...
            if self.can_force_eject:
                ...
                Trace.log(
                    e_errors.INFO,
                    "Eject failed. For STK robot will try to unload anyway")
                Trace.alarm(
                    e_errors.ERROR,
                    "Eject failed. Can be a problem with tape drive")
                after_function = self.offline
            else:
                ...
                if self.current_volume:
                    try:
                        ...
                        self.set_volume_noaccess(
                            self.current_volume, "Eject error. See log for details")
                    except BaseException:
                        exc, msg, tb = sys.exc_info()
                        broken = broken + \
                            " set_system_noaccess failed: %s %s" % (exc, msg)
        ...
        if ('external_label' in self.vol_info
        ...
        if not self.vol_info.get('external_label'):
            if self.vcc:
                if self.current_volume:
                    ...
                    Trace.log(
                        e_errors.ERROR, "dismount_volume: volume=%s" %
                        (self.current_volume,))
    ...
    def unload_volume(self, vol_info, after_function=None):
        ...
        if status and status[0] == e_errors.OK:
            ...
            Trace.log(e_errors.INFO, "dismounted %s %s %s" %
                      (self.current_volume, self.config['product_id'], time_msg))
            if self.draining:
                #self.state = OFFLINE
                self.offline()
            elif after_function:
        ...
        # on top of media changer error returns, but it doesn't  :-(
            ...
            if ((status[1] in (e_errors.MC_VOLNOTHOME, e_errors.MC_NONE,
                ...
                self.send_error_msg(error_info=s_status,
                                    ...
                                    returned_work=None)
                if self.control_socket:
                    self.send_client_done(
                        self.current_work_ticket,
                        e_errors.DISMOUNTFAILED,
                        s_status[0])
                    self.net_driver.close()
                    self.network_write_active = False  # reset to indicate no network activity
                Trace.alarm(
                    e_errors.ERROR, "dismount %s failed: %s" %
                    (self.current_volume, status))
            ...
            if self.current_volume:
                try:
                    self.set_volume_noaccess(
                        self.current_volume, "Dismount error. See log for details")
                except BaseException:
                    exc, msg, tb = sys.exc_info()
                    broken = broken + \
                        " set_system_noaccess failed: %s %s" % (exc, msg)
            if broken:
                ...
                # been mounted yet
                # Modify this to not call transfer_failed because this method
                # can be called from transfer_called
                self.transfer_failed(
                    exc=e_errors.DISMOUNTFAILED,
                    msg=broken,
                    error_source=ROBOT,
                    dismount_allowed=0)
                # this is to address AML2 dismount failures
                self.broken(broken, e_errors.DISMOUNTFAILED)
            time.sleep(3)
            self.offline()
        return
    ...
    def mount_volume(self, volume_label, after_function=None):
        ...
        # XXX DEBUG Block of code to get more info on why label is missing on
        # some mounts
        if not self.vol_info.get('external_label'):
            Trace.log(
                e_errors.ERROR,
                "mount_volume: no external label in vol_info.  volume_label=%s" %
                (volume_label,
                 ))
            if self.vcc:
                if self.current_volume:
        ...
        if not self.vol_info.get('media_type'):
            self.vol_info['media_type'] = self.media_type  # kludge
        # XXX END DEBUG Block of code to get more info on why label is missing
        # on some mounts
        ...
        if status[0] == e_errors.OK:
            ...
            if after_function:
                Trace.trace(
                    10, "mount: calling after function %s" %
                    (after_function,))
                after_function()
...
# self.dismount_volume(after_function=self.idle)
            ...
            if ((status[1] in (e_errors.MC_VOLNOTHOME, e_errors.MC_NONE,
                ...
                if status[1] == e_errors.MC_VOLNOTFOUND:
                    try:
                        self.set_volume_noaccess(
                            volume_label, "Volume not found. See log for details")
                    except BaseException:
                        exc, msg = sys.exc_info()[:2]
                        Trace.alarm(
                            e_errors.WARNING, "set_system_noaccess failed: %s %s" %
                            (exc, msg))

                if status[1] == e_errors.MC_DRVNOTEMPTY:
                    Trace.alarm(
                        e_errors.ERROR, "mount %s failed: %s" %
                        (volume_label, status))
                    self.offline()
                    return
                else:
            ...
            try:
                self.set_volume_noaccess(
                    volume_label, "Mount error. See log for details")
            except BaseException:
                exc, msg, tb = sys.exc_info()
                broken = broken + \
                    " set_system_noaccess failed: %s %s" % (exc, msg)
    ...
    def seek_to_location(self, location, eot_ok=0, after_function=None):
        ...
        Trace.trace(
            10, "seeking to %s, after_function=%s" %
        ...
        if self.state == ERROR:
            Trace.trace(
                e_errors.WARNING,
                "mover went to the error state while positioning tape. Will try to reset the mover")
            if self.current_volume:
            ...
            # the possible reason is a bad MIR causing a mover to seek for too
            # long
            return

        if after_function and not failed:
            Trace.trace(
                10, "seek calling after function %s" %
                (after_function,))
            after_function()
    ...
    def status(self, ticket):
        ...
        if self.state is HAVE_BOUND and self.dismount_time and self.dismount_time > now:
            tick['will dismount'] = 'in %.1f seconds' % (
                self.dismount_time - now)
        ticket.update(tick)
        self.reply_to_caller(ticket)
        # self.log_processes(logit=1)
        return
    ...
    def loadvol(self, vol_ticket):
        ...
        Trace.notify(
            "loading %s %s" %
            (self.shortname, vol_ticket['external_label']))
        self.state = MOUNT_WAIT
        mcc_reply = self.mcc.loadvol(vol_ticket, self.name, self.mc_device)
        # this status is 5-tuple like ('ok', 'ok', None, '', '')
        status = mcc_reply.get('status')
        if status and status[0] == e_errors.OK:
            Trace.notify(
                "loaded %s %s" %
    ...
    def unloadvol(self, vol_ticket):
        ...
        if not self.state in (
                HAVE_BOUND, IDLE):  # IDLE: tape may be in the mouth of the drive but not in the drive.
            vol_ticket['status'] = (
                e_errors.ERROR,
                'Mover is in state %s' %
                (state_name(
    ...
    def start_draining(self, ticket):
        ...
        out_ticket = {
            'status': (
                e_errors.OK, None), 'state': state_name(
                self.state), 'pid': os.getpid()}
        ticket.update(out_ticket)
        self.reply_to_caller(ticket)
        if save_state is HAVE_BOUND:
            Trace.log(
                e_errors.INFO, "Trying to dismount volume (21) %s" %
                (self.current_volume,))
            self.run_in_thread(
                'media_thread',
                self.dismount_volume,
    ...
    def warm_restart(self, ticket):
        ...
        while True:
            if self.state == OFFLINE:
                ...
                time.sleep(2)
                Trace.trace(
                    11, "waiting in state %s for OFFLINE" %
                    (self.state,))
            else:
                Trace.alarm(
                    e_errors.ERROR, "can not restart. State: %s" %
                    (self.state,))
    ...
    def clean_drive(self, ticket):
        ...
        if self.state not in (IDLE, OFFLINE):
            ret = {
                'status': (
                    "EPROTO",
                    "Cleaning not allowed in %s state" %
                    (state_name(
                        self.state)))}
        else:
            self.state = CLEANING
            ...
            if ret['status'][0] != e_errors.OK:
                Trace.alarm(
                    e_errors.WARNING, "clean request returned %s" %
                    (ret['status'],))
            self.state = save_state
        ticket.update(ret)
        self.reply_to_caller(ticket)
...
class DiskMover(Mover):
    ...
    def write_tape(self):
        ...
        """
        Trace.trace(
            8, "write_tape starting, bytes_to_write=%s" %
            (self.bytes_to_write,))
        Trace.trace(8, "bytes_to_transfer=%s" % (self.bytes_to_transfer,))
        driver = self.tape_driver
        count = 0
        ...
        Trace.trace(8, "write_tape exiting, wrote %s/%s bytes" %
                    (self.bytes_written, self.bytes_to_write))
        ...
        if self.bytes_written == self.bytes_to_write:
            ...
            if self.check_written_file():
                ...
                self.buffer.set_wrapper(saved_wrapper)
                Trace.trace(
                    22, "starting check after write, bytes_to_read=%s" %
                    (bytes_to_read,))
                driver = self.tape_driver
                first_block = 1
                while bytes_read < bytes_to_read:
                    ...
                    try:
                        ...
                        if detail == e_errors.CRC_ERROR:
                            ...
                            if detail == e_errors.ENCP_GONE:
                                ...
                                err_source = UNKNOWN
                            self.transfer_failed(
                                detail, error_source=err_source)
                        ...
                        #Trace.handle_error(exc, detail, tb)
                        Trace.alarm(
                            e_errors.ERROR, "selective CRC check error")
                        self.transfer_failed(
                            e_errors.WRITE_ERROR, detail, error_source=TAPE)
                        failed = 1
                        break
                    if b_read <= 0:
                        Trace.alarm(
                            e_errors.ERROR, "selective CRC check read error")
    ...
    def read_tape(self):
        ...
        """
        Trace.trace(
            8, "read_tape starting, bytes_to_read=%s" %
            (self.bytes_to_read,))
        if self.buffer.client_crc_on:
            # calculate checksum when reading from
            # tape (see comment in setup_transfer)
                ...
                if len(b0) >= self.wrapper.min_header_size:
                    try:
                        ...
                        Trace.log(e_errors.ERROR, "Invalid header %s" %
                                  (b0[:self.wrapper.min_header_size]))
                        self.transfer_failed(
                            e_errors.READ_ERROR, "Invalid file header %s" %
                            (msg,), error_source=TAPE)
                        # XXX NB: the client won't necessarily see this message since it's still trying
                        # to recieve data on the data socket
                        failed = 1
                        break
        ...
        if do_crc:
            ...
            if self.buffer.complete_crc != self.file_info['complete_crc']:
                ...
                Trace.log(e_errors.ERROR, "read_tape: calculated CRC %s File DB CRC %s" %
                          (self.buffer.complete_crc, self.file_info['complete_crc']))
                # this is to fix file db
                if self.file_info['complete_crc'] is None:
                    sanity_cookie = (
                        self.buffer.sanity_bytes,
                        self.buffer.sanity_crc)
                    Trace.log(e_errors.WARNING, "found complete CRC set to None in file DB for %s. Changing cookie to %s and CRC to %s" %
    ...
    def create_volume_name(self, ip_map, volume_family):
        ...
        # return string.join((ip_map,
        # volume_family,time.strftime("%Y-%m-%dT%H:%M:%SZ")),':')
    ...
    def setup_transfer(self, ticket, mode):
        ...
        ticket['mover'].update(self.config)
        ticket['mover']['device'] = "%s:%s" % (
            self.config['ip_map'], self.config['device'])
        self.current_package = ticket['fc'].get('external_label', None)
        if ticket['fc']['external_label'] is None:
    ...
    def finish_transfer_setup(self):
        ...
        if self.mode == READ:
            ...
            if 'pnfsid' in fc:
                self.file = enstore_functions3.file_id2path(
                    self.device, fc['pnfsid'])
                ...
                if not os.path.exists(dir_name):
                    try:
                        ...
                        if detail[0] != errno.EEXIST:
                            self.transfer_failed(
                                e_errors.OSERROR, str(detail), error_source=DRIVE)
                self.files = (client_filename, pnfs_filename)
            else:
                self.transfer_failed(
                    e_errors.MALFORMED, 'expected Key "pnfsid". Current dictionary %s' %
                    (fc,), error_source=USER)
                return
            if self.wrapper:
                self.header, self.trailer = self.wrapper.headers(
                    self.current_work_ticket['wrapper'])
            else:
                self.header = ''
                self.trailer = ''
            self.buffer.header_size = len(self.header)
    ...
    def stage_file(self):
        ...
        if not e_errors.is_ok(info):
            Trace.log(
                e_errors.ERROR, "stage_file: file clerk returned %s" %
                (info,))
            return None
        cache_status = info['cache_status']
        if cache_status != file_cache_status.CacheStatus.CACHED:
            Trace.log(
                e_errors.INFO, "staging status at the start %s" %
                (cache_status,))
            # make this better!
            ...
            if cache_status in (
                ...
                # in this intermediate state and preventing from being
                ...
                if not e_errors.is_ok(rc['status']):
                    Trace.log(
                        e_errors.ERROR, "Error setting cache status for %s: %s" %
                        (self.file_info['bfid'], rc))
                    return None
                return rv
            ...
            while not hasattr(self, 'too_long_in_state_sent'):
                ...
                if not e_errors.is_ok(info):
                    Trace.log(
                        e_errors.ERROR, "stage_file1: file clerk returned %s" %
                        (info,))
                    return None
                if info['cache_status'] in (file_cache_status.CacheStatus.PURGING_REQUESTED,
                    ...
                    if not open_bitfile_sent:
                        ...
                        if package_id:
                            rticket = self.fcc.open_bitfile_for_package(
                                package_id)
                        else:
                            rticket = self.fcc.open_bitfile(info['bfid'])
                        Trace.log(
                            DEBUG_LOG, "open_bifile returned %s" %
                            (rticket,))
                ...
                Trace.log(
                    DEBUG_LOG, "staging status %s cache_status %s" %
                ...
                if (info['cache_status'] == file_cache_status.CacheStatus.STAGING_REQUESTED and
                    ...
                    cache_status = info['cache_status']
                      ...
                      (cache_status in (file_cache_status.CacheStatus.STAGING,
                                        ...
                                        file_cache_status.CacheStatus.PURGING_REQUESTED))):
                    Trace.log(
                        e_errors.ERROR, "File staging has failed for %s %s " %
                        (info['bfid'], info['pnfs_name0']))
                    break
                time.sleep(2)
            ...
            if loop_counter % 100 == 0:
                ...
                if not self.check_connection():
                    self.transfer_failed(
                        e_errors.ENCP_GONE,
                        "encp gone while waiting for stage",
                        error_source=USER)
                    self.idle()
                    return None
            ...
            Trace.log(
                e_errors.INFO, "check if file is in cache %s" %
                (file_cache_location,))
    ...
    def position_media(self, filename):
        ...
        if filename:
            try:
                have_tape = self.tape_driver.open(
                    filename, self.mode, retry_count=30)
            except Exception as err:
                self.transfer_failed(
                    e_errors.MOUNTFAILED, 'mount failure: %s' %
                    (err,), error_source=DRIVE)
                self.idle()
                return
        else:
            if self.state != IDLE:
    ...
    def transfer_failed(self, exc=None, msg=None,
                        error_source=None, dismount_allowed=0):
        """
        ...
        if exc not in (e_errors.ENCP_GONE, e_errors.DELETED,
                       e_errors.NOACCESS, e_errors.NOTALLOWED):
            self.consecutive_failures = self.consecutive_failures + 1
            if self.consecutive_failures >= self.max_consecutive_failures:
                broken = "max_consecutive_failures (%d) reached" % (
                    self.max_consecutive_failures)
        ...
        if self.draining:
            ...
            if self.mode == READ:
                ...
                self.idle()
                self.current_volume = None
        Trace.log(e_errors.INFO, "transfer failed state %s" %
                  (state_name(self.state),))
    ...
    def transfer_completed(self):
        ...
        try:
            ...
            Trace.log(
                DEBUG_LOG, "transfer_completed error closing tape driver %s" %
                (str(detail),))
        if self.mode == WRITE:
            ...
            try:
                ...
                self.transfer_failed(
                    e_errors.OSERROR, 'transfer failure: %s' %
                    (str(detail),), error_source=DRIVE)
                self.offline()
                return

        Trace.trace(10, "transfer complete state0 %s" %
                    (state_name(self.state),))
        now = time.time()
        self.dismount_time = now + self.delay
        self.current_work_ticket['fc']['external_label'] = self.current_work_ticket['vc']['external_label']
        ...
        Trace.log(
            e_errors.INFO,
            "transfer complete state %s" %
            (state_name(
                self.state),
             ))
        Trace.trace(10, "transfer complete 11 state %s" %
                    (state_name(self.state),))
        if self.draining:
            self.offline()
        self.log_state()
        Trace.trace(10, "transfer complete  22 state %s" %
                    (state_name(self.state),))
        self.need_lm_update = (1, None, 1, None)
    ...
    def update_after_writing(self):
        ...
        if v['status'][0] == e_errors.NO_VOLUME:
            ...
            r = self.vcc.add(self.vol_info['library'],
                             volume_family.extract_file_family(
                                 self.vol_info['volume_family']),
                             volume_family.extract_storage_group(
                                 self.vol_info['volume_family']),
                             'disk',
                             self.current_volume,
                             r2,
                             eod_cookie='0000_000000000_0000001',
                             # actually disk file will have no wrapper
                             wrapper=self.vol_info['wrapper'],
    ...
    def format_lm_ticket(self, state=None, error_info=None,
        ...
        Trace.trace(
            20,
            "format_lm_ticket: CV %s lv %s vf %s lvf %s" %
            (self.current_volume,
             self.last_volume,
             self.volume_family,
             self.last_volume_family))
        if self.current_volume:
            volume_label = self.current_volume
            volume_family = self.volume_family
        else:
...
def identify_mc_device(device):
    ...
    for ch in changers:
        host = os.listdir(
            os.path.join(
                path_to_classes,
                ch,
                'device/scsi_device'))[0].split(':')[0]  # scsi devise is in format: 10:0:0:1
        sg = os.listdir(
            os.path.join(
                path_to_classes,
                ch,
                'device/scsi_generic'))[0]
    ...
    if ret:
        ...
        if os.path.exists(changer_dev):
            Trace.log(
                e_errors.INFO, 'media changer device %s' %
                (changer_dev,))
            return changer_dev
        else:
            return None
...
if __name__ == '__main__':
    ...
    if use_local_mc:
        ...
        if not mc_device:
            Trace.log(
                e_errors.ERROR,
                "Can not start. Can not identify media changer device")
            sys.exit(-1)
        mcc_parameters['mc_device'] = mc_device
        media_changer_cl = create_instance(
            mcc_module, mcc_class, mcc_parameters)
    mover = constructor(
        (intf.config_host,
         intf.config_port),
        intf.name,
