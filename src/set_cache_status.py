...
def modify_records(fcc, file_records):
    Trace.trace(10, "modify_records: file_records %s %s"%(len(file_records), file_records))
    list_of_records = []
    tmp_list = []

    for record in file_records: 
        if len(str(tmp_list)) + len(str(record)) < enstore_constants.MAX_UDP_PACKET_SIZE-PAD_SIZE:
            tmp_list.append(record)
        else:
           Trace.trace(10, "modify_records: list_of_records appending %s"%(tmp_list,)) 
    ...
    if tmp_list:
        # append the rest
        list_of_records.append(tmp_list)
    Trace.trace(10, "modify_records: params %s %s"%(len(list_of_records), list_of_records,))
