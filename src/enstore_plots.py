...
# there may be several different drive ids that correspond to one type of
# drive.
...
def sort_stamp_files(tmp_stamps):
    ...
    while i < num_stamps:
        if string.find(tmp_stamps[i][0], enstore_constants.MPH_FILE) == -1:
            ...
            # this is mph file, leave it here for later sorting, skip to next
            # stamp
            i = i + 1
    # now we should have jpg_stamp_files full of non mph stamps and only mph stamps
    # left in tmp_stamps.  reverse sort these and add them at end of other
    # stamps
    tmp_stamps.reverse()
    ...
    def get_all_mounts(self, new_mounts_l):
        ...
        if self.openfile:
            ...
            for line in mounts_l:
                ...
                if day in new_mounts_d:
                    # replace the old count with the new value
                    updated_mounts_l.append(
                        "%s %s\n" %
    ...
    def plot(self, total_mounts, total_mounts_type):
        ...
        for drive_type in total_mounts_type.keys():
            ...
            if total_mounts_type[drive_type]:
                self.files_to_install[drive_type] = MpdDriveTypeDataFile(
                    self.dir, drive_type)
                self.files_to_install[drive_type].open()
                self.files_to_install[drive_type].plot(
                    total_mounts_type[drive_type])
...
class XferGnuFile(enstore_files.EnFile):
    ...
    def write(self, outfile1, outfile2, ptsfile1, ptsfile2, extra=None):
        ...
        if (extra is None):
            ...
            long_string = long_string + "set title '" + extra + \
                ": Individual Transfer Activity (no null mvs)"
...
class XferDataFile(EnPlot):
    ...
    def plot(self, data):
        ...
        for [xpt, ypt, type, mover, drive_id, sg] in data:
            if (self.sg is None or self.sg == sg):
                if type == WRITE:
...
class BpdMoverGnuFile(enstore_files.EnFile):
    ...
    def write(self, mover, outfile, ptsfile, total, meansize, xfers, lw=20):
        self.openfile.write("set output '" + outfile + "'\n" +
                            ...
                            "plot '" + ptsfile +
                            "' using 1:2 t 'reads' w impulses lw " + repr(lw) + " lt 3 , '" + ptsfile +
                            "' using 1:4 t 'writes' w impulses lw " +
                            repr(lw) + " lt 1 \n"
                            )
...
class BpdMoverDataFile(EnPlot):
    ...
    def __init__(self, dir, mover):
        EnPlot.__init__(
            self, dir, "%s-%s" %
            (enstore_constants.BPD_FILE, mover))
    ...
    def install(self, dir, use_subdir):
        ...
        if not dir == new_dir:
            # we moved the files to the sub dir so on cleanup, delete them from
            # here
...
class BpdDataFile(EnPlot):
    ...
    def sum_movers(self, mover_d):
        ...
        if drive_id not in self.movers_d:
    ...
    def sum_data(self, data):
        ...
        for [xpt, ypt, type, mover, drive_id, sg] in data:
            ...
            if mover in self.movers_d:
                if adate not in self.movers_d[mover]:
    ...
    def get_total_bpd(self, bpd_l):
        ...
        for bpd in bpd_l:
            ...
            if len(fields) > 1:
                ...
                num_xfers = num_xfers + \
                    long(float(fields[1]) / float(fields[6]))
...
class TotalBpdDataFile(EnPlot):
    ...
    # make the file with the bytes per day format, first we must sum the data
    # that we have based on the day.  we will also make the plot of total
    # writes/day
