...
class Inquisitor(generic_client.GenericClient):
    ...
    def outage(self, server_list, time, rcv_timeout=0, tries=0):
        # tell the inquisitor to mark the passed servers as scheduled for an
        # outage
    ...
    def override(self, server_list, status,
                 reason="<no given reason>", rcv_timeout=0, tries=0):
    ...
    def print_show(self, ticket):
        ...
        if override_d:
            ...
            print(" ------------------------------")
            for key in keys:
                if isinstance(override_d[key], list):
                    secs_down = now - override_d[key][1]
                    print(
                        "   %s : %s for %.0d seconds" %
                        (key, override_d[key][0], secs_down))
                else:
...
def do_work(intf):
    ...
    if ticket:
        ...
        servers_l = sorted(ticket['servers'].keys())
        ...
        if intf.saagstatus in enstore_constants.SAAG_STATUS:
            if intf.reason:
                ticket = iqc.override(
                    intf.override, intf.saagstatus, reason=intf.reason)
            else:
            ...
            try:
                ...
                sys.stderr.write(
                    "    Legal values are: red, yellow, green, question\n")
                sys.stderr.flush()
            except IOError:
                pass
            intf.print_help()
