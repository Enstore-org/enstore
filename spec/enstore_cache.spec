...
Source: enstore_cache.tgz
BuildRoot: /usr/src/redhat/BUILD
AutoReqProv: no
AutoProv: no
AutoReq: no
Prefix: opt/enstore_cache
Requires: postgresql-libs boost

%description
Enstore with cache. Enstore is a Distributed Mass Storage System. 
The main storage media it uses is magnetic tape, although the new media can be added.
This product utilizes disks and disk movers.
The files vcan be written to enstore cache and then migrated to a tape.
The files can be aggregated before migrated.
...
CUMIN_DIR=$RPM_BUILD_ROOT/%{prefix}/cumin
#cp -rp $CUMIN $CUMIN_DIR
...
* Thu Dec 08 2011  <moibenko@fnal.gov> -
- modified encp to not loop in
- _get_stat for write requests
- fixed migrator
