...
for db in ${dbs}; do
    psql -U enstore -qAt -c "SELECT 'GRANT SELECT ON ' || relname || ' TO enstore_reader;' FROM pg_class JOIN pg_namespace ON pg_namespace.oid = pg_class.relnamespace WHERE nspname = 'public' AND relkind IN ('r', 'v');"  $db | grep "^GRANT" | psql -U enstore $db
done