...
#include "ftt_partition.h"

extern int ftt_describe_error(ftt_descriptor d, int opn, char *op, int keep_res, int res, char *what, int recoverable);
ftt_partbuf
ftt_alloc_parts() {
     ftt_partbuf res;
...
int
ftt_part_util_set(ftt_descriptor d,  ftt_partbuf p ) {
    int res, i;
    int len;
    int timeout;
