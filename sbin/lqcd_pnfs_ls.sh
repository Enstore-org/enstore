...
html_file=lqcd_pnfs_ls.html
tmp_file=${html_file}.tmp

theDate=`date`

rm -f $tmp_file
echo "<HTML><HEAD><TITLE>LQCD PNFS DIRECTORY LISTING</TITLE></HEAD>" > $tmp_file
echo "<BODY TEXT=#000066 BGCOLOR=#FFFFFF LINK=#0000EF VLINK=#55188A ALINK=#FF0000 BACKGROUND=enstore.gif>" >> $tmp_file
echo "<BR><BR><FONT SIZE=+4><B>Directory Listing of /pnfs/lqcd</B></FONT>" >> $tmp_file
echo "<PRE>" >> $tmp_file
echo "  " >> $tmp_file
echo "  " >> $tmp_file
echo "  " >> $tmp_file
...
ls -l -F -R /pnfs/lqcd >> $tmp_file 2>/dev/null
