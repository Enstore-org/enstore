...
# was xferby_month
SELECT_STMT1 = "select date,sum(read),sum(write) from xfer_by_day group by date order by date"
...
def fill_tape_histograms(i, server_name, server_port, hlist):
    ...
    for row in res.getresult():
        sg = row[2]
        if sg not in hlist:
            hlist[sg] = histogram.Histogram1D(
                "on_tape_by_month_%s" %
                (sg,), "Total Bytes On Tape by Month By %s" %
                (sg,), nbins, float(start_day), float(now_day))
        h = hlist[sg]
...
def plot_bpd():
    ...
    tmp.add_text(
        "set label \"Total :  %5d TiB  \" at graph .05,.9  font \"Helvetica,13\"\n" %
        (t_month + 0.5,))
    ...
    t_month_min, i_month_min, t_month_max, i_month_max = get_min_max(tmp)
    tmp.add_text(
        "set label \"Total Transferred :  %5d TiB  \" at graph .1,.8  font \"Helvetica,13\"\n" %
        (t_month_max + 0.5,))
    tmp.set_line_color(1)
    tmp.set_marker_type("impulses")
    iplotter.plot()
    ...
    t_month_min, i_month_min, t_month_max, i_month_max = get_min_max(tmp)
    tmp.add_text(
        "set label \"Total Written :  %5d TiB  \" at graph .1,.8  font \"Helvetica,13\"\n" %
        (t_month_max + 0.5,))
    tmp.set_line_color(1)
    tmp.set_marker_type("impulses")
    iplotter1.plot()
...
def plot_bytes():
    ...
    plotter1 = histogram.Plotter(
        "deletes_total_by_month",
        "Total TiB deleted per month from Enstore")
    ...
    iplotter1 = histogram.Plotter(
        ...
        "Integrated Total TiB deleted per month from Enstore")
    ...
    for p in plotters:
        ...
        tmp.add_text(
            "set label \"Total :  %5d TiB  \" at graph .8,.8  font \"Helvetica,13\"\n" %
            (t_day + 0.5,))
...
def plot_tape_bytes():
    ...
    s1 = histogram.Histogram1D(
        "on_tape_total_by_month",
        "Active bytes on tape in Enstore",
        nbins,
        float(start_day),
        float(now_day))
    s1.set_time_axis(True)
    ...
    values = sorted(histograms.values())
    for h1 in values:
        tmp = s1 + h1
    ...
    for p in plotters:
        ...
        tmp.add_text(
            "set label \"Total   :  %5d TiB \" at graph .05,.95  font \"Helvetica,13\"\n" %
            (t_day_max + 0.5))
