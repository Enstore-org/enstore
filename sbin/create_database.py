...
def create_user_and_group(groupname,
    ...
    if not find_group(groupname):
        ...
        if os.system(cmd) != 0 :
            dump_restore_database.print_error("Failed to execute command %s"%(cmd))
            return 1
    if not find_user(username):
        if not find_user_id(user_id):
            cmd = createuser + " -u %d -g %s %s "%(user_id,groupname,username)
        else:
...
def move_directory(src,dst):
    try:
        dst=dst+".%s"%(time.strftime("%Y-%m-%d.%H:%M:%S",time.localtime()))
        os.rename(src,dst)
    except OSError, msg:
        dump_restore_database.print_error("Failed to move %s to %s"%(src,dst))
...
def modify_pg_hba_file(filename,dbname,dbuser,dbuser_reader):
    ...
    for ip in os.popen(cmd).readlines():
        fp.write("host %s %s %s/24 trust \n"%(dbname,dbuser,ip.strip()[:-1]))
        fp.write("host %s %s %s/24 trust \n"%(dbname,dbuser_reader,ip.strip()[:-1]))
    fp.close()
...
def check_database(dbport, dbserverowner):
    ...
    for i in range(5):
        ...
        if os.system(cmd) == 0:
            ...
            dump_restore_database.print_error("database is not ready, trying %d"%(i))
    return 1

def create_database_user(dbport,dbuser):
    return os.system("psql template1 -p %d -c \"create user %s password \'enstore_user\' createdb;\""%(dbport,dbuser))
...
if __name__ == "__main__" :
    ...
    if not enstore_dir:
        dump_restore_database.print_error("ENSTORE_DIR variable is not defined, please setup enstore first")
        sys.exit(1)
    ...
    if create_user_and_group(dbserverowner,GROUPID,dbserverowner,USERID) != 0 :
        sys.exit(1)
    user_data = get_user_info(dbserverowner)
    if not user_data :
        dump_restore_database.print_error("Failed to find user data for user %s"%(dbserverowner))
        sys.exit(1)
    uid = user_data[2]
    gid = user_data[3]
    if not dbarea :
    ...
    if not schema_file:
        dump_restore_database.print_error("Failed to find schema file for database %s (%s)"%(dbname,schema_file))
        sys.exit(1)
    schema_file=os.path.join(enstore_dir,schema_file)
    if not os.path.exists(schema_file) :
        dump_restore_database.print_error("Schema file %s does not exist"%(schema_file))
        sys.exit(1)
    pid = find_pid(dbserver_cmd, dbarea)
    if pid :
        dump_restore_database.print_error("ERROR: database server is still running.")
        dump_restore_database.print_error("%d"%(pid))
        dump_restore_database.print_error("ERROR: database %s creation failed!"%(dbname))
        dump_restore_database.print_error("ERROR: stop above database server first!")
        sys.exit(1)
    if os.path.exists(dbarea):
        dump_restore_database.print_message("Database area %s already exists"%(dbarea))
        dump_restore_database.print_message("Moving it on the side")
        if move_directory(dbarea,dbarea) !=0 :
            sys.exit(1)
    try:
        dump_restore_database.print_message("Creating database area %s"%(dbarea))
        os.makedirs(dbarea,0777)
    except:
        dump_restore_database.print_error("Failed to create database area %s"%(dbarea))
        sys.exit(1)
    try:
        dump_restore_database.print_message("chown to uid=%d gid=%d"%(uid,gid))
        os.chown(dbarea,uid,gid)
    except:
        dump_restore_database.print_error("Failed to chown to uid=%d gid=%d"%(uid,gid))
        sys.exit(1)
    if dbname == "enstoredb":
        for d in ["db_dir","jou_dir"]:
            value = server.get(d,None)
            if not value :
                dump_restore_database.print_error("%s directory is not defined "%(d))
                sys.exit(1)
            else:
                if not os.path.exists(value):
                    try:
                        os.makedirs(value,0777)
                    except:
    ...
    if ourhost.startswith("cdfen"):
        ...
        pg_hba=os.path.join(enstore_dir,"databases/control_files/pg_hba.conf-d0en-%s"%(dbname))
    elif ourhost.startswith("cdfen"):
        pg_hba=os.path.join(enstore_dir,"databases/control_files/pg_hba.conf-cdfen-%s"%(dbname))
    else:
        pg_hba=os.path.join(enstore_dir,"databases/control_files/pg_hba.conf")
    if not os.path.exists(pg_hba) :
        dump_restore_database.print_error("pg_hba %s does not exist"%(pg_hba))
        sys.exit(1)
    #
    ...
    if create_database_user(dbport,dbuser) !=0 :
        dump_restore_database.print_error("Failed to create database user %s"%(dbuser))
        sys.exit(1)
    if create_database_read_user(dbport,dbuser_reader) !=0 :
        dump_restore_database.print_error("Failed to create database read user %s"%(dbuser_reader))
        sys.exit(1)
    if create_database(dbport,dbname) != 0:
        dump_restore_database.print_error("Failed to create database %s"%(dbname))
        sys.exit(1)
