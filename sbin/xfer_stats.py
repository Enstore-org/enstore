...
def fill_histograms(i,server_name,server_port,hlist):
    ...
    for row in res.getresult():
        if not row:
            continue
        h2.fill(time.mktime(time.strptime(row[0],'%Y-%m-%d %H:%M:%S')),row[1]/TB)
        h3.fill(time.mktime(time.strptime(row[0],'%Y-%m-%d %H:%M:%S')),row[1]/TB)
...
def plot_bpd():
    ...
    plotter1=histogram.Plotter("xfers_total_by_month","Total TBytes Transferred per Month By Enstore")
    ...
    s3_i  = histogram.Histogram1D("integrated_xfers_total_by_month","Integarted total Bytes transferred per Month By Enstore",nbins,float(start_day),float(now_day))
    ...
    iplotter1=histogram.Plotter("integrated_xfers_total_by_month","Integarted total Bytes transferred per Month By Enstore")
    ...
    for server in servers:
        ...
        if ( server_port != None ):
            ...
            h1  = histogram.Histogram1D("xfers_write_by_day_%s"%(name,),"Total Bytes Written per Day By  %s"%(server,),31,float(start_time),float(now_time))
            h2  = histogram.Histogram1D("xfers_read_by_day_%s"%(name,),"Total Bytes Read per Day By  %s"%(server,),31,float(start_time),float(now_time))

            h3  = histogram.Histogram1D("xfers_total_by_month_%s"%(name,),"Total Bytes Transferred per Month By %s"%(server,),nbins,float(start_day),float(now_day))
            h4  = histogram.Histogram1D("xfers_write_by_month_%s"%(name,),"Total Bytes Written per Month By %s"%(server,),nbins,float(start_day),float(now_day))
            h5  = histogram.Histogram1D("xfers_read_by_month_%s"%(name,),"Total Bytes Read per Month By %s"%(server,),nbins,float(start_day),float(now_day))
...
#                h2.fill(time.mktime(time.strptime(row[0],'%Y-%m-%d')),row[1])
            ...
            for row in res.getresult():
                if not row:
                    continue
                h3.fill(time.mktime(time.strptime(row[0],'%Y-%m-%d')),(row[1]+row[2])/TB)
...
#                h5.fill(time.mktime(time.strptime(row[0],'%Y-%m-%d')),row[1])
            ...
            tmp=s3_i+integral3
            tmp.set_name("integrated_xfers_monthly_%s"%(integral3.get_marker_text(),))
            tmp.set_data_file_name("integrated_xfers_monthly_%s"%(integral3.get_marker_text(),))
...
def plot_bytes():
    ...
    s  = histogram.Histogram1D("writes_total_by_month","Total bytes written per month to Enstore",nbins,float(start_day),float(now_day))
    s1 = histogram.Histogram1D("writes_total_by_day","Total bytes written per day by Enstore",31,float(start_time),float(now_time))
    s2 = histogram.Histogram1D("deletes_total_by_month","Total bytes deleted per month from Enstore",nbins,float(start_day),float(now_day))
    s3 = histogram.Histogram1D("deletes_total_by_day","Total bytes deleted  per day from Enstore",31,float(start_time),float(now_time))
    ...
    plotter1=histogram.Plotter("writes_total_by_day","Total TBytes written per day by Enstore")
    plotter2=histogram.Plotter("deletes_total_by_month","Total TBytes deleted per month from Enstore")
    plotter3=histogram.Plotter("deletes_total_by_day","Total TBytes deleted per day from Enstore")
    ...
    s1_i = histogram.Histogram1D("writes_total_by_day","Integrated Total bytes written per day by Enstore",31,float(start_time),float(now_time))
    s2_i = histogram.Histogram1D("deletes_total_by_month","Integrated Total bytes deleted per month from Enstore",nbins,float(start_day),float(now_day))
    s3_i = histogram.Histogram1D("deletes_total_by_day","Integrated Total bytes deleted  per day from Enstore",31,float(start_time),float(now_time))
    ...
    iplotter1=histogram.Plotter("integrated_writes_total_by_day","Integrated Total TBytes written per day by Enstore")
    iplotter2=histogram.Plotter("integrated_deletes_total_by_month","Integrated Total TBytes deleted per month from Enstore")
    iplotter3=histogram.Plotter("integrated_deletes_total_by_day","Integrated Total TBytes deleted per day from Enstore")
...
#        if (server == "stken") : continue
    ...
    for i in range(len(histograms)/4):
        ...
        tmp=s1_i+integral1
        tmp.set_name("integrated_writes_daily_%s"%(integral1.get_marker_text(),))
        tmp.set_data_file_name("integrated_writes_daily_%s"%(integral1.get_marker_text(),))
        ...
        tmp.set_data_file_name("integrated_deletes_monthly_%s"%(h2.get_marker_text(),))
        tmp.set_marker_text(h2.get_marker_text())
        tmp.set_time_axis(True)
        tmp.set_ylabel(h2.get_ylabel())
        tmp.set_marker_type(h2.get_marker_type())
        ...
        tmp=s3_i+integral3
        tmp.set_name("integrated_deletes_daily_%s"%(integral3.get_marker_text(),))
        tmp.set_data_file_name("integrated_deletes_daily_%s"%(integral3.get_marker_text(),))
        tmp.set_marker_text(integral3.get_marker_text())
        tmp.set_time_axis(True)
        tmp.set_ylabel(integral3.get_ylabel())
        tmp.set_marker_type(integral3.get_marker_type())
